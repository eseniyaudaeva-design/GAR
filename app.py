...
# ==========================================
# TAB 5: BULK LSI GENERATOR (PRO B2B - FINAL POLISHED)
# ==========================================
import requests
...
with tab_lsi_gen:
    st.header("🏭 Массовая генерация B2B (Smart Case + Live UI)")
    st.markdown("Умные заголовки (строчные буквы). Исправленный мониторинг.")
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        stop_words_list = (
            ...
            "экономика проекта, сервис который экономит, мы ценим ваше время"
        )
        ...
        [I] ГЛАВНЫЕ ЗАПРЕТЫ:
        1. ЗАПРЕЩЕНО создавать отдельные блоки/абзацы про "Сервис", "Доставку", "Оплату" (воду).
        2. ЗАПРЕЩЕНО писать "Мы предлагаем", "Наша компания". 
        3. Коммерческие условия указывай ТОЛЬКО сухим списком в самом конце.
        
        [II] СТРУКТУРА И ГРАММАТИКА ЗАГОЛОВКОВ:
        
        1.1. Заголовок H2: Выведи строго: <h2>{exact_h2}</h2>.
        
        1.2. БЭНГЕР (Хук):
        2-3 ударных предложения про надежность. Без "Предлагаем".
        
        1.3. Абзац 1 (Технический) + Контакты:
        {contact_html_block}
        
        1.4. Подводка к списку 1 (заканчивается двоеточием :):
        
        1.5. Список №1 (6 пунктов): ФИЗИКО-МЕХАНИЧЕСКИЕ ХАРАКТЕРИСТИКИ.
        - Конкретные цифры, диапазоны, марки стали.
           
        1.6. Абзац 2 (Технический): Особенности производства / ГОСТ.

        1.7. Подзаголовок H3: 
        Придумай технический заголовок, содержащий название товара "{exact_h2}".
        ВАЖНО: Если название товара идет в середине предложения, ПИШИ ЕГО С МАЛЕНЬКОЙ БУКВЫ.
        ПЛОХО: "Технические требования к Полосе стальной".
        ХОРОШО: "Технические требования к полосе стальной".
        ...
        Придумай заголовок (например, про Классификацию или Сортамент), включи в него название товара "{exact_h2}".
        ВАЖНО: Соблюдай падежи и пиши товар со строчной буквы, если это не начало фразы.
        
        1.13. Абзац 5 (Технический).
        
        1.14. Подводка к списку 3 (заканчивается двоеточием :):
        
        1.15. Список №3 (6 пунктов): ПАРАМЕТРЫ ПОСТАВКИ.
        (Сухие факты: Отгрузка, Упаковка, Сертификаты. Без рекламы сервиса).
    ...
    with st.expander("⚙️ Настройки и LSI", expanded=True):
        ...
        with c1:
            lsi_api_key = st.text_input("Gemini API Key", value=cached_key, type="password", key="bulk_api_key_v7")
        with c2:
    ...
    if total_q > 0:
        ...
        def render_live_table():
            ...
            # Если результатов нет, но очередь есть (и мы не в процессе запуска)
            # Мы просто ничего не пишем, чтобы не раздражать "Пустым списком"

        # Вызываем один раз для отрисовки при загрузке страницы
        render_live_table()
        ...
        if run_btn:
            if not lsi_api_key:
                ...
                # ОЧИЩАЕМ ПЛЕЙСХОЛДЕР ПЕРЕД СТАРТОМ И ПИШЕМ СТАТУС
