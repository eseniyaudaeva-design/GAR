...
def generate_five_blocks(client, base_text, tag_name, seo_words=None):
    ...
    3. СТРОГИЙ ЗАПРЕТ: Используй тег <b> ТОЛЬКО для этих SEO-слов. Не выделяй жирным ничего другого (ни вводные фразы, ни термины, если их нет в списке).
    4. ЕСТЕСТВЕННОСТЬ: Склоняй слова, меняй окончания и падежи.
    5. КОНТЕКСТ: Если слово "СПБ" — пиши "Санкт-Петербург", "Дон" — "Ростов-на-Дону".
    -------------------------------------------
        """
    ...
    5. Второй (завершающий) абзац текста (<p>).
    
    ТЕМЫ БЛОКОВ:
    --- БЛОК 1 (Вводный) ---
    - Заголовок: <h2>{tag_name}</h2>
    - Описание товара, назначение.
    
    --- БЛОКИ 2, 3, 4, 5 (Технические детали) ---
    - Заголовки: <h3> (Характеристики, Применение, Производство, Особенности и т.д.).
