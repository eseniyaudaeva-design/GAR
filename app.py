...
1. МОРФОЛОГИЯ: Смело меняй окончания, числа и падежи. (Например: если ключ "алюминий", можно писать "алюминиевого", "из алюминия").
...
4. КОНТЕКСТ: Если слово совсем не подходит по смыслу (например "медь" в тексте про "сталь"), пропусти его, но постарайся внедрить максимум возможных.
-------------------------------------------
"""

    # 2. СИСТЕМНЫЙ ПРОМТ (Добавлена роль SEO-оптимизатора)
    system_instruction = (
    ...
    try:
        ...
        # Удаляем жирное, НО НЕ то, что мы сами попросили (b)
        # Ранее тут стояла чистка </?b>, она удаляла наши SEO-выделения. УБИРАЕМ ЕЁ.
        # content = re.sub(r'</?(b|strong)>', '', content, flags=re.IGNORECASE) <--- ЭТУ СТРОКУ УДАЛЯЕМ ИЛИ КОММЕНТИРУЕМ
