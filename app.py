...
# === ЦИКЛ ПО ПАЧКЕ (УМНАЯ ЛОГИКА: ТЕКСТ -> КОНТЕКСТ -> ТАБЛИЦА) ===
        for i, page in enumerate(target_pages_batch):
            ...
            try:
                ...
                # =========================================================
                # ШАГ 2. ГЕНЕРАЦИЯ ТАБЛИЦ (КОНТЕКСТНАЯ ИНТЕГРАЦИЯ)
                # =========================================================
                if use_tables and client:
                    # Переменная "Память", чтобы таблицы не дублировали друг друга
                    ...
                    for t_topic in table_prompts:
                        ...
                        # === ПРОМТ v5.1 (SMART CONTEXT INTEGRATION) ===
                        # Убрал "Любой ценой". Добавил "Органично впиши".
                        prompt_tbl = f"""
    ТЫ — ГЛАВНЫЙ ТЕХНОЛОГ. Задача: HTML-таблица для "{header_for_ai}".
    ТЕМА ТАБЛИЦЫ: {t_topic} ({topic_guide})
    
    ИСХОДНЫЕ ДАННЫЕ:
    1. 🧠 КОНТЕКСТ СТАТЬИ (ЧИТАТЬ ОБЯЗАТЕЛЬНО):
    {context_snippet}
    (Используй этот текст, чтобы понять, о чем речь, и какие параметры важны).
    
    2. 🔑 КЛЮЧЕВЫЕ СЛОВА (ДАННЫЕ ИЗ БАЗЫ):
    [{tech_data}]
    
    3. 🚫 ИСТОРИЯ (ЧТО УЖЕ НАПИСАНО В ПРЕДЫДУЩИХ ТАБЛИЦАХ):
    {previous_tables_context}
    (Не повторяй параметры, которые уже есть в этом списке).
    
    --- ИНСТРУКЦИЯ ПО ЛОГИКЕ ---
    1. АНАЛИЗ КЛЮЧЕЙ: Посмотри на список "КЛЮЧЕВЫЕ СЛОВА". Сопоставь их с "КОНТЕКСТОМ СТАТЬИ".
       - Если ключ — это тип изделия (например, "ВГП"), а тема таблицы позволяет -> создай строку "Вид/Тип" и впиши его.
       - Если ключ — это размер -> впиши в размеры.
       - Найди каждому ключу ЛОГИЧНОЕ место в таблице. Не теряй их, но и не лепи наугад.
       
    2. СТРУКТУРА: 
       - Таблица должна дополнять текст, а не просто копировать его.
       - Если в "Истории" уже есть "Хим состав", не пиши его снова здесь. Пиши про механику или размеры.
       
    3. ОФОРМЛЕНИЕ:
       - Используй тег <b> для выделения значений из списка ключей.
    
    Выдай только код <table> с классом 'brand-accent-table'.
    """
                        try:
                            # Температура 0.35 - оптимально для логики + небольшой свободы формулировок
                            ...
                            if start_idx != -1 and end_idx != -1:
                                ...
                                # Запоминаем контент этой таблицы для следующей итерации
