...
st.markdown(f"""
   <style>
        ...
        /* 1. ГЛАВНЫЙ УДАР ПО ОРАНЖЕВОМУ ЦВЕТУ */
        :root {{
            --primary-color: {PRIMARY_COLOR} !important; /* Меняем глобальную переменную на синий */
        }}
        ...
        /* ======================================================= */
        /* ИСПРАВЛЕНИЕ КРУЖКОВ (RADIO BUTTONS)                     */
        /* ======================================================= */

        /* 1. СОСТОЯНИЕ: НЕ ВЫБРАН */
        /* Находим div с атрибутом radio и его первый вложенный div (сам круг) */
        div[data-baseweb="radio"] > div {{
            background-color: #FFFFFF !important;  /* Белый фон */
            border: 1px solid #000000 !important;  /* Черная обводка */
        }}

        /* 2. СОСТОЯНИЕ: ВЫБРАН (CHECKED) */
        /* Ищем input, который нажат, и берем следующий за ним div (сам виджет) */
        input:checked + div[data-baseweb="radio"] > div {{
            background-color: {PRIMARY_COLOR} !important; /* СИНИЙ фон */
            border: 1px solid {PRIMARY_COLOR} !important; /* СИНЯЯ обводка */
        }}
        
        /* 3. БЕЛАЯ ТОЧКА ВНУТРИ ВЫБРАННОГО */
        input:checked + div[data-baseweb="radio"] > div > div {{
