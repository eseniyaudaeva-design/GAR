...
with tab_lsi_gen:
    ...
    **единый слитный текст** (объемом как 3 блока в Оптовом генераторе, ~2500 знаков).
    ...
    def generate_lsi_article(api_key, topic, lsi_keywords):
        ...
        ОБЪЕМ:
        Текст должен быть объемом примерно 2500 символов (эквивалент 3-х текстовых блоков).
        
        LSI ИНСТРУКЦИЯ:
        Равномерно распредели по тексту следующие слова: {{{lsi_string}}}.
        Вписывай их естественно, меняя окончания и падежи. НЕ выделяй их жирным.
        
        СТРУКТУРА (Единый HTML):
        1. <h2>{topic}</h2> - Вводный абзац (что это, назначение).
        2. <h3>Технические характеристики</h3> - Абзац с фактурой (марки, ГОСТы, свойства).
        3. <ul><li>...</li></ul> - Список ключевых преимуществ или параметров.
        4. <h3>Особенности применения</h3> - Где используется, для каких задач.
        5. <p> - Заключительный абзац про условия поставки (без телефонов, просто "наша компания предлагает...").
        
        Выдай ТОЛЬКО чистый HTML код статьи.
        """
