...
with tab_lsi_gen:
    ...
    def generate_lsi_article(api_key, context, exact_h2, lsi_keywords):
        ...
        # 1. Блок стоп-слов
        stop_words_list = (
            "разные, применяют, применение, надежный, используют, использование, производство, "
            "производят, изготовление, изготовляют, материал, качеств, помощи, сплав, вещество, "
        ...
        # === СИСТЕМНЫЙ ПРОМТ ===
        system_instruction = (
            "Ты — профессиональный технический копирайтер B2B. "
            "Ты строго следуешь ТЗ, структуре и ограничениям по лексике. "
            "Ты выдаешь ТОЛЬКО HTML-код."
        )
        
        # === ПОЛЬЗОВАТЕЛЬСКИЙ ПРОМТ (ТВОЙ ПОЛНЫЙ ВАРИАНТ) ===
        user_prompt = f"""
        ...
        1.1. Тема: Выведи строго: <h2>{exact_h2}</h2> (Используй этот текст дословно, без изменений).
        
        1.2. Коммерческий бэнгер/хук: Яркое, цепляющее первое предложение.
        
        1.3. Информационный абзац 1: 2-3 информационные предложения, раскрывающие суть продукта, его определение и ключевое назначение.
        ВАЖНО: Абзац 1 ОБЯЗАТЕЛЬНО должен содержать точную фразу с кодом:
        {contact_html_block}
        ...
        1.5. Список №1 (6 пунктов): Раскрывает ключевые характеристики, разновидности или преимущества. 
           ...
           - Полноценные предложения — точкой. Последний пункт всегда с точкой.
           
        1.6. Информационный абзац 2: 2-3 информационные предложения, описывающие технологию производства, сферы применения или соответствие стандартам (ГОСТ).
        
        1.7. Подзаголовок (Придумай релевантный H3, например "Технические особенности").
        
        1.8. Информационный абзац 3: 2-3 информационные предложения.
        ...
        1.11. Информационный абзац 4: 2-3 информационные предложения с коммерческим призывом к действию.

        [II] ОГРАНИЧЕНИЯ И LSI:
        1. Объем текста должен составлять до 4000 символов.
        2. LSI ИНСТРУКЦИЯ: Равномерно распредели слова: {{{lsi_string}}}. Выдели внедренные слова жирным (<b>слово</b>).
        3. СТОП-СЛОВА: Частота использования следующих слов должна быть менее 1%:
        ({stop_words_list})

        [III] ОФОРМЛЕНИЕ И СТИЛЬ:
        - Стиль: Деловой, экспертный, но "человечный".
        - Факты: Все суждения подкрепляй измеримыми фактами, цифрами, ссылками на ГОСТы.
        - Запрещено: Вопросительные и восклицательные знаки, вводные слова, оценочные прилагательные в превосходной степени.
        - Переменные: Оставь переменные #PHONE# и #EMAIL# как есть в коде.

        Этап 2: Комплексный аудит. (Выполняется мысленно перед выдачей). Выдай сразу финальный, вычитанный HTML код.
        """
