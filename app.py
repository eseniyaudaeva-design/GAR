...
# ==========================================
# TAB 5: BULK LSI GENERATOR (PRO B2B - HARDCORE TECH)
# ==========================================
import requests
...
with tab_lsi_gen:
    st.header("🏭 Массовая генерация B2B (Hardcore Tech + SEO Headers)")
    st.markdown("Генерация пачками. **Только технические блоки**. Ключ в каждом заголовке.")
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        # Стоп-слова: убираем любой намек на "воду" и "сервисность" в тексте
        stop_words_list = (
            ...
            "мы предлагаем, наш сервис, экономия времени, забота о клиенте, "
            "доставка, логистика, оплата" 
        )
        ...
        system_instruction = (
            "Ты — главный технолог завода. Ты ненавидишь маркетинг. "
            "Ты пишешь только о железе, свойствах и ГОСТах. "
            "Твоя типографика: используй ТОЛЬКО среднее тире (–). "
            "Ты выдаешь ТОЛЬКО HTML-код."
        )
        
        user_prompt = f"""
        ЗАДАЧА: Напиши строго технический текст для: "{exact_h2}".
        
        [I] ГЛАВНЫЕ ЗАПРЕТЫ:
        1. ЗАПРЕЩЕНО создавать отдельные блоки/абзацы про "Сервис", "Доставку", "Оплату".
        2. ЗАПРЕЩЕНО писать "Мы предлагаем", "Наша компания". 
        3. Весь текст должен быть про САМ ТОВАР (свойства, производство, применение).
        4. Коммерческие условия (резка, упаковка) указывай ТОЛЬКО сухим списком в самом конце, как технические параметры поставки.
        ...
        1.2. БЭНГЕР (Хук):
        2-3 ударных предложения про надежность/выгоду ТОВАРА. Без "Предлагаем".
        Пример: "Фундамент, который переживет здание. Арматура с запасом прочности 30%."
        
        1.3. Абзац 1 (Технический): 
        Назначение и суть. ОБЯЗАТЕЛЬНО вставь:
        {contact_html_block}
        
        1.4. Подводка к списку 1 (заканчивается двоеточием :):
        
        1.5. Список №1 (6 пунктов): ФИЗИКО-МЕХАНИЧЕСКИЕ ХАРАКТЕРИСТИКИ.
        - Конкретные цифры, диапазоны, марки стали, классы прочности.
        - Формат: "Параметр: Значение".
           
        1.6. Абзац 2 (Технический): Особенности производства или соответствие ГОСТ.

        1.7. Подзаголовок H3: 
        ОБЯЗАТЕЛЬНО включи фразу "{exact_h2}" (или корень) в заголовок.
        Пример: "Производство {exact_h2}" или "Сортамент {exact_h2}".
        
        1.8. Абзац 3 (Технический).
        
        1.9. Подводка к списку 2 (заканчивается двоеточием :):
        
        1.10. Список №2 (6 пунктов): ОБЛАСТИ ПРИМЕНЕНИЯ.
        - Конкретные узлы, конструкции, отрасли.
           
        1.11. Абзац 4 (Технический).
                              
        1.12. Подзаголовок H3: 
        ОБЯЗАТЕЛЬНО включи фразу "{exact_h2}" в заголовок.
        Пример: "Технические требования к {exact_h2}" или "Классификация {exact_h2}".
        
        1.13. Абзац 5 (Технический): Про контроль качества, допуски или методы испытаний.
        
        1.14. Подводка к списку 3 (заканчивается двоеточием :):
        
        1.15. Список №3 (6 пунктов): ФОРМА ВЫПУСКА И ПАРАМЕТРЫ ПОСТАВКИ.
        (Здесь "разбавляем" техничку коммерцией, но сухо).
        Пример:
        <ul>
            <li>Форма выпуска: в бухтах или прутках (до 12 м);</li>
            <li>Состояние поставки: отожженное / нагартованное;</li>
            <li>Упаковка: увязка в связки стальной лентой;</li>
            <li>Документация: сертификат качества на плавку;</li>
            <li>Резка: в размер (газ/лентопил);</li>
            <li>Временная защита: промасливание (по требованию).</li>
        </ul>
           
        1.16. Финальный призыв (1 предложение, сухо).

        [III] ОФОРМЛЕНИЕ:
        - LSI слова: {{{lsi_string}}}. Выдели <b>жирным</b>.
        - Типографика: Только среднее тире (–).
        - ТОЛЬКО HTML.
        """
    ...
    with st.expander("⚙️ Настройки и LSI", expanded=True):
        ...
        with c1:
            lsi_api_key = st.text_input("Gemini API Key", value=cached_key, type="password", key="bulk_api_key_v6")
        with c2:
