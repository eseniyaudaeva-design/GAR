...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 4. УМНЫЙ ПУЛ СТИЛЕЙ (Без карикатур, с микро-отклонениями)
    styles_pool =[
        "УЛЬТРА-КОРОТКО. 3-6 слов. Просто констатация (например: 'Норм база, отгрузили быстро').",
        "БЕЗ ФАНАТИЗМА. Обычный текст, но сделай ровно ОДНУ опечатку по соседней клавише ИЛИ пропусти ровно ОДИН пробел после запятой. Весь остальной текст - идеальный.",
        "ПРОСТОЙ МУЖИК. Напиши грамотно, но в одном месте поставь многоточие вместо точки. Без сложных слов.",
        "ОФИСНЫЙ. Упомяни про документы (УПД, закрывашки), но БЕЗ НОМЕРОВ. Никаких цифр гостов.",
        "СВОИМИ СЛОВАМИ. Опиши микро-ситуацию (как грузили в ГАЗель или резали в размер). Разговорный язык, без заумных фраз.",
        "СЛЕГКА НЕБРЕЖНЫЙ. Пишет нормально, но начинает первое предложение с маленькой буквы."
    ]
    ...
    for i in range(target_count):
        ...
        # РЕАЛИСТИЧНЫЕ СЮЖЕТЫ ДЛЯ СНИЖЕНИЯ ОЦЕНКИ
        if ratings[i] == 3.5:
            plot = "СЮЖЕТ 3.5 ЗВЕЗД: Тебя взбесила ИНФРАСТРУКТУРА базы. Например: жуткие ямы на въезде, сломался терминал оплаты картой, зависла база при выписке пропуска ИЛИ простоял в очереди на весовой лишний час. Сам металл отличный, менеджеры молодцы, но за организацию территории/процесса — минус."
        elif ratings[i] in [4.0, 4.5]:
            plot = "СЮЖЕТ 4 ЗВЕЗДЫ: Всё ок, но маленькая бытовая придирка (на складе грязно, долго искали нужный остаток, охранник нахамил). В целом всё хорошо."
        else:
            plot = "СЮЖЕТ 5 ЗВЕЗД: Обычный успешный заказ. Расскажи про быструю погрузку, ровную резку или наличие нужного металла."

        assigned_style = styles_pool[i % len(styles_pool)]
        authors_listing.append(f"Отзыв #{i+1} | Имя: {name} | Пол: {gender_instruction} | Оценка: {ratings[i]}\nЗАДАЧА: {plot}\nСТИЛЬ: {assigned_style}\n")
...
- Если в стиле указано "сделай опечатку" или "без заглавных" — примени это к ОДНОМУ ИЛИ ДВУМ СЛОВАМ! Не превращай весь текст в нечитаемый мусор. 
- 95% текста любого отзыва должно быть написано обычным, простым русским языком. Люди комбинируют нормальную речь с мелкими огрехами.

4. ОГРАНИЧЕНИЯ:
- Если в LSI есть город — проигнорируй его.
- Смайлы: 👍 или 👌 (максимум в 1 отзыве из всего массива).
