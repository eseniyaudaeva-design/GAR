...
### 💎 SEO-ЗАДАНИЕ (ГЛАВНЫЙ ПРИОРИТЕТ: ЕСТЕСТВЕННОСТЬ)
Ниже список ключевых фраз В НАЧАЛЬНОЙ ФОРМЕ (Леммы).
Твоя задача — внедрить их в текст, МАКСИМАЛЬНО ИЗМЕНЯЯ их форму под контекст предложения.
...
### 🧱 СЛОВАРНЫЙ ЗАПАС (SEO)
Вот список отдельных слов (лемм) в начальной форме:
[{seo_list_str}]

ТВОЯ ЗАДАЧА — ИСПОЛЬЗОВАТЬ ЭТИ СЛОВА В ТЕКСТЕ, НО СТРОГО ПО ПРАВИЛАМ РУССКОГО ЯЗЫКА.

❌ КАК ДЕЛАТЬ НЕЛЬЗЯ (ЗА ЭТО БАН):
1. Не вставляй слово в начальной форме, если предложение требует падежа!
   - Если слово "труба", а ты пишешь "много...", НЕЛЬЗЯ писать "много труба". НАДО: "много <b>труб</b>".
   - Если слово "стальной", НЕЛЬЗЯ писать "стальной кругом". НАДО: "<b>стальным</b> кругом".
   
2. Не повторяй одно и то же слово 10 раз!
   - Одно слово из списка — ОДНО употребление на весь текст.

3. Не пихай слова туда, где они не нужны.
   - Текст должен быть связным рассказом, а не набором ключевиков.

✅ КАК НАДО:
- Склоняй, спрягай, меняй числа, меняй падежи. Делай с этими словами всё, чтобы предложение звучало идеально гладко для носителя языка.
- Выделяй жирным <b> только то слово, которое ты взял из списка (уже в измененной форме).
"""

    # 2. СИСТЕМНЫЙ ПРОМТ
    system_instruction = (
        "Ты — профессиональный технический копирайтер и жесткий SEO-специалист. "
        "Твоя задача: написать МАКСИМАЛЬНО ОБЪЕМНЫЙ экспертный текст для карточки товара на основе предоставленной фактуры. "
        "Ты выдаешь ТОЛЬКО HTML-код. "
        "Стиль: Деловой, без воды, факты, цифры, ГОСТы, текст должен быть длинным. "
        "ЗАПРЕТЫ: Никакой Украины, политики. Не используй Markdown."
    )

    # ИЗМЕНЕНИЕ: Теперь темы блоков выбираются динамически на основе анализа текста
    user_prompt = f"""
    ИСХОДНЫЕ ДАННЫЕ:
    ...
    2. Длинный текст с фактами (минимум 1000 знаков на блок, если есть фактура).
    3. Маркированный список (<ul>).
    4. Вывод.
    
    ТЕМЫ БЛОКОВ (ЛОГИКА РАСПРЕДЕЛЕНИЯ):
    Блок 1: <h2>{forced_header}</h2> - Общее описание, назначение и главные особенности.
    Блоки 2-{num_blocks}: АНАЛИЗИРУЙ ФАКТУРУ. Выдели {num_blocks-1} ключевых тем, которые логичны ИМЕННО ДЛЯ ЭТОГО ТОВАРА.
    
    Примеры логики (ты выбираешь сам):
    - Если это металл: Марки стали, Сплавы, Обработка, ГОСТы.
    - Если это оборудование: Принцип работы, Технические характеристики, Монтаж, Эксплуатация.
    - Если это стройматериал: Состав, Прочность, Укладка, Преимущества.
    
    ВАЖНО: Адаптируй заголовки <h3> под суть товара. Не пиши про "сплавы", если товар пластиковый.
    
    ФОРМАТ ВЫВОДА:
    Только чистый HTML.
    Разделитель: |||BLOCK_SEP|||
    Ключевые слова выделяй <b>.
    """
...
# --- 2. ТАБЛИЦА (ПОЛНОСТЬЮ АВТОНОМНЫЙ ГЕНЕРАТОР) ---
                if use_tables and client:
                    for t_topic in table_prompts:
                        # Ключевые слова от скрипта
                        tech_data = f"{str_tables_final}" 
                        
                        # Если тема не задана, ставим задачу на полезность
                        topic_instruction = f"Тема таблицы: {t_topic}"
                        if not t_topic or t_topic == "Характеристики":
                            topic_instruction = "Тема: Самая полезная сводная информация для этого конкретного продукта."

                        # === ПРОМТ БЕЗ ШАБЛОНОВ И КОСТЫЛЕЙ ===
                        prompt_tbl = (
                            f"Ты — эксперт по продукту. Твоя задача — сгенерировать HTML-таблицу для карточки товара: '{header_for_ai}'.\n"
                            f"Контекст (категория): '{page['name']}'.\n"
                            f"{topic_instruction}.\n\n"
                            
                            f"ВХОДНЫЕ ДАННЫЕ (SEO-СЛОВА):\n"
                            f"Обязательно используй эти термины/стандарты/размеры/марки: [{tech_data}].\n\n"
                            
                            f"ТВОЯ ЗАДАЧА (ДУМАЙ САМ):\n"
                            f"1. Проанализируй, что это за товар. Что важно знать покупателю?\n"
                            f"2. САМ придумай структуру колонок. Не используй шаблоны. Таблица для болта, для станка и для услуги должны выглядеть ПО-РАЗНОМУ.\n"
                            f"3. Если это товар с вариациями -> сделай таблицу типоразмеров.\n"
                            f"4. Если это сложное устройство -> сделай таблицу технических характеристик (ТТХ).\n"
                            f"5. Если это материал -> сделай таблицу физико-химических свойств.\n\n"
                            
                            f"ГЛАВНЫЕ ПРИНЦИПЫ (ЧТОБЫ НЕ БЫЛО СПАМА):\n"
                            f"🔥 ПОЛЕЗНОСТЬ: Таблица должна давать инсайты, цифры, факты. Никакой 'воды'.\n"
                            f"⛔ АНТИ-СПАМ: ЗАПРЕЩЕНО писать название товара ('{header_for_ai}') внутри ячеек таблицы! Это мусор. Название уже есть в заголовке H1.\n"
                            f"⛔ УНИКАЛЬНОСТЬ: Не делай колонки, где во всех строках одно и то же значение. Такие данные выноси в заголовок таблицы или в описание.\n"
                            
                            f"ФОРМАТ ВЫВОДА:\n"
                            f"ТОЛЬКО HTML код: <table class='brand-accent-table'>...</table>"
                        )

                        try:
                            # Температура 0.6 — даем ей больше свободы, чтобы она "придумала" структуру сама
                            resp = client.chat.completions.create(model="google/gemini-2.5-pro", messages=[{"role": "user", "content": prompt_tbl}], temperature=0.6)
...
# --- 5. ГЕО (ИСПРАВЛЕННЫЙ: ЧИСТО ДОСТАВКА, БЕЗ ЛИШНЕГО SEO) ---
                if use_geo and client:
                    ...
                    try:
                         ...
                         # ПРОМТ ТЕПЕРЬ ЧИСТЫЙ. Никаких "стальных труб" и "купить". Только логистика.
                         prompt_geo = (
                             f"Напиши один короткий, естественный HTML параграф (<p>) о доставке товара '{header_for_ai}'.\n"
                             f"ЗАДАЧА: Сообщить клиенту, что мы доставляем товар в следующие города: {cities}.\n"
                             f"ТРЕБОВАНИЯ:\n"
                             f"1. Текст должен быть связным (не просто список).\n"
                             f"2. Не спамь названиями городов. Можно написать 'в Екатеринбург, Москву и другие регионы...'.\n"
                             f"3. НЕ ИСПОЛЬЗУЙ никаких других ключевых слов (купить, цена, гост). Пиши ТОЛЬКО про логистику.\n"
                             f"4. Выдели названия городов тегом <b>."
