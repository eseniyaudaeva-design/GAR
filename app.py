...
# ЧЕРНЫЙ СПИСОК МУСОРНЫХ СЛОВ (Убрали лимит, но мусор все равно не нужен)
...
def generate_five_blocks(client, base_text, tag_name, seo_words=None):
    ...
    if seo_words and len(seo_words) > 0:
        ...
        [SEO ТЗ: КЛЮЧЕВЫЕ СЛОВА]
        Ниже список слов, которые нужно употребить в тексте (постарайся внедрить как можно больше, но без спама): 
        {keywords_str}
        
        !!! ВАЖНЫЕ ПРАВИЛА ВНЕДРЕНИЯ !!!:
        1. ГРАММАТИКА: Слова в списке даны в начальной форме (или как теги). ТЫ ОБЯЗАН ИХ СКЛОНЯТЬ! 
           - Не пиши "доставка россия", пиши "доставка по России". 
           - Не пиши "купить труба", пиши "купить трубу".
           - Текст должен быть идеальным с точки зрения русского языка.
        2. ВЫДЕЛЕНИЕ: Каждое внедренное слово (в любой форме) выделяй тегом <b>.
           - Пример: "Мы осуществляем <b>доставку</b> по всей <b>России</b>".
        3. ЧАСТОТА: Достаточно 1 употребления каждого слова.
        4. ОБЪЕМ: Если слов много — пиши длинный, развернутый текст, чтобы они смотрелись естественно.
        """
    ...
    ПОМНИ: Текст должен быть связным, грамотным и органичным. Выделяй ключи <b>жирным</b>.
