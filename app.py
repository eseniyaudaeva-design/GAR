...
with tab_lsi_gen:
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
            ...
            system_instruction = (
                "Ты — технический редактор. Ты пишешь рублено, фактами. "
                "Ты избегаешь союзов, заменяя их запятыми. "
                "Ты следишь за регистром букв: внутри предложений название товара всегда с маленькой буквы."
            )
            
            user_prompt = f"""
            ЗАДАЧА: Напиши сухую техническую статью для товара: "{exact_h2}".
            
            [I] ЖЕСТКИЕ ПРАВИЛА СТИЛЯ:
            
            1. МИНИМУМ СОЮЗА "И":
               - При перечислении свойств ЗАМЕНЯЙ "и" на запятую.
               - ПЛОХО: "Прочность и стойкость".
               - ХОРОШО: "Прочность, стойкость".
               
            2. РЕГИСТР В ЗАГОЛОВКАХ (H3):
               - Заголовок начинается с Большой буквы. Все остальные слова — с маленькой (кроме аббревиатур ГОСТ, AISI).
               - Название товара внутри заголовка пиши со строчной буквы!
               - ПЛОХО: "Монтаж Трубы Стальной".
               - ХОРОШО: "Монтаж трубы стальной".
               
            3. КЛЮЧЕВОЕ СЛОВО ("{exact_h2}"):
               - ЦЕЛЬ: Точно 1.5% плотности от всего текста.
               - ВАЖНО: Используй ИМЕННО слова из фразы "{exact_h2}" (и их падежные формы).
               - НЕ СЧИТАЙ за вхождение синонимы (товар, изделие, прокат). Нужна именно эта фраза.
               - Распределяй ключи равномерно по тексту.
    
            [II] ТРЕБОВАНИЯ К ОФОРМЛЕНИЮ:
    
            1. СПИСКИ (ТЕХНИЧЕСКИЕ):
               - Цифры: пробел между числом и ед. измерения ("4 мм").
               - Тире: используй среднее тире (–) между параметром и значением.
               - Пунктуация: в конце пункта точка с запятой (;), в конце списка точка.
    
            2. ФОРМАТ ЧИСЕЛ:
               - ТОЛЬКО ЗНАЧАЩИЕ ЦИФРЫ (запрещено "3,0").
               - Дробные через запятую.
               - Диапазоны температур/давлений: "Рабочая температура -40...+400 °C".
               - Максимальные значения: ПИСАТЬ ОДНО ЧИСЛО ("Макс. нагрузка 5 тонн"). Не пиши "от и до" для максимума.
    
            [III] СТРУКТУРА ТЕКСТА:
            
            1.1. Заголовок: <h2>{exact_h2}</h2>.
            
            1.2. БЭНГЕР: 3 предложения. Что это (по ГОСТ), материал, назначение. Сухо.
            ...
            (Размеры, сечения, диаметры).
               
            1.6. Абзац 2. Описание производства (метод изготовления, состояние поставки).
            
            1.7. ТАБЛИЦА ХАРАКТЕРИСТИК (СПРАВОЧНАЯ):
            4-5 строк. Данные НЕ должны дублировать список №1.
            Включай: ГОСТ/ТУ, Марку сплава, Плотность (г/см³), Твердость (HB/HRC), Упаковку.
            КОД ТАБЛИЦЫ:
            ...
            1.8. Подзаголовок H3 (СТРОГИЙ ШАБЛОН): 
            "Классификация [название товара в родит. падеже с маленькой буквы]"
            (Пример: Классификация ленты нихромовой).
            ...
            1.13. Подзаголовок H3 (СТРОГИЙ ШАБЛОН):
            "Монтаж [название товара в родит. падеже с маленькой буквы]" ИЛИ "Обработка [товара...]".
            ...
            (Коррозия, термостойкость, ресурс). ИЗБЕГАЙ СОЮЗА "И".
               
            1.17. Абзац 6. Итоговое резюме (без слов "В заключение"). Условия отгрузки.
    
            [IV] ДОПОЛНИТЕЛЬНО:
            1. LSI: {{{lsi_string}}} (вписывай органично).
