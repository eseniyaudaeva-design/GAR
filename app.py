...
with tab_seo_main:
    ...
    with col_main:
        ...
        with top_col2:
            ...
            # Первый ряд: Тип страницы и Запрос
            c1, c2 = st.columns([1, 1])
            with c1:
                my_input_type = st.selectbox("📄 Источник страницы", ["Релевантная страница на вашем сайте", "Исходный код страницы или текст", "Без страницы"], key="my_page_source_radio")
            with c2:
                st.text_input("📝 Основной запрос", placeholder="Например: купить трубу стальную", key="query_input")

            # Второй ряд: URL или Текст (появится только если нужно)
            if my_input_type == "Релевантная страница на вашем сайте":
                st.text_input("🔗 Ссылка на страницу", placeholder="https://site.ru/...", key="my_url_input")
            elif my_input_type == "Исходный код страницы или текст":
                st.text_area("💻 Вставьте код/текст", height=150, placeholder="<html>...</html>", key="my_content_input")
