...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    try:
        ...
        prompt = f"""Ты — мастер имитации живых отзывов.
Тема: "{h2_header}". 
Твоя задача: написать {target_count} отзывов, которые абсолютно НЕ выглядят как работа ИИ. 

ГЛАВНАЯ ЗАДАЧА — УНИКАЛЬНОСТЬ: 
Все отзывы пишут разные живые люди. Категорически запрещено использовать одинаковый стиль написания, похожие структуры предложений или однотипные ошибки. Один пишет сухо и по делу, другой многословно, третий обрывисто. Никаких повторяющихся паттернов!

СПИСОК АВТОРОВ:
{nl.join(authors_listing)}

КРИТИЧЕСКИЕ ПРАВИЛА ХАОСА И ОГРАНИЧЕНИЙ:
1. РАЗНООБРАЗИЕ ГРАМОТНОСТИ И ОШИБОК: 
   - Распредели авторов: кто-то пишет лениво (всё с маленькой буквы, без точек), кто-то спешит (разные и неповторяющиеся опечатки, пропуск пробелов), кто-то пишет обычно.
   - Ошибки должны быть хаотичными и не повторяться из отзыва в отзыв. 
   - НИКАКОЙ красивой литературы и сложных ИИ-оборотов.
2. ПОЛ АВТОРА: Строго соблюдай род глаголов (женщина — "заказывала, довольна", мужчина — "заказывал, доволен"). НЕ ОШИБАЙСЯ!
3. ЗАПРЕТ НА АНГЛИЙСКИЙ: Строжайше запрещено использовать английский язык. Отзывы должны быть написаны исключительно кириллицей.
4. ЗАПРЕТ НА ГОРОДА/ГЕО: В отзывах категорически не должны упоминаться города. Если в переданных LSI-словах случайно попался город (например, Москва, СПб и т.д.) — проигнорируй его и не используй.
5. СМАЙЛЫ: Из допустимых смайликов МОЖНО использовать ТОЛЬКО палец вверх (👍) и ОК (👌). 
   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНЫ любые другие смайлы, особенно зеленые галочки (✅). 
   - Ставь их хаотично: кто-то ставит два подряд, кто-то ни одного.
6. ТИРЕ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО длинное тире "—". Используй только короткий минус "-".
7. LSI И СКЛОНЕНИЯ: Список: {", ".join(lsi_words[:20])}.
   - ВАЖНО: Склоняй их как хочешь (падежи, числа), чтобы в тексте они звучали максимально ЕСТЕСТВЕННО. Выделяй их **жирным**.

ЛОГИКА НИЗКИХ ОЦЕНОК:
Если оценка ниже 5.0 — опиши ВНЕШНИЙ косяк (пробки, аншлаг на складе, завод тупил), но менеджеры при этом молодцы и спасли ситуацию.
...
"""
        resp = client.chat.completions.create(
            ...
            temperature=1.15 # Немного повысил температуру для еще большей вариативности стилей
        )
