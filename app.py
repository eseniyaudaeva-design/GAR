...
if "arsenkin_token" in st.session_state:
    ...
    except (FileNtFoundError, KeyError): ARSENKIN_TOKEN = None
...
if "yandex_dict_key" in st.session_state:
    ...
    except (FileNtFoundError, KeyError): YANDEX_DICT_KEY = None
...
def get_yandex_dict_info(text, api_key):
    if not api_key: return {'lemma': text, 'pos': 'unkNwn'}
...
def check_positions_N_ALT(query, target_url, region_name, api_token):
    ...
    try:
        ...
        if not task_id: return 0, {"error": "N Task ID", "resp": resp}
...
def get_hybrid_word_type(word, main_marker_root, specs_dict=None):
    ...
    if morph:
        Nrm = morph.parse(w)[0].normal_form
...
def generate_ai_content_blocks(api_key, base_text, tag_name, forced_header, num_blocks=5, seo_words=None):
    ...
    if seo_words:
        lsi_cleaned = ", ".join([w.strip() for w in lsi_string.split(',') if w.strip().lower() not in['ооо', 'зао', 'пао', 'ип', 'ао', 'msk', 'спб']])
        
        seo_instruction_block = f"""
    --- ЖЕСТКОЕ ПРАВИЛО РАБОТЫ С LSI-СЛОВАМИ ---
    Список слов: {{{lsi_cleaned}}}
    
    1. УБИВАЙ КОММЕРЧЕСКИЙ МУСОР: Если в списке есть слова "каталог", "наименование", "наш", "ассортимент", "преимущество" — КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО строить вокруг них рекламные фразы. 
    2. Если слово невозможно вписать в СУХОЙ ГОСТовский текст (описание физики, химии, размеров) — ПРОСТО ВЫКИНЬ ЭТО СЛОВО И ЗАБУДЬ О НЕМ. Тебя похвалят за то, что ты его выкинул.
    3. Обязательно склоняй слова под контекст! Никакого именительного падежа, если он там не нужен.
    4. Оберни каждое использованное слово в тег <b>.
    """

    system_instruction = (
        "ТЫ — СУРОВЫЙ СОВЕТСКИЙ ИНЖЕНЕР-ТЕХНОЛОГ. Твоя задача писать СУХОЙ ТЕХНИЧЕСКИЙ СПРАВОЧНИК. "
        "Твой текст читают инженеры и закупщики заводов. Им нужны только цифры, факты, ГОСТы, марки сплавов и технологии монтажа. "
        "Думай в контексте того, что ты пишешь: это должна быть осмысленная, логичная и последовательная техническая статья, а не бессвязный набор фактов.\n\n"
        "КАТЕГОРИЧЕСКИЙ СТОП-ЛИСТ (ЗА ЭТИ ФРАЗЫ ТЕКСТ БУДЕТ НЕМЕДЛЕННО ЗАБРАКОВАН):\n"
        "- 'Мы рады представить', 'Предлагаем вашему вниманию', 'В нашем каталоге'\n"
        "- 'Ключевой продукт нашего производства', 'Гарантирует надежность', 'Исключительная стойкость'\n"
        "- 'Идеальное решение', 'Важно отметить', 'Несомненно', 'Высококачественный'\n\n"
        "ЗАПРЕТ НА ЛАТИНИЦУ:\n"
    ...
    try:
        ...
        if not raw_content: return["Error: API вернул пустой ответ"] * num_blocks

        content = re.sub(r'^```[a-zA-Z]*\s*', '', raw_content.strip())
        content = re.sub(r'\s*```$', '', content.strip())
        
        blocks =[b.strip() for b in content.split("|||BLOCK_SEP|||") if b.strip()]
        if not blocks: blocks = [content]
            
        cleaned_blocks = [b for b in blocks if b]
        ...
        cleaned_blocks[0] = f"<h2>{h2_topic}</h2>\n{first_block}"

        return cleaned_blocks[:num_blocks]

    except Exception as e:
        safe_blocks =[f"<h2>{h2_topic}</h2>\n<p>Ошибка генерации: {e}</p>"]
        while len(safe_blocks) < num_blocks: safe_blocks.append("")
...
def generate_faq_gemini(api_key, h1, lsi_words, target_count=5):
    ...
    # Расширенный список мусорных и опасных корней
    forbidden_roots = [
        "украин", "ukrain", "ua", "всу", "зсу", "ато", "сво", "войн",
        "киев", "львов", "харьков", "одесс", "днепр", "мариуполь",
        "донец", "луганс", "днр", "лнр", "донбасс", "мелитополь",
    ...
    prompt_1 = f"""Ты технический эксперт в металлопрокате и B2B продажах.
...
РАБОТА С LSI:
    1. Список слов-кандидатов: {lsi_text}.
    2. Впиши подходящие слова максимально органично. Склоняй их!
    3. ВЫДЕЛИ ЖИРНЫМ ШРИФТОМ (**слово**) все использованные LSI-слова!
    4. УМНЫЙ ФИЛЬТР: если слово звучит как корпоративный штамп, не вписывается в живую речь 
       вопроса или ответа, является латиницей (кроме марок стали и стандартов) - ПРОПУСТИ его.
       Примеры для пропуска: "наименование", "ассортимент", "каталог позиций", "msk", любые 
       латинские слова не являющиеся маркой стали или стандартом.
    5. ВЕРНИ СТРОГО В JSON: [{{ "Тип": "Коммерческий" или "Информационный", "Вопрос": "...", "Ответ": "..." }}]
"""
    try:
        ...
        # === ЭТАП 2: СТРОГИЙ РЕДАКТОР ===
        prompt_2 = f"""Я сгенерировал черновик FAQ для страницы "{h1}". Вот он (JSON):
{draft_text}
...
2. ФАКТЧЕКИНГ: Убедись, что все ГОСТы написаны заглавными, а марки стали корректно. Проверь, чтобы диапазоны были без пробелов (10-20 мм).
3. Вычисти "воду" и фразы ИИ ("Важно отметить", "Конечно", "В заключение"). Ответы должны быть короткими и по делу.
4. Сохрани заданное количество вопросов ({target_count}). Если их меньше - допиши недостающие.
5. ОБЯЗАТЕЛЬНО СОХРАНИ выделение жирным шрифтом (**слово**) для LSI!
6. ВЕРНИ ТОЛЬКО ГОЛЫЙ JSON-МАССИВ строго в таком формате:
[{{"Тип": "Коммерческий" или "Информационный", "Вопрос": "...", "Ответ": "..."}}]"""
        ...
        # === ПОСТ-ПРОЦЕССИНГ (Тире и спецсимволы) ===
...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    if target_count <= 1:
        ...
        if target_count > 3:
            ...
            for idx in indices:
                if (sum(ratings) / target_count) <= 4.72: break
    ...
    focus_points = [
        "Тема: Склад и отгрузка. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'нормально погрузили', 'чистый металл', 'быстро оформили'. Опиши процесс своими словами: упомяни кран, мужиков на складе или заезд на базу.",
        "Тема: Общение по телефону. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'толково проконсультировал', 'помог рассчитать', 'ответил на вопросы'. Напиши живым языком, как менеджер помог прикинуть остатки.",
        "Тема: Покупка всего в одном месте. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'удобно', 'взял сопутствующую мелочевку'. Упомяни конкретную железку (электроды, заглушки, петли), которую забыл, но докупил на месте.",
        "Тема: Резка металла. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'порезали ровно', 'в размер', 'ровный рез'. Напиши про болгарку, станок или как пилили, чтобы влезло в Газель.",
        "Тема: Доставка. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'адекватная доставка', 'выгрузили без проблем', 'водитель набрал'. Опиши заезд манипулятора на узкий участок или звонок водителя с трассы.",
        "Тема: Оформление и оплата. АНТИПРИМЕРЫ (НЕ ИСПОЛЬЗОВАТЬ): 'актуальные цены', 'быстро выставили счет'. Напиши про перевод, терминал, бухгалтерию или чек."
    ]
    ...
    for i in range(target_count):
        ...
        if ratings[i] == 5.0:
            ...
            sentiment = f"Оценка: {ratings[i]} звезды. Суть: произошла небольшая заминка при покупке, но всё благополучно решилось. Детали и ситуацию придумай сам. АНТИПРИМЕРЫ (НЕ ПИСАТЬ): 'небольшая заминка на складе', 'из-за загруженности', 'пришлось немного подождать', 'в целом всё нормально', 'не подвели по итогу', 'переоформили без проблем'."
            
        else:
            # Сюда как раз попадают ваши 3.5 звезды
            sentiment = f"Оценка: {ratings[i]} звезды. Суть: произошла неурядица, которая НЕ ЗАВИСЕЛА от нашей базы, но наши менеджеры всё вырулили. Детали и ситуацию придумай сам. АНТИПРИМЕРЫ (НЕ ПИСАТЬ): 'подвела транспортная компания', 'груз завис', 'менеджер дергал ТК', 'осадок есть', 'дали скидку', 'путаница с остатками'."
        ...
        if i in yo_indices:
            yo_rule = "Используй букву 'ё' там где она должна быть (всё, ещё, пошёл, нёс, приёмка). Только естественные случаи."
        else:
        ...
        if lsi_words_for_review:
            lsi_rule = f"КАНДИДАТЫ ДЛЯ ВСТАВКИ: {', '.join(lsi_words_for_review)}. Вставляй ТОЛЬКО те слова, которые органично впишутся в живую речь работяги. Если слово звучит как корпоративный штамп (наименование, ассортимент) или просто не лезет в текст - ПРОСТО ВЫБРОСЬ ЕГО. То, что решишь использовать, оберни в <b>...</b>."
        else:
...
1. ИСПОЛЬЗОВАНИЕ АНТИПРИМЕРОВ И ФАНТАЗИИ (САМОЕ ВАЖНОЕ):
   - В заданиях к отзывам даны "АНТИПРИМЕРЫ". Это заезженные шаблоны, которые тебе КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать.
   - Твоя задача — придумать уникальную бытовую ситуацию с нуля и описать её своими словами, ни разу не использовав фразы из антипримеров. Не повторяй сюжеты!
2. ЗАПРЕТ НА КОМПАНИИ И ЛАТИНИЦУ: 
   - Не используй слова "компания", "фирма", "поставщик", "конкуренты". Пиши "на базе", "на складе", "у ребят".
   - Никаких английских слов (ok, asap, price, manager). Только кириллица (исключение - марки стали, например AISI 304).
3. РЕАЛИСТИЧНАЯ ДОСТАВКА И СРОКИ:
   - Металл не возят "в тот же день". Минимальный срок - 2 дня. ЗАПРЕЩЕНО писать: "доставили сегодня", "за пару часов".
4. СТОП-СЛОВА (МАРКЕРЫ ИИ):
   - Никогда не используй: безупречный, высококачественный, настоятельно рекомендую, порадовало, в целом, подводя итог, оптимальный, профессионализм, клиентоориентированность, превзошел ожидания.
5. ЖИВАЯ РЕЧЬ И КАНЦЕЛЯРИТ:
   - Мужики не говорят "со своей задачей справляется" или "отличный элемент для моих задач".
   - ЗАПРЕЩЕНЫ слова: "проволочек", "данное изделие", "в кратчайшие сроки".
6. РАБОТА С LSI (УМНЫЙ ФИЛЬТР):
   - Не пытайся впихнуть невпихуемое! Если LSI-слово звучит неестественно или ломает живую речь — смело ВЫБРАСЫВАЙ его.
   - Вставляй только то, что звучит на 100% естественно.
   - Использованные слова склоняй и оборачивай в <b>...</b>.
7. ИМЕНА И ОПЕЧАТКИ:
   - Иногда (в 20% случаев) называй водителей или менеджеров простыми русскими именами (Алексей, Саня, Елена).
   - Если в задании сказано сделать опечатку — сделай её. Но LSI-слова (в тегах <b>) коверкать ЗАПРЕЩЕНО.
 8.  ЖИВЫЕ ОПЕЧАТКИ И ОШИБКИ:
...
def generate_full_article_v2(api_key, h1_marker, h2_topic, lsi_list):
    ...
    1.5. Список N1 (6 пунктов): ТЕХНИЧЕСКИЕ ПАРАМЕТРЫ.
    (Формат: <ul>). "И" меняй на запятую.
    ОБЯЗАТЕЛЬНО: каждый пункт должен содержать конкретное значение (число, диапазон, марку, ГОСТ).
    ЗАПРЕЩЕНО: пункты без значений типа "Радиус закругления" без числа - не пиши такой пункт вовсе.
    Числовые диапазоны СТРОГО через тире БЕЗ слов "от" и "до": "4-12 м", "3-35 мм".
    КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: "от 4 до 12 метров", "от 3 до 35 мм".
    ...
    ПРАВИЛА LSI:
    1. Используй слова из списка максимально органично.
    2. Каждое слово - РОВНО 1 РАЗ (не повторяй).
    3. Выделяй каждое вставленное слово тегом <b>жирный</b>.
    4. Если слов много - УВЕЛИЧИВАЙ ОБЪЕМ ТЕКСТА. Пиши дополнительные предложения, чтобы слова вписывались плавно.
    5. КРИТИЧЕСКИ ВАЖНО: если слово невозможно вписать естественно - ПРОПУСТИ ЕГО. 
       Примеры слов которые надо пропустить: аббревиатуры на латинице (кроме AISI/DIN/ГОСТ), 
       названия компаний, слова типа "ООО", "ИП", географические названия в неуместном контексте,
       любые слова после которых получается канцелярский или абсурдный оборот.
       ЛУЧШЕ ПРОПУСТИТЬ, ЧЕМ НАПИСАТЬ КРИВО.
...
СТРОГАЯ СТРУКТУРА БЛОКА И HTML-РАЗМЕТКА:
1. Заголовок (в тегах <h3>...</h3>)
2. Абзац текста - введение (ОБЯЗАТЕЛЬНО оберни весь абзац в теги <p> и </p>)
3. Подводка к списку (ОБЯЗАТЕЛЬНО оберни в теги <p> и </p>)
4. Маркированный список (в тегах <ul><li>...</li></ul>)
5. Абзац текста - заключение (ОБЯЗАТЕЛЬНО оберни весь абзац в теги <p> и </p>)
ПОСЛЕ пункта 5 не пиши ничего лишнего!
    """
...
# ==========================================
    ...
    with col_sidebar:
        ...
        if "settings_Nindex" not in st.session_state: st.session_state.settings_Nindex = True
        ...
        if "settings_Nrm" not in st.session_state: st.session_state.settings_Nrm = True
               ...
               # === НОВЫЙ СБОР СЛОВ ДЛЯ ОТЗЫВОВ ===
                ...
                if raw_candidates:
                    for w in raw_candidates:
                        ...
                        if (len(w_clean) > 2 
                            ...
                            and w_clean not in STOP_NUNS
                            and w_clean not in kNwn_geo):
                            ...
                            if is_kNwn_product or (is_Nun and not is_name_or_geo and not is_orphan_modifier):
                ...
                with st.spinner(f"📦 Сборка отзывов для: {task.get('q', 'запроса')}..."):
                    for _ in range(st.session_state.get('reviews_per_query', 3)):
                        ...
                        if not used_lsi_word and lsi_Nuns:
                            top_n = min(len(lsi_Nuns), 10)
                            lsi_obj = lsi_Nuns[random.randint(0, top_n - 1)]
                    ...
                    if 'ws_reviews_export_data' in st.session_state and st.session_state.ws_reviews_export_data:
                        ...
                        if current_reviews:
                            ...
                            for rev in current_reviews:
                                r_name = rev.get('Имя', 'Аноним')
                                r_date = rev.get('Дата', '')
                                r_source = rev.get('Источник', random.choice(review_sources))
                                r_rating = float(rev.get('Оценка', 5.0))
                                r_text = str(rev.get('Отзыв', '')).replace('**', '<b>').replace('**', '</b>')
                                
                                full_stars = int(r_rating)
                                has_half = 1 if r_rating % 1 != 0 else 0
                                stars_html = '⭐' * full_stars + ('½' if has_half else '')
                                empty_stars = '☆' * (5 - full_stars - has_half)
                                
                                card_html = f"""
                                <div style="background:#ffffff; border:1px solid #cbd5e1; border-left:8px solid #3b82f6; 
                                            border-radius:12px; padding:20px; margin-bottom:16px; 
                                            box-shadow:0 4px 6px rgba(0,0,0,0.07); font-family:sans-serif;">
                                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                                        <div>
                                            <div style="font-weight:700; color:#0f172a; font-size:1.05rem;">{r_name}</div>
                                            <div style="color:#64748b; font-size:0.82rem; margin-top:3px;">📅 {r_date} &nbsp;|&nbsp; 📍 {r_source}</div>
                                        </div>
                                        <div style="text-align:right;">
                                            <div style="color:#eab308; font-size:1.2rem;">{stars_html}<span style="color:#d1d5db">{empty_stars}</span></div>
                                            <div style="font-size:0.85rem; font-weight:600; color:#1e293b;">{r_rating}</div>
                                        </div>
                                    </div>
                                    <div style="color:#334155; line-height:1.6; margin-top:14px; font-size:0.97rem;">{r_text}</div>
                                </div>
                                """
