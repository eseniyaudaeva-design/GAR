...
1. Ключ: "тонна"
   ❌ ПЛОХО: "Цена за <b>тонна</b>..." (Ошибка падежа)
   ✅ ХОРОШО: "Цена за <b>тонну</b>..." (Винительный падеж)
   
2. Ключ: "качество"
   ❌ ПЛОХО: "Гарантируем высокое <b>качества</b>..." (Несогласованность)
   ✅ ХОРОШО: "Гарантируем высокое <b>качество</b>..."

3. Ключ: "промышленность"
   ❌ ПЛОХО: "Во многих <b>промышленность</b>..."
   ✅ ХОРОШО: "Во многих отраслях <b>промышленности</b>..."
-------------------------------------------
"""
...
# === ЦИКЛ ПО ПАЧКЕ (v5.4: ИНТЕГРАЦИЯ С tech_specs.json) ===
        for i, page in enumerate(target_pages_batch):
            ...
            try:
                ...
                # =========================================================
                # ШАГ 2. ГЕНЕРАЦИЯ ТАБЛИЦ (С ПОДТЯЖКОЙ ИЗ СЛОВАРЯ)
                # =========================================================
                if use_tables and client:
                    previous_tables_context = ""
                    
                    # --- ЗАГРУЗКА И ПОИСК ПРАВИЛЬНЫХ МАРОК ---
                    # Мы делаем это тут, чтобы найти правильное написание для текущих ключей
                    found_correct_specs = []
                    try:
                        # 1. Загружаем словарь (если он есть)
                        specs_map = {}
                        if os.path.exists("data/tech_specs.json"):
                            with open("data/tech_specs.json", "r", encoding="utf-8") as f:
                                raw_specs = json.load(f)
                                # Собираем плоский список всех марок
                                flat_specs = []
                                if isinstance(raw_specs, dict):
                                    flat_specs = [x for v in raw_specs.values() for x in v]
                                else:
                                    flat_specs = raw_specs
                                # Создаем карту: "ст3" -> "Ст3"
                                for s in flat_specs:
                                    if isinstance(s, str):
                                        specs_map[s.lower().strip()] = s.strip()
                        
                        # 2. Ищем совпадения в ключах (str_tables_final) и названии
                        ...
                        for token in search_tokens:
                            ...
                            if token in specs_map:
                                # Добавляем ЭТАЛОННОЕ написание
                                if specs_map[token] not in found_correct_specs:
                                    found_correct_specs.append(specs_map[token])
                                    
                    except Exception as e:
                        # Если файла нет или ошибка json, не ломаем скрипт, просто список будет пуст
                        pass
                        
                    # Формируем строку для промта
                    specs_reference_str = ", ".join(found_correct_specs) if found_correct_specs else "Нет данных в справочнике, используй общие правила."
                    ...
                    for t_topic in table_prompts:
                        ...
                        if "Размер" in t_topic or "Сортамент" in t_topic:
                            topic_guide = "Акцент на геометрические размеры, вес, длину."
                        elif "Хим" in t_topic:
                            topic_guide = "Акцент на состав сплава и элементы."
                        else:
                            topic_guide = "Технические параметры и эксплуатационные свойства."

                        # === ПРОМТ v5.4 (С ЭТАЛОННЫМ СПРАВОЧНИКОМ) ===
                        prompt_tbl = f"""
    ТЫ — ГЛАВНЫЙ ТЕХНОЛОГ. Задача: HTML-таблица для "{header_for_ai}".
    ТЕМА ТАБЛИЦЫ: {t_topic} ({topic_guide})
    
    ИСХОДНЫЕ ДАННЫЕ:
    1. 🧠 КОНТЕКСТ СТАТЬИ:
    {context_snippet}
    
    2. 🔑 КЛЮЧЕВЫЕ СЛОВА (СЫРЫЕ ДАННЫЕ):
    [{tech_data}]
    
    3. 📚 СПРАВОЧНИК (ЭТАЛОННЫЕ НАПИСАНИЯ):
    [{specs_reference_str}]
    (Если ты видишь марку или ГОСТ из списка "КЛЮЧЕВЫЕ СЛОВА", найди её в "СПРАВОЧНИКЕ" и пиши ТОЧНО ТАК ЖЕ. Например: если ключ "09г2с", а в справочнике "09Г2С" -> пиши "09Г2С").
    
    4. 🚫 ИСТОРИЯ (НЕ ДУБЛИРОВАТЬ):
    {previous_tables_context}
    
    --- ИНСТРУКЦИЯ ПО ЛОГИКЕ ---
    1. ИНТЕГРАЦИЯ КЛЮЧЕЙ: Найди каждому ключу логичное место.
       - Если ключ — это марка стали -> колонка "Марка".
       - Если ключ не подходит (например, тип "ВГП" в таблице размеров) -> впиши в строку "Аналоги", "Применение" или "Вид".
    
    2. ЗАПРЕТЫ:
       - ❌ НИКАКИХ "Не путать с...", "Не является...".
       - ❌ Не пиши марки маленькими буквами, если это аббревиатура (Ст3, 09Г2С — правильно).
    
    Выдай только код <table> с классом 'brand-accent-table'.
    """
                        try:
                            # Температура 0.3
                            ...
                            if start_idx != -1 and end_idx != -1:
                                ...
                                previous_tables_context += f"\n[Таблица '{t_topic}' содержит]: {text_content_only[:600]}..." 
