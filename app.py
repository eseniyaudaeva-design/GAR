...
# --- ФУНКЦИЯ КЛАССИФИКАЦИИ v5.0 (Inverted Logic) ---
def classify_semantics_with_api(words_list, yandex_key):
    ...
    # 1. ГРЯЗНЫЕ СЛОВА (Точно мусор/интерфейс)
    SITE_UI_GARBAGE = {
    ...
    # 2. ОБЩИЕ/КОММЕРЧЕСКИЕ СУЩЕСТВИТЕЛЬНЫЕ (ВСЁ, ЧТО НЕ ТОВАР)
    # Если слово здесь - оно летит в Коммерцию. Если нет - в Товары.
    GENERAL_AND_COMMERCIAL = {
        # Торговля
        ...
        # Характеристики (но не сам товар)
        ...
        'преимущество', 'особенность', 'применение', 'назначение',
        
        # Общие слова (которые часто путают с товарами)
        'деталь', 'оборудование', 'инструмент', 'элемент', 'конструкция', 'система', 'материал', 'сырье', 
        'продукция', 'товар', 'изделие', 'наименование', 'позиция', 'тип', 'вид', 'класс', 'группа', 
        'категория', 'раздел', 'регион', 'город', 'область', 'район'
    }
    ...
    for word in words_list:
        ...
        # Глаголы на -ться/-тся (сразу в мусор)
        ...
        if w_lower in GENERAL_AND_COMMERCIAL or w_lower in SITE_UI_GARBAGE or any(r in w_lower for r in GEO_ROOTS):
            categories['commercial'].add(w_lower); continue
    ...
    for word in api_candidates:
        ...
        # 2. Существительные и Прилагательные -> ТОВАРЫ (по умолчанию)
        # Так как мы уже исключили слова типа "Цена", "Вес", "Регион" в списке GENERAL_AND_COMMERCIAL,
        # то все остальные существительные (Алюминий, Анод) считаем товарами.
        elif pos in ['noun', 'adjective', 'participle'] and len(lemma) > 2:
            categories['products'].add(lemma)
            
        # 3. Если не знаем что это (Unknown), но похоже на слово
