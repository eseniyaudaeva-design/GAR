...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # --- 5. ЖЕСТКОЕ РАСПРЕДЕЛЕНИЕ СТИЛЕЙ ---
    ...
    # --- 6. МЕХАНИКИ УНИКАЛЬНОСТИ ---
    error_mechanics = [
        "Сделай опечатку (промахнись по клавише).",
        "Напиши одно слово так, как оно слышится (ошибка в гласной).",
        "Забудь поставить пробел после одного предлога.",
        "Пропусти букву в середине длинного слова.",
        "Сделай грамматическую ошибку в окончании."
    ]
    
    slang_mechanics = [
        "Используй слова-паразиты.",
        "Замени одно слово на разговорное сокращение.",
        "Используй просторечное выражение эмоции."
    ]

    # ИЗМЕНЕНО: Сюжеты без рекламного уклона
    plot_types = [
        "Для дачи или гаража: забор, крыша, сарай, теплица. Просто взял для стройки.",
        "Бюджет: по деньгам вышло нормально, по карману не ударило. БЕЗ сравнения с конкурентами.",
        "Процесс покупки: приехал на базу, оплатил, забрал ИЛИ заказал доставку (ждал пару дней, привезли)."
    ]
    ...
    for i in range(target_count):
        ...
        if assigned_gender == 'Женский':
            gender_instruction = "ЖЕНЩИНА. ВНИМАНИЕ: СТРОГО используй глаголы женского рода (я заказала, я забрала, мне привезли, я решила)."
        else:
            gender_instruction = "МУЖЧИНА. Используй глаголы мужского рода (я заказал, я забрал, я решил)."

        base_plot = random.choice(plot_types)
        plot_instruction = f"Ситуация: {base_plot}"
        
        if ratings[i] == 5.0:
             plot_instruction += " Отзыв позитивный. Всё прошло штатно."
        elif ratings[i] == 3.5:
             plot_instruction += " Есть серьезный минус (долго ждал погрузку, перепутали документы, сложный заезд на базу)."
        else:
             plot_instruction += " В целом пойдет, но есть мелкая придирка (грязный металл, ждал доставку дольше обычного)."

        style_inst = ""
        current_grammar = grammar_pool[i]
        
        if current_grammar == 'heavy':
            mech = random.choice(error_mechanics)
            slang = random.choice(slang_mechanics)
            style_inst = f"Стиль: ОЧЕНЬ небрежный. {mech} Почти не используй запятые. {slang}"
        elif current_grammar == 'moderate':
            mech = random.choice(error_mechanics)
            style_inst = f"Стиль: С легкими ошибками. Пропусти пару запятых. {mech}"
        else:
            style_inst = "Стиль: Грамотный. Нормальная пунктуация, без явных опечаток."

        if i == ultra_short_index:
            length_inst = "Длина: 3-5 слов (ультра-коротко)."
        else:
            length_inst = random.choice(["Длина: 1-2 предложения.", "Длина: 2-3 предложения."])
            
        emoji_inst = "РАЗРЕШЕН 1 смайлик." if i == emoji_index else "СМАЙЛИКИ СТРОГО ЗАПРЕЩЕНЫ."

        authors_listing.append(
            f"--- БРИФ ДЛЯ ОТЗЫВА #{i+1} ---\n"
            f"Автор: {name} | Пол: {gender_instruction}\n"
            f"Оценка: {ratings[i]} звезд\n"
            f"Контекст: {plot_instruction}\n"
            f"{style_inst}\n"
            f"{length_inst} | {emoji_inst}\n"
        )
    ...
    try:
        ...
        # ИЗМЕНЕНО: Жесткие правила против маркетинга и неадекватной доставки
        prompt = f"""Ты — генератор реалистичных отзывов от разных людей (мужчин и женщин) для товара "{h2_header}". 
Твоя задача — написать уникальный текст для каждого брифа. Отзывы пишут обычные люди (стройка, дача, бытовой ремонт).

ПРАВИЛА (КРИТИЧЕСКИ ВАЖНО):
1. КОНТЕКСТ АВТОРА: Внимательно читай бриф! Если автор ЖЕНЩИНА, текст обязан быть от женского лица.
2. ПОЛНЫЙ ЗАПРЕТ УТП И МАРКЕТИНГА: ЗАПРЕЩЕНО использовать слова "поставщик", "компания", "на рынке", "без накруток", "идеальная альтернатива", а также упоминать названия магазинов. Не сравнивай цены с другими! Пиши бытово: "по деньгам норм", "вышло недорого".
3. РЕАЛИСТИЧНАЯ ДОСТАВКА: ЗАПРЕЩЕНО писать, что привезли "за час", "в тот же день" или "очень быстро". Металл так быстро не возят. Пиши: "привезли в назначенный день", "ждал пару дней", "доставка обычная".
4. ЖИВОЙ ЯЗЫК: Запрещены слова-шаблоны ботов: "качественный", "соответствует заявленному", "рекомендую к покупке", "недостатки отсутствуют", "данный товар".
5. РАЗНООБРАЗИЕ: Каждый отзыв должен звучать по-разному. Не дублируй одни и те же опечатки или слова-паразиты между отзывами.
6. LSI СЛОВА: {", ".join(final_lsi)}. Каждое слово используй СТРОГО 1 РАЗ на всю партию отзывов! Оборачивай в <b>...</b>.
...
"""
        ...
        # === ЖЕЛЕЗНЫЙ ПОСТ-ПРОЦЕССИНГ (Регистр и точки) ===
