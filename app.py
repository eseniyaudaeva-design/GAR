        ...
        if st.session_state.get('analysis_done') and st.session_state.get('analysis_results'):
            ...
            if 'relevance_top' in results and not results['relevance_top'].empty:
                ...
                with st.expander("📊 График релевантности (Нажмите, чтобы раскрыть)", expanded=False):
                    st.info("**Логика расширенного анализа**\nГрафик строится по максимальной выборке из парсинга для **калибровки линии тренда** и точного выявления аномалий. Это позволяет системе отличить качественные ресурсы от случайных "выбросов" в выдаче", icon="ℹ️")
