...
# ==========================================
st.set_page_config(layout="wide", page_title="GAR PRO + GEN", page_icon="📊")
...
STATIC_DATA_GEN = {
    ...
    'IP_PROP4820': """<p>Наши изделия успешно применяются на некоторых предприятиях Урала, центрального региона, Поволжья, Сибири.
Партнеры по логистике предложат доставить заказ самым удобным способом – автомобильным, железнодорожным, даже авиационным транспортом.
Для вас разработают транспортную схему под удобный способ получения. Погрузка выполняется полностью с соблюдением особенностей техники безопасности.</p>
<div class="h4">
<h4>Самовывоз</h4>
</div>
...
<p>Отправим прокат на ваш объект собственным автопарком.
Получение в упаковке для безопасной транспортировки, а именно на деревянном поддоне.</p>
<div class="h4">
<h4>Сотрудничаем с ТК</h4>
</div>
<p>Доставка с помощью транспортной компании по России и СНГ.
Окончательная цена может измениться, так как ссылается на прайс-лист, который предоставляет контрагент, однако, сравним стоимость логистических служб и выберем лучшую.</p>""",
...
<div class="an-col-12">
        <ul>
            <li style="font-weight: 400;">
                <p><span style="font-weight: 400;">С помощью менеджера в центрах продаж</span></p>
            </li>
        </ul>
        <p>Важно! Цена не является публичной офертой. Приходите в наш офис, чтобы уточнить поступление, получить ответы на почти любой вопрос, согласовать возврат, счет, рассчитать логистику.</p>
        <ul>
            <li style="font-weight: 400;">
                <p><span style="font-weight: 400;">На расчетный счет</span></p>
            </li>
        </ul>
        <p>По внутреннему счету в отделении банка или путем перечисления средств через личный кабинет (транзакции защищены, скорость зависит от отделения). Для права подтверждения нужно показать согласие на платежное поручение с отметкой банка.</p>
        <ul>
            <li style="font-weight: 400;">
                <p><span style="font-weight: 400;">Наличными или банковской картой при получении</span></p>
            </li>
        </ul>
        <p><span style="font-weight: 400;">Поможем с оплатой: объем имеет значение. Крупным покупателям – деньги можно перевести после приемки товара.</span></p>
        <p>Менеджеры предоставят необходимую информацию.</p>
        <p>Заказывайте через прайс-лист:</p>
        <p><a class="btn btn-blue" href="/catalog/">Каталог (магазин-меню):</a></p>
    </div>
</div>
 <br>""",
...
# ==========================================
# 4. ЛОГИКА (ГАР + ГЕНЕРАТОР)
# ==========================================

# --- ЛОГИКА ГАР (NLP и Парсинг) ---
try:
    import pymorphy2
...
def get_arsenkin_urls(query, engine_type, region_name, depth_val=10):
    ...
    while status == "process" and attempts < max_attempts:
        ...
        try:
            ...
            if res_check_data.get("status") == "finish":
                status = "done"
                break
            if str(res_check_data.get("code")) == "429": continue 
    ...
    try:
        ...
        final_url_list = []
        if isinstance(collect, list) and len(collect) > 0 and isinstance(collect[0], list) and len(collect[0]) > 0 and isinstance(collect[0][0], list):
...
def parse_page(url, settings):
    ...
    try:
        ...
        if settings['noindex']: tags_to_remove.extend(['noindex', 'nav', 'footer', 'header', 'aside'])
...
def calculate_metrics(comp_data_full, my_data, settings, my_serp_pos, original_results):
    ...
    if not comp_docs:
        ...
        for item in original_results:
            ...
            table_rel_fallback.append({"Домен": domain, "Позиция": item['pos'], "Ширина (балл)": 0, "Глубина (балл)": 0})
        my_label = f"{my_data['domain']} (Вы)" if (my_data and my_data.get('domain')) else "Ваш сайт"
        table_rel_fallback.append({"Домен": my_label, "Позиция": my_serp_pos if my_serp_pos > 0 else len(original_results) + 1, "Ширина (балл)": 0, "Глубина (балл)": 0})
        return {"depth": pd.DataFrame(), "hybrid": pd.DataFrame(), "relevance_top": pd.DataFrame(table_rel_fallback), "my_score": {"width": 0, "depth": 0}, "missing_semantics_high": [], "missing_semantics_low": []}
    ...
    for word, freq in doc_freqs.items():
        if word not in my_lemmas_set:
            ...
            if freq >= min_docs_threshold: missing_semantics_high.append(item)
            elif N <= 5 or freq >= 2: missing_semantics_low.append(item)
    ...
    for word in vocab:
        ...
        idf = max(0.1, idf) 
        spam_percent = 0
        if my_tf_total > rec_max and rec_max > 0: spam_percent = round(((my_tf_total - rec_max) / rec_max) * 100, 1)
        elif my_tf_total > 0 and rec_max == 0: spam_percent = 100 
    ...
    for item in original_results:
        ...
        competitor_stats_raw.append({"domain": domain, "pos": pos, "raw_w": raw_width, "raw_d": raw_depth})
    ...
    for c in competitor_stats_raw:
        ...
        table_rel.append({"Домен": c['domain'], "Позиция": c['pos'], "Ширина (балл)": score_w, "Глубина (балл)": score_d})
    ...
    my_label = f"{my_data['domain']} (Вы)" if (my_data and my_data.get('domain')) else "Ваш сайт"
    table_rel.append({"Домен": my_label, "Позиция": my_serp_pos if my_serp_pos > 0 else len(original_results) + 1, "Ширина (балл)": my_score_w, "Глубина (балл)": my_score_d})
...
with col_sidebar:
    st.markdown("## 🛠 Меню инструментов")
    app_mode = st.radio("Режим работы", ["SEO Анализатор (ГАР)", "Генератор текстов (Perplexity)"], key="app_mode_switcher")
    st.markdown("---")
...
if app_mode == "SEO Анализатор (ГАР)":
    ...
    def render_paginated_table(df, title_text, key_prefix, default_sort_col=None, use_abs_sort_default=False):
        ...
        with st.container():
            ...
            with col_s1:
                sort_col = st.selectbox("🗂 Сортировать весь список по:", df.columns, key=f"{key_prefix}_sort_box",
            ...
            with col_s2:
                sort_order = st.radio("Порядок:", ["Убывание", "Возрастание"], horizontal=True, key=f"{key_prefix}_order_box",
        ...
        def highlight_rows(row):
            ...
            for col_name in row.index:
                if col_name == 'is_missing' and row['is_missing']: styles.append(base_style + 'color: #D32F2F; font-weight: bold;')
                elif col_name != 'is_missing' and col_name != 'diff_abs': styles.append(base_style + 'font-weight: 600;')
        ...
        st.dataframe(styled_df, use_container_width=True, height=dynamic_height, column_config={c: None for c in cols_to_hide})
    ...
    with col_main:
        ...
        if my_input_type == "Релевантная страница на вашем сайте":
            my_url = st.text_input("URL страницы", placeholder="[https://site.ru/catalog/tovar](https://site.ru/catalog/tovar)", label_visibility="collapsed", key="my_url_input")
    ...
    if st.session_state.get('start_analysis_flag'):
        ...
        if source_type == "API":
            ...
            for result in found_results:
                ...
                if my_domain and my_domain == domain:
                    if my_serp_pos == 0 or pos < my_serp_pos: my_serp_pos = pos
                    continue 
                if any(x in domain for x in excl): continue 
                filtered_results_all.append(result)

            target_urls_raw = filtered_results_all[:TARGET_COMPETITORS]
            st.info(f"Получено уникальных URL: {len(found_results)}. Выбрано **{len(target_urls_raw)}** релевантных конкурентов. Ваш сайт в ТОПе: **{'Да (Поз. ' + str(my_serp_pos) + ')' if my_serp_pos > 0 else 'Нет'}**.")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for f in concurrent.futures.as_completed(futures):
                res = f.result()
                if res: comp_data_full.append(res)
                done += 1
        ...
        if not comp_data_full: st.warning("⚠️ Не удалось скачать контент со страниц конкурентов.")
            ...
            if high or low:
                ...
                with st.expander(f"🧩 Упущенная семантика ({count_total} слов) — Нажмите, чтобы развернуть", expanded=False):
                    if high:
                        ...
                        st.markdown(f"<div style='background-color:#F8FAFC; padding:15px; border-radius:10px; line-height: 1.6; border: 1px solid #E2E8F0; color: #333; font-size: 14px;'>{', '.join(words_list_h)}</div>", unsafe_allow_html=True)
                    if high and low: st.divider()
                    if low:
                        st.markdown("**Дополнительный список связанных слов:**")
                        words_list_l = [item['word'] for item in low]
                        st.markdown(f"<div style='background-color:#F8FAFC; padding:15px; border-radius:10px; line-height: 1.6; border: 1px solid #E2E8F0; color: #555; font-size: 13px;'>{', '.join(words_list_l)}</div>", unsafe_allow_html=True)
                    st.caption("Слова отсортированы по частоте встречаемости.")

            st.markdown(f"""<div class="legend-box"><span class="text-red">Красный</span>: слова, которых нет у вас. <span class="text-bold">Жирный</span>: слова, участвующие в анализе.</div>""", unsafe_allow_html=True)
...
# ==========================================
# ЛОГИКА: РЕЖИМ ГЕНЕРАТОРА ТЕКСТОВ
# ==========================================
elif app_mode == "Генератор текстов (Perplexity)":
    
    with col_main:
        st.title("🤖 Генерация HTML-блоков (Perplexity)")
        st.markdown("""
        Этот инструмент парсит указанную страницу, собирает ссылки из блока `.popular-tags-inner` (теги) 
        и генерирует 5 HTML-блоков текста для каждого тега, используя модель **Sonar Pro**.
        """)
        ...
        with st.container(border=True):
            ...
            st.markdown("#### 2. Целевая страница")
            target_url_gen = st.text_input("Ссылка для обработки (Родительская категория)", placeholder="[https://site.ru/catalog/parent-category/](https://site.ru/catalog/parent-category/)")
            
            start_gen = st.button("🚀 Начать генерацию", type="primary")

        if start_gen:
            if not pplx_api_key:
                st.error("Ошибка: Не введен API ключ Perplexity!")
                st.stop()
            ...
            st.write(f"⏳ Получение данных со страницы: {target_url_gen}")
            base_text_content, tags = gen_get_page_data(target_url_gen)

            if tags and base_text_content:
                st.success(f"Найдено тегов: **{len(tags)}**. Начинаем генерацию...")
                ...
                for i, tag in enumerate(tags, 1):
                    ...
                    row = {
                        ...
                        'IP_PROP4820': STATIC_DATA_GEN['IP_PROP4820'],
                        'IP_PROP4821': STATIC_DATA_GEN['IP_PROP4821'],
                        'IP_PROP4822': STATIC_DATA_GEN['IP_PROP4822'],
                        'IP_PROP4823': STATIC_DATA_GEN['IP_PROP4823'],
                        'IP_PROP4824': STATIC_DATA_GEN['IP_PROP4824'],
                        'IP_PROP4825': STATIC_DATA_GEN['IP_PROP4825'],
                        'IP_PROP4826': STATIC_DATA_GEN['IP_PROP4826'],
                        'IP_PROP4834': STATIC_DATA_GEN['IP_PROP4834'],
                        'IP_PROP4835': STATIC_DATA_GEN['IP_PROP4835'],
                        'IP_PROP4836': STATIC_DATA_GEN['IP_PROP4836'],
                        'IP_PROP4837': STATIC_DATA_GEN['IP_PROP4837'],
                    }
                    all_rows.append(row)
                ...
                # Конвертация в Excel
                output = io.BytesIO()
                # ИСПРАВЛЕНИЕ: Используем стандартный движок (обычно openpyxl) без явного указания xlsxwriter
