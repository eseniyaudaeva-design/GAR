...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 1. Математика оценок (одна 3.5, остальное добиваем до 4.71+)
    def get_balanced_ratings(n):
        if n <= 1: return [5.0] * n
        ratings = [3.5]
    ...
    # 4. БАНК УНИКАЛЬНЫХ СТИЛЕЙ (чтобы отзывы не были похожи вообще)
    styles_pool =[
        "УЛЬТРА-КОРОТКО. Лень писать. Всего 2-4 слова (например: 'Норм база', 'Пойдет').",
        "БАТЯ В WHATSAPP. Писать всё строчными буквами... ставить много многоточий... рубленые фразы.",
        "ТОРОПЫГА. Спешил, поэтому НЕТ ПРОБЕЛОВ после запятых (металл норм,порезали,забрал).",
        "СНАБЖЕНЕЦ. Сухо, строго, фокус на бухгалтерию (ЭДО, сертификаты ГОСТ, УПД).",
        "ОПЕЧАТКИ. Поток сознания одним предложением без точек, сделать пару опечаток (спсибо, хорошр).",
        "ПРОРАБ. Использовать сленг (хлысты, болгарка), фокус на геометрии металла и точной резке.",
        "ОБЫЧНЫЙ. Нейтральный рассказ про то, как грузили в прицеп или Газель на самовывозе.",
        "ДЕТАЛЬНЫЙ. Упомянуть про весовую (сошлось до килограмма) и ровные края при резке."
    ]
    ...
    for i in range(target_count):
        ...
        # Пол с жестким указанием глаголов для ИИ
        ...
        # Сюжет в зависимости от оценки
        if ratings[i] == 3.5:
            plot = "СЮЖЕТ: Внешний форс-мажор (опоздал нанятый водитель ИЛИ банк задержал деньги). База не виновата, менеджеры спасли ситуацию. Тон: раздражен на ситуацию, но благодарен базе."
        elif ratings[i] in [4.0, 4.5]:
            plot = "СЮЖЕТ: Всё отлично, но есть МЕЛКАЯ придирка (на складе пыльно/шумно, небольшая очередь на весовой). НЕ ругать товар."
        else:
            plot = "СЮЖЕТ: Успешный заказ. Опиши процесс (погрузка, или резка, или доки)."

        # Присваиваем уникальный стиль
        assigned_style = styles_pool[i % len(styles_pool)]

        authors_listing.append(f"Отзыв #{i+1} | Имя: {name} | Пол: {gender_instruction} | Оценка: {ratings[i]}\nЗАДАЧА ДЛЯ ЭТОГО ОТЗЫВА: {plot}\nСТИЛЬ: {assigned_style}\n")
    ...
    try:
        ...
        prompt = f"""Задача: Написать {target_count} АБСОЛЮТНО РАЗНЫХ отзывов о компании-поставщике (товар: "{h2_header}").

Вот список каждого отзыва с ЕГО ПЕРСОНАЛЬНЫМ СТИЛЕМ И СЮЖЕТОМ (ВЫПОЛНЯЙ СТРОГО):
...
2. СТОП-СЛОВА И ШАБЛОНЫ (ЗАПРЕЩЕНО ИСПОЛЬЗОВАТЬ!):
- ЗАПРЕЩЕНЫ слова: "пошли навстречу", "спасибо за понимание", "качество на высоте", "рекомендую", "оперативно", "оптимальный".
- ЗАПРЕЩЕНЫ фразы-представления: "Я как бухгалтер", "Закупаем постоянно", "Работаю снабженцем". Пиши сразу по делу!

3. ОГРАНИЧЕНИЯ:
- Если в LSI есть город — проигнорируй его.
- Смайлы: 👍 или 👌 (максимум в 1-2 отзывах, в остальных без смайлов).
...
"""
        ...
        # ЖЕСТКО ПРИШИВАЕМ ДАТЫ ЧЕРЕЗ PYTHON
