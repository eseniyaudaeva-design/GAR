...
def analyze_serp_anomalies(df_rel):
    """
    Анализирует таблицу релевантности (Улучшенная версия v2).
    Использует Медиану для защиты от перекосов и более жесткий порог.
    """
    if df_rel.empty:
    ...
    # Используем МЕДИАНУ, так как она устойчива к выбросам
    median_score = df['Total'].median()
    
    # Порог: 70% от медианы. Если медиана 90, то все что ниже 63 - аномалия.
    # Но не ниже 30 баллов (совсем дно).
    threshold = max(median_score * 0.70, 30) 
    ...
    # Собираем списки
    for _, row in df.iterrows():
        # Пытаемся найти полную ссылку, если нет - берем домен
        current_url = row.get('URL', row['Домен'])
        
        # Проверка на просадку
        if row['Total'] < threshold:
            reason = f"Скор {int(row['Total'])} (Медиана {int(median_score)})"
    ...
    trend_status = {"type": "flat", "msg": "➡️ **Ровный топ:** Конкуренция высокая."}
    if slope < -1.2:
        trend_status = {"type": "normal", "msg": "📉 **Нормальный топ:** Лидеры (1-5) лучше оптимизированы, чем хвост."}
    elif slope > 1.2:
        trend_status = {"type": "inverted", "msg": "📈 **Перевернутый топ:** ⚠️ Сайты внизу топа лучше лидеров. Топ-5 держится на ПФ/Бренде."}
