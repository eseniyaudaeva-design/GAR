...
with tab_wholesale_main:
    ...
    # ==========================================
    # ИНТЕРФЕЙС И НАСТРОЙКИ (ВЕРХНИЙ БЛОК)
    # ==========================================
    
    # 1. Единый контейнер API ключей
    with st.container(border=True):
        st.subheader("🔑 Настройки API (Сохраняются глобально)")
        ...
        with col_api1:
            ...
            if ws_gem_key_input: st.session_state['SUPER_GLOBAL_KEY'] = ws_gem_key_input
        with col_api2:
            turg_key = st.text_input("Тургенев API Key:", value=st.session_state.get('turg_key_bulk', ''), type="password", key="ws_turg_key_fixed")
            if turg_key: st.session_state['turg_key_bulk'] = turg_key
            use_turgenev_bulk = st.checkbox("Включить Тургенев", key="use_turgenev_bulk", value=st.session_state.get('use_turgenev_bulk', False))
        with col_api3:
            textru_key = st.text_input("Text.ru API Key:", value=st.session_state.get('textru_key_bulk', ''), type="password", key="ws_textru_key_fixed")
            if textru_key: st.session_state['textru_key_bulk'] = textru_key
            use_textru_bulk = st.checkbox("Включить Text.ru", key="use_textru_bulk", value=st.session_state.get('use_textru_bulk', False))
            use_ds_bulk = st.checkbox("Валидация DeepSeek", key="use_ds_bulk", value=st.session_state.get('use_ds_bulk', True))

    # 2. Настройки генерации и ввод
    with st.container(border=True):
        st.subheader("⚙️ Данные и Настройки")
        ...
        with c_i1:
            if "Подфильтровые" in gen_mode: 
                raw_urls = st.text_area("Вставьте ссылку на родительскую категорию (с новой строки):", height=120)
            elif "URL" in gen_mode:
                raw_urls = st.text_area("Ссылки на категории (с новой строки):", height=120)
            else: 
                raw_h1 = st.text_area("Список H1 (Маркер):", height=120)
                raw_h2 = st.text_area("Список H2 (Заголовок):", height=120)
        
        with c_i2:
            st.markdown("**Глобальные рубильники:**")
            ...
            with c_b2:
                st.checkbox("Авто-расчет (3-5 шт)", value=True, key="ws_auto_blocks")
    ...
    with c_start:
        ...
        if not is_running:
            ...
            st.info(f"⏳ Конвейер в работе: Обработка {curr + 1} из {q_len} ...")
        ...
        if task_idx < len(queue):
            ...
            with st.status(f"⚙️ Обработка: {h2_header} (Товар {task_idx + 1} из {len(queue)})", expanded=True) as status_logger:
                ...
                try:
                    ...
                    # 1. ЗАГРУЗКА БАЗ (EXCEL)
                    links_db = {}
                    if global_tags and os.path.exists("data/links_base.xlsx"):
                        try:
                            df_l = pd.read_excel("data/links_base.xlsx")
                            for _, r in df_l.iterrows():
                                u = str(r.iloc[0]).strip(); nm = str(r.iloc[1]).strip()
                                if u != 'nan' and nm != 'nan': links_db[nm.lower()] = {'url': u, 'name': nm}
                        except: pass
                    ...
                    if global_promo and os.path.exists("data/images_db.xlsx"):
                        try:
                            ...
                            for _, r in df_i.iterrows():
                                u = str(r.iloc[0]).strip(); img = str(r.iloc[1]).strip()
                    ...
                    safe_base_text = base_text_raw if str(base_text_raw).strip() and base_text_raw != "None" else "Техническая информация о товаре."
                    ...
                    if curr_use_text and client:
                        ...
                        status_logger.write(f"🤖 Пишем SEO-текст (Блоков: {auto_num_blocks})...")
                        ...
                        if not blocks_raw or "Error" in str(blocks_raw[0]):
                            status_logger.error(f"❌ Ошибка нейросети: {blocks_raw[0]}")
                        else:
                            for i_b in range(min(5, len(blocks_raw))): blocks[i_b] = blocks_raw[i_b]
                            generated_full_text = " ".join(blocks)
                        
                    if curr_use_tables and client:
                        dims_str = ", ".join(cat_dimensions)
                        prompt_tbl = f"ТЫ — СТРОГИЙ ТЕХНОЛОГ. Сгенерировать HTML-таблицу для '{h2_header}'. Контекст: {generated_full_text[:1500]}. Параметры:[{dims_str}]. Формат: <table class='brand-accent-table'><thead><tr><th>...</th></tr></thead><tbody></tbody></table>."
                    ...
                    # 3. ФОРМИРОВАНИЕ HTML МОДУЛЕЙ
                    if curr_use_tags:
                        html_t = []
                        for item in target_tag_data[:15]: html_t.append(f'<a href="{item["url"]}" class="tag-item">{item["name"]}</a>')
                        injections.append(f'<div class="popular-tags-text">\n<div class="popular-tags-inner-text">\n<div class="tag-items">\n{"\n".join(html_t)}\n</div>\n</div>\n</div>')
                        
                    if curr_use_promo:
                        gallery_items = []
                        for item in target_promo_data[:5]: gallery_items.append(f'<div class="gallery-item"><h3><a href="{item["url"]}" target="_blank">{item["name"]}</a></h3><figure><a href="{item["url"]}" target="_blank"><picture><img src="{item["img"]}" loading="lazy"></picture></a></figure></div>')
                    ...
                    if curr_use_geo and client:
                        ...
                        prompt_geo = f"Напиши один HTML параграф (<p>) о доставке товара '{h2_header}' в города: {cities}."
                    ...
                    for i_inj, inj in enumerate(injections):
                        ...
                        if blocks[target_idx]: blocks[target_idx] += "\n\n" + inj
                        else: blocks[target_idx] = inj
                    ...
                    # 5. ПРОВЕРКИ И API-КОНВЕЙЕР (СТРОГО ПОСЛЕДОВАТЕЛЬНО)
                    ...
                    status_logger.write("🔍 Отправляем на проверки API...")
                    ...
                    if st.session_state.get('use_turgenev_bulk') and st.session_state.get('turg_key_bulk') and plain_text_merged:
                        try:
                            ...
                            try:
                                ...
                                row_data['Тургенев Комментарий'] = "Ок" if t_num <= 5 else "Риск > 5"
                    ...
                    # Запись и переход к следующему шагу
                    ...
                    if st.session_state.auto_current_index < len(queue):
                        ...
                        st.session_state.pop('analysis_done', None); st.session_state.pop('analysis_results', None)
                        st.session_state.ws_waiting_for_analysis = True
                        st.rerun() # <- Рестарт происходит ТОЛЬКО после записи всей строки
                    else:
                        st.session_state.ws_automode_active = False
                        st.rerun()
    ...
    if not st.session_state.gen_result_df.empty:
            ...
            if st.button("🔄 ОБНОВИТЬ СТАТУСЫ TEXT.RU", type="primary", use_container_width=True):
                ...
                if txtru_key_active:
                    for idx, row in st.session_state.gen_result_df.iterrows():
                        if "⏳" in str(row.get('Уникальность', '')) and row.get('Text.ru UID'):
                            stts = check_textru_status_sync(row.get('Text.ru UID'), txtru_key_active)
        ...
        with col_dl:
            st.download_button(label=f"📥 СКАЧАТЬ EXCEL ({len(df_export)} шт.)", data=buffer.getvalue(), file_name=f"wholesale_SMART.xlsx", mime="application/vnd.ms-excel", disabled=is_running, use_container_width=True)
        with col_cl:
            if st.button("🗑️ Очистить таблицу", use_container_width=True): st.session_state.gen_result_df = st.session_state.gen_result_df.iloc[0:0]; st.rerun()

        with st.expander("👀 Техническая таблица результатов", expanded=False):
            st.dataframe(df_export, use_container_width=True)

        # 7. БЛОК ПРЕДПРОСМОТРА (ЛЕНТОЧНЫЙ ВЫВОД С СОХРАНЕНИЕМ СТИЛЕЙ)
        st.markdown("---")
        ...
        if 'Product Name' in df_export.columns:
            ...
            sel_p = st.selectbox("Выберите сгенерированный товар:", all_products, index=len(all_products)-1 if all_products else 0)
            ...
            if sel_p:
                ...
                if active_cols:
                    ...
                    <style>
                        ...
                        .brand-accent-table { width: 100%; border-collapse: collapse; text-align: left; }
                        .brand-accent-table th { background-color: #277EFF; color: white; padding: 12px; }
                        .brand-accent-table td { padding: 12px; border-bottom: 1px solid #eee; }
                    ...
                    st.markdown(f"### 📄 Контент для: {sel_p}")
