...
# === ЦИКЛ ПО ПАЧКЕ (v6.0: КОНТЕКСТ + ПРИМЕРЫ BAD/GOOD) ===
        for i, page in enumerate(target_pages_batch):
            ...
            # --- ПОДГОТОВКА ДАННЫХ ---
            try:
                # Используем стандартный requests (надежный вариант)
                ...
                # =========================================================
                # ШАГ 2. ГЕНЕРАЦИЯ ТАБЛИЦ (СТРОГИЙ ПРОМТ С КОНТЕКСТОМ)
                # =========================================================
                if use_tables and client:
                    ...
                    for t_topic in table_prompts:
                        ...
                        # Логика подачи ключей (только 1 раз)
                        if not keys_already_inserted and str_tables_final.strip():
                            curr_keys = str_tables_final
                            keys_instr = "Вставь эти данные, исправляя регистр."
                        else:
                            curr_keys = ""
                            keys_instr = ""

                        topic_guide = "Размеры и допуски" if "Размер" in t_topic else ("Химический состав" if "Хим" in t_topic else "Технические характеристики")

                        # === ПРОМТ v6.0 (ПОЛНЫЙ ФАРШ: КОНТЕКСТ + ПРИМЕРЫ) ===
                        prompt_tbl = f"""
    ТЫ — ТЕХНИЧЕСКИЙ РЕДАКТОР. Задача: Составить HTML-таблицу для "{header_for_ai}".
    ТЕМА: {t_topic} ({topic_guide})
    
    ВХОДНЫЕ ДАННЫЕ:
    1. СЫРЫЕ КЛЮЧИ: [{curr_keys}]
       -> {keys_instr}
    
    2. КОНТЕКСТ СТАТЬИ (ВАЖНО): {context_snippet}
       -> Проанализируй этот текст. Таблица не должна противоречить ему.
       -> Не пиши "воду" (высокое качество), которая уже есть в тексте. Давай только параметры.
    
    3. ИСТОРИЯ (УЖЕ НАПИСАНО): {previous_tables_context}
       -> Если параметр уже есть здесь, НЕ ПИШИ его в новую таблицу.
    
    --- ПРАВИЛА НАПИСАНИЯ (СОБЛЮДАТЬ СТРОГО) ---
    1. 🛑 РЕГИСТР БУКВ: Данные в "Ключах" могут быть с маленькой буквы. Исправь это!
       - ❌ ПЛОХО: ст3, 09г2с, 40х, гост 1050, вгп, aisi 304.
       - ✅ ХОРОШО: Ст3, 09Г2С, 40Х, ГОСТ 1050, ВГП, AISI 304.
       
    2. 🧠 ЛОГИКА ВНЕДРЕНИЯ:
       - Если ключ не подходит по смыслу в текущую тему (например "ВГП" в таблицу "Марки стали"), создай отдельную строку "Смежные товары" или "См. также".
       - НЕ ИСПОЛЬЗУЙ фразы отрицания ("Не путать с...", "Не является...").
    
    3. ТЕХНИКА:
       - Только код <table>...</table>.
       - Без заголовков перед таблицей.
       - Обязательно используй <thead>.
       - Класс таблицы: brand-accent-table.
    """
                        try:
                            # Температура 0.3 - строгость к правилам
                            ...
                            # Очистка заголовков "над" таблицей
                            ...
                            if start_idx != -1 and end_idx != -1:
                                ...
                                # Запоминаем контент для "Истории"
                                content_stripped = re.sub(r'<[^>]+>', ' ', clean_table_inner)
                                previous_tables_context += f"\n[Таблица {t_topic} уже содержит]: {content_stripped[:600]}..."
