...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 2. АБСОЛЮТНО РАНДОМНЫЕ ДАТЫ с 01.02.2020 по сегодня (БЕЗ СОРТИРОВКИ!)
    start_dt = date(2020, 2, 1)
    end_dt = date.today()
    delta_days = (end_dt - start_dt).days
    
    # Просто генерируем случайные даты для каждого отзыва
...
ПРАВИЛО 1: ЖЕСТКАЯ УНИКАЛЬНОСТЬ И СКЛОНЕНИЕ LSI (КРИТИЧНО!)
- Список LSI: {", ".join(lsi_words[:20])}
- КАЖДОЕ LSI-СЛОВО ИСПОЛЬЗУЙ СТРОГО 1 РАЗ НА ВЕСЬ МАССИВ ОТЗЫВОВ! Оборачивай в теги <b>...</b>.
- АБСОЛЮТНОЕ ПРАВИЛО: СКЛОНЯЙ LSI-слова по падежам и числам! Вписывай их в текст органично. 
- ПРИМЕР ОШИБКИ: "взял трубы из <b>нержавейка</b>". ПРАВИЛЬНО: "взял трубы из <b>нержавейки</b>". 

ПРАВИЛО 2: ГЕНДЕРНАЯ ГРАМОТНОСТЬ (ПРОВЕРЯЙ ПОЛ!)
- Если в списке указан Пол: Женский, ВСЕ глаголы прошедшего времени должны быть женского рода ("заказала", "приехала", "оплатила"). 
- Если Пол: Мужской — строго мужской род ("забрал", "позвонил"). Внимательно смотри на пол автора в списке!

ПРАВИЛО 3: "ШОУ, А НЕ РАССКАЗ" (БЕЗ ШАБЛОНОВ ПРОФЕССИЙ)
- КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ писать фразы: "Я как бухгалтер", "Работаю снабженцем", "Постоянно берем для цеха", "Муж в командировке". ЛЮДИ ТАК НЕ ПИШУТ!
- Профессия должна читаться между строк (через боли):
  * Прораб пишет: "Арматура не ржавая, длины в допуске, машина пришла к 8 утра без опозданий".
  * Офис пишет: "ЭДО подписали быстро, сертификаты на металл дали вместе с УПД".
  * Частник пишет: "Порезали профиль по 2 метра, влезло в прицеп, спасибо".

ПРАВИЛО 4: РЕАЛИСТИЧНЫЕ ОШИБКИ (ХАОТИЧНО ДЛЯ 30-40% АВТОРОВ)
...
- Опечатки "толстых пальцев": случайная соседняя буква по клавиатуре ЙЦУКЕН (швеллер ровныц, спсибо).

ПРАВИЛО 5: ОЦЕНКА 3.5 И 4.0 (КАК ПОВЫШАТЬ ДОВЕРИЕ)
- Для отзыва с оценкой 3.5 или 4.0 ЗАПРЕЩЕНО ругать саму компанию, недовес или наших сотрудников!
- ПРИЧИНА СНИЖЕНИЯ ОЦЕНКИ ТОЛЬКО ВО ВНЕШНЕМ ФОРС-МАЖОРЕ (придумай ситуацию):
  * Проблемы с банком (долго шли деньги).
  * Косяк нанятой самим клиентом транспортной компании (сломалась машина по пути, водитель перепутал адрес).
  * Сломался кран/размыло дорогу на самом объекте клиента.
- ОБЯЗАТЕЛЬНАЯ КОНЦОВКА такого отзыва: похвалить менеджеров базы за то, что пошли навстречу, вошли в положение, подождали. 

ПРАВИЛО 6: ОГРАНИЧЕНИЯ И ЗАПРЕТЫ
- ЗАПРЕТ НА ГОРОДА: Если в LSI есть город — ПРОИГНОРИРУЙ ЕГО.
- Никаких приветствий ("Всем привет", "Доброго времени").
- Смайлы: ТОЛЬКО 👍 и 👌 (не больше 1 раза на весь массив отзывов).
- ИСКЛЮЧИ стоп-слова: "качество на уровне", "оптимальный", "данный товар", "оперативно", "рекомендую", "без сюрпризов". Пиши проще ("нормально", "быстро", "пойдет", "без проблем").
...
"""
        ...
        # ЖЕСТКО ПРИШИВАЕМ РАНДОМНЫЕ ДАТЫ ЧЕРЕЗ PYTHON (нейросеть их больше не касается)
