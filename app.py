...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 2. Рандомные даты (ОТ 1 ЯНВАРЯ 2020 ДО ТЕКУЩЕГО ВРЕМЕНИ)
    ...
    # 3. Пол: 80% мужчин / 20% женщин
    male_count = int(target_count * 0.8)
    female_count = target_count - male_count
    gender_pool = ['Мужской'] * male_count + ['Женский'] * female_count
    random.shuffle(gender_pool)

    # 4. ТИПЫ И ПОДТИПЫ СЮЖЕТОВ (Иерархия из примеров)
    plot_types = [
        {
            "type": "Прагматик-экономист", 
            "subtypes": [
                "Искал дешевле, этот вариант оказался лучше пластика/аналогов.",
                "Сравнивал цены по рынку, здесь вышло дешевле.",
                "Брал на замену старой системы, получилось экономно."
            ]
        },
        {
            "type": "Частник-самоделкин", 
            "subtypes": [
                "Устанавливал сам (с мужем/другом) на выходных.",
                "Брал для ливневки/канализации на частном участке (в новый дом).",
                "Материал тяжелый, поднимали вдвоем, но зато полная тишина, соседи не жалуются."
            ]
        },
        {
            "type": "Прораб / Объект", 
            "subtypes": [
                "Брали для объекта/мастерской, привезли быстрее, чем обещали.",
                "Стыкуется отлично со старыми чугунными трубами/стояками.",
                "Сантехник ставил, сказал, что крест подошел идеально, материал хороший."
            ]
        },
        {
            "type": "Форс-мажор / Человеческий фактор", 
            "subtypes": [
                "Была задержка (отключение света/профилактика на складе), но менеджер Александр нашел выход.",
                "Помогли подобрать необходимый диаметр/параметры по телефону, оформили быстро.",
                "Пришлось постоять в очереди почти час, но качество металла всё компенсировало."
            ]
        },
        {
            "type": "Лаконичный", 
            "subtypes": [
                "Просто констатация: всё ок, спс, товар соответствует описанию.",
                "Спс норм труба/металл, привезли и ладно.",
                "Коротко: рекомендую для строительства."
            ]
        }
    ]

    # 5. ГЕНЕРАТОР ХАОСА (Строгий контроль: 1 идеальный, 1 с эмодзи)
    ...
    punctuation_errors = [
        "Вообще не используй запятые.",
        "Запятых очень мало, часто пропущены.",
        "Запятые стоят неправильно (не там, где нужно).",
        "Вместо пробела после пары слов поставь многоточие '...'",
        "Используй пробелы вместо точек между предложениями."
    ]
    
    register_errors = [
        "Весь текст написан строчными буквами (без заглавных).",
        "Начинай новые предложения с маленькой буквы.",
        "Вдруг одно случайное слово в середине напиши с Заглавной буквы."
    ]

    typo_errors = [
        "Используй интернет-сокращения (спс, норм, щас).",
        "Сделай пару реалистичных опечаток и описок (достсвка, новьий, довлна, нареканий нареканий).",
        "Сделай ошибку в окончании слова или пропусти букву."
    ]

    authors_listing = []
    
    for i in range(target_count):
        if i < len(chosen_authors): name = chosen_authors[i].get('name', f'Автор_{i}')
        else: name = f'Автор_{i}'
            
        assigned_gender = gender_pool[i]
        gender_instruction = "Строго ЖЕНСКИЙ пол (глаголы: заказала, установила)" if assigned_gender == 'Женский' else "Строго МУЖСКОЙ пол (глаголы: заказал, установил)"

        # Выбираем сюжет
        cat = random.choice(plot_types)
        sub = random.choice(cat["subtypes"])
        plot_instruction = f"СЮЖЕТ: {cat['type']} -> {sub}"
        
        # Корректировка под оценку
        if ratings[i] == 3.5:
             plot_instruction += " Обязательно добавь минус: внутренняя поверхность с шероховатостями ИЛИ взбесила инфраструктура базы (очередь)."
        elif ratings[i] in [4.0, 4.5]:
             plot_instruction += " В целом всё хорошо, но есть микро-придирка (тяжело поднимать, небольшая задержка)."
        else:
             plot_instruction += " Успешная покупка."

        # Стилистика и комбинирование ошибок
        if i == perfect_index:
            style_inst = "СТИЛЬ: Идеально грамотный, литературный русский язык. Все знаки препинания на месте. 0 ошибок."
        else:
            errors_to_apply = []
            if random.random() > 0.3: errors_to_apply.append(random.choice(punctuation_errors))
            if random.random() > 0.4: errors_to_apply.append(random.choice(register_errors))
            if random.random() > 0.6: errors_to_apply.append(random.choice(typo_errors))
            
            if not errors_to_apply: errors_to_apply.append(random.choice(punctuation_errors)) # Гарантия хотя бы одной ошибки
            style_inst = "СТИЛЬ: Небрежный живой язык из интернета. Примени эти искажения:\n- " + "\n- ".join(errors_to_apply)

        if i == emoji_index:
            style_inst += "\n- ЭМОДЗИ: Добавь 1-3 простых смайлика (👍, 👌)."
        else:
            style_inst += "\n- СТРОГИЙ ЗАПРЕТ: Никаких смайликов и эмодзи."

        # Разная длина и структура
        length_inst = random.choice([
            "Структура: очень коротко (1-2 рубленые фразы).",
            "Структура: средняя длина, одним сплошным потоком без разбивки на предложения.",
            "Структура: развернуто (3-4 предложения)."
        ])

        authors_listing.append(
            f"--- Отзыв #{i+1} ---\n"
            f"Имя: {name} | Пол: {gender_instruction} | Оценка: {ratings[i]}\n"
            f"{plot_instruction}\n"
            f"{length_inst}\n"
            f"{style_inst}\n"
        )
    ...
    try:
        ...
        prompt = f"""Ты имитируешь обычных людей, пишущих отзывы о покупке стройматериалов/металла (текущий товар: "{h2_header}").
Твоя главная цель - сделать отзывы МАКСИМАЛЬНО очеловеченными, написанными абсолютно по-разному.
Используй заданные ошибки, опечатки и отсутствие запятых, чтобы текст выглядел естественно-небрежным, как переписка в мессенджере.

Напиши {target_count} отзывов.
Задачи для каждого:
{nl.join(authors_listing)}

ГЛОБАЛЬНЫЕ ПРАВИЛА:
1. LSI СЛОВА: {", ".join(lsi_words[:20])}
- Каждое LSI-слово используй СТРОГО 1 РАЗ на всю пачку! Оберни в <b>...</b>.
- Обязательно склоняй слова по падежам органично.
2. ЗАПРЕТЫ КАНЦЕЛЯРИТА:
- Никаких двоеточий (:), длинных тире (—), кавычек.
- Никаких фраз-шаблонов ботов ("качество на высоте", "соответствует заявленному", "рекомендую к сотрудничеству").
3. ВЕРНИ СТРОГО JSON МАССИВ (Ключи на русском):
[{{ "Имя": "...", "Оценка": ..., "Текст": "..." }}]
"""
        
        # Переключено на ту же модель, что у генератора FAQ и текстов
