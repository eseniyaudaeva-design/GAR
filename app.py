...
# 2. ФУНКЦИЯ ГЕНЕРАЦИИ (ОБНОВЛЕННАЯ С НОВЫМ ПРОМТОМ)
    def generate_lsi_article(api_key, topic, lsi_keywords):
        ...
        system_instruction = (
            ...
            "Стиль: Деловой, экспертный, но понятный. Избегай воды. "
            "Ты выдаешь ТОЛЬКО HTML-код."
            "СТРОГИЕ ЗАПРЕТЫ: "
            "1. Не используй упоминания Украины. "
            "2. НИКОГДА не используй ссылки. "
            "3. Не используй Markdown (```)."
        )
        
        # === ПОЛЬЗОВАТЕЛЬСКИЙ ПРОМТ ===
        user_prompt = f"""
        ЗАДАЧА: Напиши полноценную статью для категории каталога: "{topic}".
        Статья должна состоять ровно из 3-х логических блоков.
        
        ОБЩИЕ ТРЕБОВАНИЯ:
        1. ОБЪЕМ: Каждый блок должен быть объемным (около 800 символов). Раскрывай тему подробно.
        2. ЧИСТОТА: Исключи любые ссылки на источники.
        3. ПОЛЬЗА: Текст должен быть технически грамотным и полезным для специалиста по закупкам. Избегай "воды".
        
        LSI ИНСТРУКЦИЯ (СЛОВА):
        Равномерно распредели по всему тексту следующие слова: {{{lsi_string}}}.
        Вписывай их естественно, меняя окончания и падежи. 
        ВАЖНО: Выдели внедренные слова жирным шрифтом (<b>слово</b>).

        SEO ИНСТРУКЦИЯ (ЗАГОЛОВКИ):
        В заголовке КАЖДОГО из 3-х блоков обязательно используй основную фразу "{topic}".
        ВАЖНО: Не вставляй её в "сыром" виде везде одинаково. Переформулируй, склоняй, разбавляй другими словами, вписывай естественно в контекст заголовка.

        СТРУКТУРА КАЖДОГО БЛОКА (Строго соблюдай этот шаблон для каждого из 3-х блоков):
        1. Заголовок (<h2> только для 1-го блока, <h3> для 2-го и 3-го).
        2. Первый абзац текста (<p>) - развернутый, информативный.
        3. Вводное предложение, подводящее к списку (например: "Основные характеристики:", "Сферы применения:", "Преимущества:").
        4. Маркированный список (<ul> c <li>).
        5. Второй (завершающий) абзац текста (<p>) - развернутый.
        
        ФИНАЛЬНЫЕ УСЛОВИЯ:
        - Выдай ЕДИНЫЙ HTML код (все 3 блока идут друг за другом).
        - Никаких вводных слов типа "Вот ваш код".
        - Никакого Markdown (```).
        """
