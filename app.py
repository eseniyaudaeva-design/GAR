...
def generate_ai_content_blocks(api_key, base_text, tag_name, forced_header, num_blocks=5, seo_words=None):
    if not base_text: return["Error: N base text"] * num_blocks
    ...
    ЖЕСТКИЕ ПРАВИЛА ИСПОЛЬЗОВАНИЯ LSI:
    1. ЗАПРЕЩЕНО вставлять LSI-слова списком или через запятую.
    2. Каждое LSI-слово должно быть ГАРМОНИЧНО вписано в контекст предложения. 
    3. Используй разные падежи и формы слов (склоняй их), чтобы текст читался естественно.
    4. Распределяй LSI-слова равномерно по всей статье, а не группируй их в одном абзаце.
    5. Если слово не подходит по смыслу — пропусти его или перефразируй предложение, но сохрани пользу для читателя.
    6. Текст должен быть человечным, без признаков машинной генерации и "переспама".
    -------------------------------------------
    """

    # Твоя оригинальная системная роль + правила оформления (тире, точки запятой)
    system_instruction = (
        ...
        "Формула Главреда для B2B: В тексте должны быть ответы на вопросы: что это? какую проблему решает? кому подойдет? какие есть разновидности? "
        "СТРОГИЕ ЗАПРЕТЫ: "
        "1. Не используй упоминания Украины, украинских городов, политические темы, валюту гривну. Контент строго для РФ. "
        ...
        "ПРАВИЛА ОФОРМЛЕНИЯ: "
        "1. Каждый пункт любого маркированного списка должен строго заканчиваться точкой с запятой (;), а самый последний — точкой (.). "
        "2. Числовые диапазоны (длина, вес) пиши СТРОГО через тире: '10-20 мм'. Запрещено писать 'от 10 до 20 мм'. "
        "3. Не ставь двоеточие внутри пунктов списка."
    )

    # Твой оригинальный пользовательский промт
    user_prompt = f"""
    ИСХОДНЫЕ ДАННЫЕ:
    ...
    (Ты ОБЯЗАН вставить |||BLOCK_SEP||| между каждым блоком, чтобы система могла их разрезать).
    ...
    1. Заголовок (<h2> только для 1-го блока, <h3> для остальных). Заголовок H2 должен быть: <h2>{h2_topic}</h2>
    ...
    try:
        ...
        # Очистка от Markdown, если нейросеть всё же его выдала
        ...
        # Если ИИ проигнорировал разделитель, отдаем как один блок
        if not blocks: blocks = [content]
            
        cleaned_blocks =[]
        for b in blocks:
            if b: cleaned_blocks.append(b)
            
        # Гарантируем нужное количество блоков, добивая пустыми
    ...
    for i in range(target_count):
        ...
        if ratings[i] == 5.0:
            sentiment = random.choice([
                "5 звезд: всё прошло штатно, обычная рабочая закупка. Никаких проблем не возникло.",
                "5 звезд: помогли разобраться с выбором, сами не лезли с советами - только когда спросил.",
                "5 звезд: всё чётко, без лишних слов. Взял что надо, уехал.",
                "5 звезд: брал не первый раз, стабильно всё нормально.",
            ...
            sentiment = random.choice([
                ...
                f"{ratings[i]} звезды: одна позиция оказалась не та по размеру - выяснилось что я сам неправильно указал в заявке. Переоформили без проблем, но время потерял.",
                f"{ratings[i]} звезды: пришлось немного подождать пока погрузят - очередь была. Но это мелочи, в целом всё нормально.",
                f"{ratings[i]} звезды: немного напутали с документами при отгрузке, но быстро исправили по звонку.",
            ...
            sentiment = random.choice([
                ...
                "3.5 звезды: резка заняла дольше чем обещали - загрузили станок под завязку в тот день. Предупредили, но поздновато. Зато качество реза нормальное.",
            ])
...
2. РЕАЛИСТИЧНАЯ ДОСТАВКА: 
       ЖЁСТКИЙ ЗАПРЕТ: никогда не пиши "доставили в тот же день", "привезли сегодня", 
...
3. СТОП-СЛОВА (Маркеры ИИ): Никогда не используй: безупречный, высококачественный, 
    ...
    manager и любых других. Только кириллица. Исключение - марки стали (AISI 304) и стандарты.
