...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # Динамические даты с 2020 года по сегодняшний день
    ...
    try:
        ...
        prompt = f"""Ты — алгоритм генерации максимально реалистичного пользовательского контента.
Тема: "{h2_header}". 
Твоя задача: написать {target_count} отзывов. 

ГЛАВНОЕ ПРАВИЛО: ПОЛНЫЙ ОТКАЗ ОТ КАРИКАТУР И ШАБЛОНОВ. 
Каждый отзыв должен быть уникальным по стилистике, длине и точке фокуса. Не начинай отзывы одинаково.

АБСОЛЮТНЫЙ ХАОС ОШИБОК И ОПЕЧАТОК (КРИТИЧЕСКИ ВАЖНО):
1. Распределение: 
   - Примерно 60-70% отзывов ДОЛЖНЫ содержать ошибки (от легких до заметных).
   - Оставшиеся 30-40% — "чистые" (без грубых речевых, лексических и грамматических ошибок). Но даже в них могут быть мелкие пунктуационные огрехи (отсутствие точки в конце, одна пропущенная запятая, всё написано с маленькой буквы).
2. Запрет на повторение: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать одинаковый шаблон ошибок. Если один отзыв содержит только опечатки (перепутаны буквы), следующий не может использовать этот же прием — пусть там будет кривая грамматика или отсутствие запятых. МИКШИРУЙ ИХ!
3. Каталог ошибок (выбирай случайно для каждого текста):
   - Опечатки: перестановка соседних букв ("бытсро", "спсибо"), промах по клавиатуре ("ш" вместо "щ", "а" вместо "о" в безударных).
   - Пунктуация: лишние запятые (где они не нужны), пропуск нужных запятых, пропуск пробела после запятой, многоточия не к месту, полное отсутствие знаков препинания.
   - Речевые/Грамматические: кривое согласование слов ("забирал с склад"), неправильные окончания, использование разговорных сокращений ("щас", "спс", "походу", "вобщем", "норм").
   - Регистр: текст полностью строчными (маленькими) буквами, случайная ЗАГЛАВНАЯ буква посреди слова, отсутствие большой буквы в начале предложения.
4. Естественность: Ошибки должны выглядеть как результат спешки реального человека. Не делай текст абсолютно нечитаемым бредом.

КРИТИЧЕСКИЕ ОГРАНИЧЕНИЯ (НАРУШЕНИЕ СМЕРТЕЛЬНО):
1. ЛОГИКА ОЦЕНОК 3.5 И НИЖЕ 5.0: 
   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО ругать компанию, склад, логистов, качество товара. 
   - Причина снижения — ИСКЛЮЧИТЕЛЬНО внешний форс-мажор: пробки, снегопад, фура ТК сломалась, сам клиент перепутал адрес.
   - ОБЯЗАТЕЛЬНО: укажи, что НАШИ менеджеры всегда были на связи и спасли ситуацию. Товар идеальный. Оценка снижена чисто за нервы из-за третьих лиц.
2. НИКАКОГО АНГЛИЙСКОГО: Исключительно кириллица. Запрещены английские аббревиатуры.
3. НИКАКИХ ГОРОДОВ: Если в LSI есть города (Москва, Спб и т.д.) — игнорируй их полностью.
4. СМАЙЛЫ: ТОЛЬКО 👍 и 👌. Строго запрещены ✅ и любые другие. Смайлы ставить не везде (а хаотично).
5. ТИРЕ: Только короткий дефис "-". Длинное "—" под запретом.
6. РОД: Строго соблюдай пол автора (Ж - "заказывала", М - "заказывал").

LSI-СЛОВА И ТЕГИ:
Список: {", ".join(lsi_words[:20])}. 
Склоняй их максимально естественно. 
ОБЯЗАТЕЛЬНО выделяй LSI-слова тегами <b>...</b> (например, <b>стальная труба</b>). Запрещено использовать **.

ВЕРНИ СТРОГО JSON МАССИВ (без markdown-разметки, просто массив):
