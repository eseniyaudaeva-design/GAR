...
def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        try:
            ...
            return "Error: Library 'openai' not installed"
        ...
        [I] ЭТАП 1: ЖЕСТКАЯ СТРУКТУРА
        
        1.1. Заголовок: Выведи строго: <h2>{exact_h2}</h2>.
        
        1.2. КОММЕРЧЕСКИЙ ХУК (БЭНГЕР):
        Это 2-3 коротких, ударных предложения в самом начале.
        СТРОЖАЙШИЙ ЗАПРЕТ: Не начинай со слов "Предлагаем", "Реализуем", "Продаем", "Компания". Не пиши определений ("Это изделие...").
        
        КАК НАДО (примеры стиля):
        - "Ваш самый стойкий сотрудник. Не боится дождя, снега и агрессивной среды."
        - "Дефекты на входе – брак на выходе. Наша катанка – чистая стартовая точка для вашей проволоки."
        ...
        ОБЯЗАТЕЛЬНО вставь код контактов:
        {contact_html_block}
        
        1.4. Подводка к списку 1:
        Фраза должна подводить к характеристикам и СТРОГО заканчиваться двоеточием (:).
        Пример: "Рабочие параметры продукции:"
        
        1.5. Список №1 (6 пунктов): ТЕХНИЧЕСКИЕ ХАРАКТЕРИСТИКИ.
        КРИТИЧЕСКИ ВАЖНО: 
        - ЗАПРЕЩЕНО писать общие фразы ("Высокая прочность", "Точность размеров").
        - НАДО писать КОНКРЕТНЫЕ ЦИФРЫ. Если точных данных нет — придумай реалистичный диапазон для этого типа металла/товара.
        - Формат: "Параметр: Значение".
        Пример:
        <ul>
            <li>Предел текучести: 350 МПа;</li>
            <li>Толщина стенки: 3–10 мм (ГОСТ 8732);</li>
            <li>Температурный режим: от -70 до +450 °C;</li>
            <li>Слой цинка: 40-60 мкм;</li>
            <li>...</li>
        </ul>
           
        1.6. Информационный абзац 2: Про производство/стандарты.

        1.7. Подзаголовок H3 (Релевантный теме).
        
        1.8. Информационный абзац 3.
        
        1.9. Подводка к списку 2: СТРОГО заканчивается двоеточием (:).
        
        1.10. Список №2 (6 пунктов): ПРИМЕНЕНИЕ ИЛИ ВЫГОДЫ.
        - Не "Строительство", а "Заливка фундаментов высотных зданий".
        - Не "Долговечность", а "Срок службы без коррозии 25 лет".
        ...
        1.14. Подводка к списку 3: СТРОГО заканчивается двоеточием (:).
        
        1.15. Список №3 (6 пунктов): Сервис/Условия. 
        - "Отгрузка: в день оплаты", "Резка: в размер на лентопиле".
           
        1.16. Финальный абзац: Призыв к действию ("Закажите расчет...", "Скомплектуем объект...").

        [II] ОГРАНИЧЕНИЯ И LSI:
        1. Объем: ~3500 знаков.
        2. LSI: Внедри слова: {{{lsi_string}}}. Выдели их <b>жирным</b>.
        3. СТОП-СЛОВА (Штраф за использование): ({stop_words_list}).

        [III] СТИЛЬ:
        - Метафоричный в заголовке, сухой и цифровой в списках.
        - ТОЛЬКО HTML код в ответе.
        """
