...
with tab_lsi_gen:
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
            ...
            system_instruction = (
                "Ты — строгий технический редактор справочника по металлопрокату и оборудованию. "
                "Твой стиль: сухой, энциклопедический, безэмоциональный. "
                "Ты ненавидишь вводные слова и маркетинговые прилагательные."
            )
            
            user_prompt = f"""
            ЗАДАЧА: Напиши сухую техническую статью для товара: "{exact_h2}".
            
            [I] ГЛАВНЫЕ ТРЕБОВАНИЯ К ОФОРМЛЕНИЮ (СТРОГО):
    
            1. ЗАГОЛОВКИ H3 (ШАБЛОННЫЕ):
               - СТРОГИЙ ФОРМАТ: "Существительное + Товар".
               - ПРИМЕРЫ: "Технические характеристики {exact_h2}", "Производство {exact_h2}", "Назначение {exact_h2}".
               - ЗАПРЕЩЕНО: Знаки препинания (двоеточия, тире, точки), прилагательные ("Важные", "Уникальные", "Эффективные"), слова "Особенности", "Преимущества", "Гарантия".
               - Товар в заголовке склоняй по смыслу!
            ...
            3. ФОРМАТ ЧИСЕЛ:
               - ТОЛЬКО ЗНАЧАЩИЕ ЦИФРЫ (Никаких "3,0").
               - Дробные через запятую (2,5).
    
            4. ДИАПАЗОНЫ И ЛОГИКА (КРИТИЧНО):
               - "Рабочая температура": можно диапазон ("от -40 до +450 °C").
               - "Максимальная температура" (или давление/нагрузка): ПИСАТЬ ОДНО ЧИСЛО.
               - ОШИБКА: "Максимальная температура от 1000 до 1400".
               - ПРАВИЛЬНО: "Макс. температура +1400 °C" ИЛИ "Температурный режим 1000-1400 °C".
                 
            5. ПОДВОДКИ К СПИСКАМ:
               - Всегда заканчиваются двоеточием (:).
            
            [II] СТРУКТУРА ТЕКСТА:
            
            1.1. Заголовок: <h2>{exact_h2}</h2>.
            
            1.2. БЭНГЕР: 3-4 предложения. Сухое определение: что это, из чего сделано, главный стандарт (ГОСТ/ТУ).
            ...
            1.5. Список №1 (6 пунктов): ОСНОВНЫЕ ТЕХНИЧЕСКИЕ ПАРАМЕТРЫ.
            (Размеры, Диаметры, Марки стали/сплава).
               
            1.6. Абзац 2. Описание материального исполнения (сплавы, методы изготовления).
            
            1.7. ТАБЛИЦА ХАРАКТЕРИСТИК (СПРАВОЧНАЯ):
            Создай таблицу на 4-6 строк.
            !!! ВАЖНО: В таблице НЕ ДОЛЖНО БЫТЬ дублей из Списка №1.
            НАПОЛНЕНИЕ: Только редкие справочные данные:
            - Нормативный документ (ГОСТ/DIN);
            - Химический состав / Марка;
            - Плотность / Удельный вес;
            - Твердость / Предел прочности;
            - Форма поставки / Упаковка.
            ...
            <table class="brand-accent-table">
                ...
                <tbody>
                    <tr><td>[Справочный параметр 1]</td><td>[Значение 1]</td></tr>
                    <tr><td>[Справочный параметр 2]</td><td>[Значение 2]</td></tr>
                    <tr><td>[Справочный параметр 3]</td><td>[Значение 3]</td></tr>
                    <tr><td>[Справочный параметр 4]</td><td>[Значение 4]</td></tr>
                </tbody>
            </table>
            
            1.8. Подзаголовок H3: 
            СТРОГО ОДИН ИЗ ВАРИАНТОВ: "Классификация {exact_h2}" ИЛИ "Конструкция {exact_h2}". (Склоняй товар!).
            
            1.9. Абзац 3. Детальное описание выбранной темы H3.
            
            1.10. Подводка к списку 2 (:).
            
            1.11. Список №2 (6 пунктов): СФЕРЫ ПРИМЕНЕНИЯ.
            (Отрасли: Машиностроение, Нефтегаз, Энергетика).
               
            1.12. Абзац 4. Технические условия эксплуатации.
                                  
            1.13. Подзаголовок H3:
            СТРОГО ОДИН ИЗ ВАРИАНТОВ: "Монтаж {exact_h2}" ИЛИ "Обработка {exact_h2}". (Склоняй товар!).
            
            1.14. Абзац 5. Описание процессов работы с товаром.
            
            1.15. Подводка к списку 3 (:).
            
            1.16. Список №3 (6 пунктов): ЭКСПЛУАТАЦИОННЫЕ СВОЙСТВА.
            (Коррозионная стойкость, Свариваемость, Ремонтопригодность).
               
            1.17. Абзац 6. Резюме и условия заказа.
    
            [III] ДОПОЛНИТЕЛЬНО:
            1. LSI: {{{lsi_string}}}. (Вписать органично).
            2. СТОП-СЛОВА: ({stop_words_list}).
            3. Выведи ТОЛЬКО HTML (без markdown).
            """
