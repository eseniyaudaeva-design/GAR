...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # --- 5. БАЗОВЫЕ СЮЖЕТЫ (ТОЛЬКО ЧАСТНИКИ) ---
    plot_types = [
        "Прагматик: искал где выгоднее доставка или цена, сравнивал варианты для своего участка.",
        "Самоделкин: брал для личных нужд (ремонт, дача, строительство своего дома), делал всё сам или с напарником/женой.",
        "Обычный покупатель: просто заказал нужный материал для бытовых задач, без лишних деталей."
    ]
    ...
    for i in range(target_count):
        ...
        gender_instruction = "Строго ЖЕНСКИЙ пол (глаголы: заказала, привезла)" if assigned_gender == 'Женский' else "Строго МУЖСКОЙ пол (глаголы: заказал, привез)"

        # Сборка сюжета
        base_plot = random.choice(plot_types)
        plot_instruction = f"СЮЖЕТ: {base_plot} Адаптируй этот сюжет СТРОГО под применение товара '{h2_header}' в частном доме или при мелком ремонте. СТРОГИЙ ЗАПРЕТ: Никаких промышленных масштабов, оптовых баз и никаких рекламных УТП."
        
        if ratings[i] == 5.0:
             plot_instruction += " ВНИМАНИЕ: Оценка 5.0. Текст должен быть позитивным. ЗАПРЕЩЕНО упоминать минусы."
        elif ratings[i] == 3.5:
             plot_instruction += " ВНИМАНИЕ: Оценка 3.5. Обязательно добавь существенный минус сервиса (например, долгая очередь на загрузку, менеджер перепутал адрес), но сам товар нормальный."
        else:
             plot_instruction += " Оценка 4.0 или 4.5. Упомяни мелкую, незначительную бытовую придирку."
        ...
        if i == ultra_short_index:
            length_inst = "Структура: Ультра-коротко (например, 'все ок', 'нормально'). Максимум 3-5 слов."
        else:
            length_inst = random.choice([
                "Структура: средняя длина (1-2 обычных предложения).",
                "Структура: развернуто (3-4 предложения)."
            ])
            
        # Эмодзи (независимо от длины)
        if i == emoji_index:
            style_inst += "\n- ЭМОДЗИ: Добавь 1 простой смайлик (👍, 👌, 🔥). В остальных отзывах смайлики ЗАПРЕЩЕНЫ."
        else:
            style_inst += "\n- ЭМОДЗИ: СТРОГИЙ ЗАПРЕТ на смайлики."
    ...
    try:
        ...
        prompt = f"""Ты имитируешь обычных людей, покупающих стройматериалы/металл исключительно для частных нужд (текущий товар: "{h2_header}").
Твоя главная цель - сделать отзывы МАКСИМАЛЬНО очеловеченными, бытовыми и написанными по-разному.
Перед тем как писать, подумай: как этот товар применяют в быту (дача, ремонт, свой дом).
...
ГЛОБАЛЬНЫЕ ПРАВИЛА:
1. LSI СЛОВА: {", ".join(final_lsi)}
- Каждое LSI-слово используй СТРОГО 1 РАЗ на всю пачку! Оберни в <b>...</b>.
- Обязательно склоняй слова по падежам органично.
2. ЗАПРЕТЫ КАНЦЕЛЯРИТА И СИМВОЛОВ:
- Никаких двоеточий (:), длинных тире (—), кавычек.
- Никаких фраз-шаблонов ("качество на высоте", "соответствует заявленному", "рекомендую к сотрудничеству").
- НИКАКИХ УТП (уникальных торговых предложений), маркетингового жаргона и восхвалений магазина.
