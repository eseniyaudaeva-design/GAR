...
def classify_semantics_with_api(words_list, yandex_key):
    """
    Исправленная классификация: ПРИОРИТЕТ СЛОВАРЯ МЕТАЛЛА НАД ВСЕМ ОСТАЛЬНЫМ.
    """
    # 1. Загружаем наши словари (Функция, которую ты уже добавил выше)
    # Важно: load_custom_dictionaries должна возвращать 3 значения (как в твоем коде)
    ...
    # Список стандартного мусора, который мог перебивать товары
    # Раньше слова "профиль" или "лист" могли попасть сюда.
    SITE_UI_GARBAGE = {
        'меню', 'поиск', 'главная', 'карта', 'сайт', 'личный', 'кабинет', 'вход', 'регистрация', 
        'корзина', 'избранное', 'сравнение', 'телефон', 'адрес', 'контакты', 'email', 
    ...
    for word in words_list:
        ...
        # 1. Сначала проверяем регулярки (числа, размеры)
        ...
        # =========================================================================
        # ВАЖНО: ПРОВЕРКА ПО JSON-СЛОВАРЮ ИДЕТ ПЕРВОЙ!
        # =========================================================================
        
        # Если слово есть в файле metal_products.json - это ТОВАР.
        # Даже если это слово "профиль", "лист" или "круг".
        if lemma in PRODUCTS_SET:
            categories['products'].add(lemma)
            continue # <-- ВЫХОДИМ, чтобы не попасть в блок коммерции ниже
            
        if lemma in SPECS_SET:
            categories['dimensions'].add(lemma)
            continue

        # 3. Только теперь проверяем на коммерцию и мусор
        if lemma in COMM_SET or lemma in SITE_UI_GARBAGE:
            categories['commercial'].add(lemma)
            continue
            
        # 4. Проверка на услуги
        if lemma in SERVICE_KEYWORDS or lemma.endswith('обработка') or lemma.endswith('изготовление'):
            categories['services'].add(lemma)
            continue

        # Если никуда не попало - оставляем на API/Эвристику
        api_candidates.append(word_lower)

    # --- ПОСТ-ОБРАБОТКА (ЕСЛИ API ПОДКЛЮЧЕН) ---
    yandex_results = {} 
    ...
    for word in api_candidates:
        ...
        # ЕЩЕ РАЗ проверяем по словарю (на случай если API вернуло нормализацию)
        if lemma in PRODUCTS_SET:
             categories['products'].add(lemma)
             continue
        ...
        if pos == 'noun' or pos == 'unknown':
            if len(lemma) > 2: 
                # Если существительное не найдено ни в мусоре, ни в коммерции - считаем товаром
                categories['products'].add(lemma)
        else: 
            # Прилагательные, глаголы и прочее кидаем в коммерцию/мусор
            categories['commercial'].add(lemma)
