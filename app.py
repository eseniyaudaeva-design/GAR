...
with tab_lsi_gen:
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        """
        Генерация через OpenAI client (LiteLLM), как во вкладке 2.
        """
        if not api_key: return "Error: No API Key"
        
        # Импорт ВНУТРИ функции, чтобы избежать ошибок инициализации
        try:
            from openai import OpenAI
            client = OpenAI(api_key=api_key, base_url="https://litellm.tokengate.ru/v1")
        except ImportError:
        ...
        1. Структура текста (Строго соблюдай):
        
        1.1. Тема: Выведи строго: <h2>{exact_h2}</h2> (дословно).
        
        1.2. Коммерческий бэнгер/хук: Яркое первое предложение.
        
        1.3. Информационный абзац 1: Суть продукта.
        ВАЖНО: Вставь ОБЯЗАТЕЛЬНО этот код контактов:
        {contact_html_block}
        
        1.4. Обобщающее предложение.
        
        1.5. Список №1 (6 пунктов): Характеристики/преимущества. <ul><li>...</li></ul>.
           
        1.6. Информационный абзац 2: Технология/ГОСТы.

        1.7. Подзаголовок (H3).
        
        1.8. Информационный абзац 3.
        
        1.9. Обобщающее предложение.
        
        1.10. Список №2 (6 пунктов): Физ-мех свойства. <ul><li>...</li></ul>.
           
        1.11. Информационный абзац 4.
                              
        1.12. Подзаголовок (H3, например "Производство").
        
        1.13. Информационный абзац 5.
        
        1.14. Обобщающее предложение.
        
        1.15. Список №3 (6 пунктов): Применение/детали. <ul><li>...</li></ul>.
           
        1.16. Финальный абзац с призывом (без слова "купить", используй "заказать", "оформить").

        [II] ОГРАНИЧЕНИЯ И LSI:
        1. Объем: до 4000 знаков.
        2. LSI: Внедри слова: {{{lsi_string}}}. Выдели их <b>жирным</b>.
        3. СТОП-СЛОВА (избегать): ({stop_words_list}).

        [III] СТИЛЬ:
        - Деловой, экспертный. Без воды.
        - Переменные #PHONE# и #EMAIL# оставь как есть.
        - ТОЛЬКО HTML код в ответе.
        """
        
        try:
            # Вызов аналогичный 2-й вкладке
            response = client.chat.completions.create(
                model="google/gemini-2.5-pro",
                messages=[
                    {"role": "system", "content": system_instruction},
