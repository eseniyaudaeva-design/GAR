...
if use_tables:
    with st.container(border=True):
        ...
        # --- ОПТИМИЗАЦИЯ 1: Размещаем контекст и выбор кол-ва в одну строку ---
        ...
        with col_ctx:
            tech_context_final_str = st.text_area(
                ...
                height=68, # Чуть уменьшили высоту
        ...
        # --- ОПТИМИЗАЦИЯ 2: Создаем колонки динамически на основе cnt ---
        # Теперь блоки будут стоять РЯДОМ, а не друг под другом
        cols = st.columns(cnt)
        
        for i, col in enumerate(cols):
            with col:
                # Визуально выделяем каждый блок, если нужно, или просто используем пространство колонки
                st.caption(f"**Таблица {i+1}**")
                
                # Определяем дефолтный индекс
                default_idx = 0
                if i == 0: 
                    try: default_idx = table_presets.index("Технические характеристики")
                    except: pass
                elif i == 1:
                    try: default_idx = table_presets.index("Таблица размеров")
                    except: pass
                elif i == 2:
