...
def generate_full_article_v2(api_key, h1_marker, h2_topic, lsi_list):
    ...
    [II] ОФОРМЛЕНИЕ И ПУНКТУАЦИЯ (СТРОГО СОБЛЮДАТЬ):
    
    1. СПИСКИ <ul> (Маркированные):
       - Используй для перечисления характеристик, свойств, сфер применения.
       - ВАЖНО: Каждый пункт списка заканчивай ТОЧКОЙ С ЗАПЯТОЙ (;).
       - ВАЖНО: Самый последний пункт списка заканчивай ТОЧКОЙ (.).
       - Пример: 
         <li>Диаметр 50 мм;</li>
         <li>Вес 10 кг.</li>
       
    2. СПИСКИ <ol> (Нумерованные):
       - Используй ТОЛЬКО для пошаговых процессов (этапы производства, монтаж).
       - ВАЖНО: Каждый пункт нумерованного списка заканчивай ТОЧКОЙ (.).
       
    3. ДИАПАЗОНЫ ЧИСЕЛ:
       - Пиши строго через тире: "2-160 мм", "300-500 кг".
       - ЗАПРЕЩЕНО писать "от ... до ..." (Например: НЕЛЬЗЯ "от 2 до 160 мм").
       
    4. ОБЩЕЕ:
       - Не используй двоеточие (:) ВНУТРИ пунктов списка.
    ...
    (Формат: <ul>. Пунктуация: точка с запятой, в конце точка).
    Цифры диапазонов пиши через тире (10-20 мм).
    ...
    1.14. Абзац 5. Технология работы (используй <ol> если описываешь шаги, ставь точки в конце).
    
    1.15. Подводка к списку 3 (:).
    
    1.16. Список №3 (6 пунктов): ЭКСПЛУАТАЦИОННЫЕ СВОЙСТВА.
    (Без союзов "и"). Формат: <ul>. Пунктуация: точка с запятой, в конце точка.
