...
# ==========================================
# TAB 5: LSI LIST GENERATOR (PRO B2B - FULL PROMPT)
# ==========================================
with tab_lsi_gen:
    st.header("📝 Генерация B2B текста (Full Hardcore Mode)")
    st.markdown("""
    Генерате жестко укажем нейросети: *«Вставь этот текст в тег `<h2>` без изменений»*. (Парсить URL ради одного заголовка в этой вкладке — излишнее усложнение, проще скопировать руками).
2.  **По поводу промта:** Возвращаю **полную версию** твоего промта со всеми техническими деталями (JS-цели, структура 1.1–1.11ция по строгому ТЗ: 4000 знаков, JS-цели, списки по 6 пунктов, стоп-слова < 1%.
    """)

    # --- ИНИЦИАЛИЗАЦИЯ СОСТОЯНИЯ ---
    if 'lsi_gen_result_html' not in st, стоп-слова, аудит).

Ниже **готовый код для Вкладки 5**. Замени всё содержимое `with tab_lsi_gen:` на этот вариант.

```python
# ==========================================
.session_state:
        st.session_state.lsi_gen_result_html = None
# TAB 5: LSI LIST GENERATOR (SINGLE MODE) - FULL PROMPT
# ==========================================    if 'lsi_gen_result_topic' not in st.session_state:
        st.
with tab_lsi_gen:
    st.header("📝 Генерация LSI-текста (session_state.lsi_gen_result_topic = ""
    if 'lsi_gen_resultHARDCORE B2B)")
    st.markdown("""
    Генерация текста (~4000 знаков) по_list' not in st.session_state:
        st.session_state.lsi_gen_result_list = []

    # 1. ВВОД ДАННЫХ
    with st.container твоему жесткому ТЗ:
    *   Внедрение сложного JS-кода для целей ((border=True):
        col_inp_1, col_inp_2 = st.columns(2Яндекс/Google).
    *   Структура 1.1 — 1.11.
    *   )
        ...
        with col_inp_1:
            target_topic = st.text_input(
                "Фильтр стоп-слов (<1%).
    *   Стиль Главред B2B.
1. Тема/Контекст (для понимания нейросетью)", 
                placeholder="Труба бес    """)

    # --- ИНИЦИАЛИЗАЦИЯ СОСТОЯНИЯ ---
    if 'шовная 50х3 ст20", 
                help="О чем писать статью?"
            )
lsi_gen_result_html' not in st.session_state:
        st.session_state.lsi_gen_result_html = None
    if 'lsi_gen_result_topic'            
            # НОВОЕ ПОЛЕ ДЛЯ H2
            target_h2_exact = st.text_ not in st.session_state:
        st.session_state.lsi_gen_result_topicinput(
                "2. Точный заголовок H2 (со страницы)", 
                placeholder="Т = ""
    if 'lsi_gen_result_list' not in st.session_state:
руба стальная бесшовная 50х3 мм ст.20 ГОСТ 8732        st.session_state.lsi_gen_result_list = []
    ...
    with st.container(border=True):
        ...
        with col_inp_2:
            raw_lsi_common = st, col_inp_2 = st.columns(2)
        
        with col_inp_1:
            target_category = st.text_input(
                "1. Заголовок H2 (В.text_area(
                "3. Список LSI", 
                height=150, 
                placeholder="купить\nцена\nдоставка\nв наличии\nоптом", 
                help="Сставь точь-в-точь как на сайте)", 
                placeholder="Труба стальная бесшовнаялова будут выделены жирным."
            )

        # API KEY
        cached_key = st.session_ 57х3.5 мм ст.20", 
                help="Скрипт использует этоstate.get('gemini_key_cache', "")
        if not cached_key:
            try: значение для тега <h2> без изменений."
            )
            
        with col_inp_2:
            raw_ cached_key = st.secrets["GEMINI_KEY"]
            except: pass
        lsi_apilsi_common = st.text_area(
                "2. Список LSI (через Enter или_key = st.text_input("Google Gemini API Key", value=cached_key, type="password", запятую)", 
                height=150, 
                placeholder="купить\nцена\n key="lsi_gen_api_key")

    # 2. ФУНКЦИЯ ГЕНЕРАЦИИ (ТВОЙ ПОЛНЫЙ ПРОМТ)
    def generate_lsi_ГОСТ 8732-78\nв наличии\nпроизводитель\nдоставка", 
                help="Эти слова будут внедрены в текст и выделены жирным."
            )

        # API KEY
article(api_key, context_topic, exact_h2, lsi_keywords):
        if not api_key        cached_key = st.session_state.get('gemini_key_cache', "")
        if: return "Error: No API Key"
        
        from openai import OpenAI
        client = OpenAI(api not cached_key:
            try: cached_key = st.secrets["GEMINI_KEY"]
            _key=api_key, base_url="https://litellm.tokengate.ru/v1except: pass
        lsi_api_key = st.text_input("Google Gemini API Key", value")
        
        lsi_string = ", ".join(lsi_keywords)
        
        #=cached_key, type="password", key="lsi_gen_api_key")

    #  HTML блок с JS (Экранируем фигурные скобки {{}} для f-string)
        contact_html2. ФУНКЦИЯ ГЕНЕРАЦИИ (ПОЛНЫЙ ПРОМТ)
    def generate__block = (
            'Предлагаем консультацию с менеджером по номеру '
            '<nobrlsi_article(api_key, topic, lsi_keywords):
        if not api_key:><a href="tel:#PHONE#" onclick="ym(document.querySelector(\'#ya_counter\').getAttribute return "Error: No API Key"
        
        from openai import OpenAI
        client = OpenAI(api_(\'data-counter\'),\'reachGoal\',\'tel\');gtag(\'event\', \'key=api_key, base_url="https://litellm.tokengate.ru/v1")Click po nomeru telefona\', {{\'event_category\' : \'Click\', \'event_label\'
        
        lsi_string = ", ".join(lsi_keywords)
        
        #  : \'po nomeru telefona\'}});gtag(\'event\', \'Lead_Goal\', {{\'event_category\' : \'Click\', \'event_label\' : \'Leads Goal\'}});" class="1. Блок стоп-слов
        stop_words_block = (
            "разные, применяют, применениеa_404 ct_phone">#PHONE#</a></nobr>, '
            'либо пишите, надежный, используют, использование, производство, "
            "производят, изготовление, изготовляют, на почту <a href="mailto:#EMAIL#" onclick="ym(document.querySelector(\'#ya_counter материал, качеств, помощи, сплав, вещество, "
            "получение, элемент, марка\').getAttribute(\'data-counter\'),\'reachGoal\',\'email\');gtag(\'event, получают, получать, изделие, фабрикат, содержание, "
            "содержат, состав, конструкции, может, можно, могут, выпуск, качественный, "
            "производитель, эффективный, кроме,\', \'Click napisat nam\', {{\'event_category\' : \'Click\', \'event_label\' : \'napisat nam\'}});gtag(\'event\', \'Lead_Goal\', {{\' рынок, рынке, решения"
        )

        # 2. Блок HTML с JS (Экранируем фигуevent_category\' : \'Click\', \'event_label\' : \'Leads Goal\'}});" class="рные скобки для Python f-string: {{ }})
        # Внутри f-строки одинарная { превращается вa_404">#EMAIL#</a>.'
        )

        stop_words_list = (
 {{
        js_code_snippet = (
            'Предлагаем консультацию с менеджером по номеру            "разные, применяют, применение, надежный, используют, использование, производство, "
            "произ '
            '<nobr><a href="tel:#PHONE#" onclick="ym(document.querySelector(\'#ya_counter\').getAttribute(\'data-counter\'),\'reachGoal\',\'tel\');gtag(\'event\', \'Click po nomeru telefona\', {{\'event_category\' : \'Click\', \'event_label\' : \'po nomeru telefona\'}});gtag(\'event\', \'Lead_Goal\', {{\'event_category\' : \'Click\', \'event_label\' : \'Leadsводят, изготовление, изготовляют, материал, качеств, помощи, сплав, вещество, "
            "получение, элемент, марка, получают, получать, изделие, фабрикат, содержание, "
            "содержат, состав, конструкции, может, можно, могут, выпуск, качественный, "
        ...
        # === СИСТЕМНЫЙ ПРОМТ ===
        system_instruction = (
            "Ты — профессиональный технический Goal\'}});" class="a_404 ct_phone">#PHONE#</a></nobr>, '
            'либо пишите на почту <a href="mailto:#EMAIL#" onclick="ym(document.querySelector копирайтер B2B. "
            "Ты строго следуешь ТЗ, структуре и ограничениям по лексике. "
            "Ты выдаешь ТОЛЬКО HTML-код."
        )
        
        #(\'#ya_counter\').getAttribute(\'data-counter\'),\'reachGoal\',\'email\ === ПОЛЬЗОВАТЕЛЬСКИЙ ПРОМТ (ТВОЙ ОРИГИНАЛ) ===
');gtag(\'event\', \'Click napisat nam\', {{\'event_category\' : \'Click        user_prompt = f"""
        ЗАДАЧА: Напиши текст для товара/категории:\', \'event_label\' : \'napisat nam\'}});gtag(\'event\', \'Lead_Goal\', {{\'event_category\' : \'Click\', \'event_label\' : \'Leads "{context_topic}".
        
        [I] ЭТАП 1: ТРЕБОВАНИЯ К Goal\'}});" class="a_404">#EMAIL#</a>.'
        )

        # === НАПИСАНИЮ ТЕКСТА
        1. Структура текста:
        Строго СИСТЕМНЫЙ ПРОМТ ===
        system_instruction = (
            "Ты — профессиональный придерживайся следующей структуры:
        
        1.1. Тема: Выведи строго: технический копирайтер и аудитор. "
            "Твоя задача — написать текст и провести его ау <h2>{exact_h2}</h2> (Используй этот текст дословно, без изменений).
        
дит (самопроверку) перед выдачей. "
            "Ты выдаешь ТОЛЬКО чистый HTML-        1.2. Коммерческий бэнгер/хук: Яркое, цепляющее первое предложение.
        
        1.3. Информационный абзац 1: 2-3 информакод без markdown."
        )
        
        # === ПОЛЬЗОВАТЕЛЬСКИЙ ПРОМТ (Тционные предложения, раскрывающие суть продукта, его определение и ключевое назначение.
        ВАЖНО: АбзаВОЙ ПОЛНЫЙ ВАРИАНТ) ===
        user_prompt = f"""
        ЗАц 1 ОБЯЗАТЕЛЬНО должен содержать точную фразу с кодом:
        {contact_htmlДАЧА: Напиши текст для категории: "{topic}".
        Объем текста: до 40_block}
        
        1.4. Обобщающее предложение (подводка к первому00 символов.

        [I] LSI ИНСТРУКЦИЯ:
        Рав списку).
        
        1.5. Список №1 (6 пунктов): Раскрывает ключевые характеристики, разновидности или преимущества. 
           - Все 6 пунктов сделай примерно одинаковой средней длиныномерно распредели слова: {{{lsi_string}}}.
        Вписывай их естественно.
        ВАЖНО предложениями.
           - Оформление <ul><li>...</li></ul>.
           
        1.6.: Выдели внедренные LSI слова тегом <b>жирно</b>.

        [II] ЭТАП  Информационный абзац 2: 2-3 информационные предложения, описывающие технологию производства, сферы1: ТРЕБОВАНИЯ К НАПИСАНИЮ ТЕКСТА (СТРУКТУРА):
        Строго придерживайся следующей структуры:

        1.1. Тема: применения или соответствие стандартам (ГОСТ).
        
        1.7. Подзаголовок (При Выведи строго <h2>{topic}</h2> (Используй это значение без изменений).
        
        1.думай релевантный H3, например "Технические особенности").
        
        1.8. Информационный аб2. Коммерческий бэнгер/хук: Яркое, цепляющее первое предложение.
зац 3: 2-3 информационные предложения.
        
        1.9. Обоб        
        1.3. Информационный абзац 1: 2-3 информационные предложения,щающее предложение (подводка ко второму списку).
        
        1.10. Список №2 (6 пунктов): Детализирует физико-механические свойства. 
           - Все 6 раскрывающие суть продукта, его определение и ключевое назначение.
        ВАЖНО: Этот абзац ОБ пунктов сделай примерно одинаковой средней длины предложениями.
           - Оформление <ul><li>...</li></ul>.
ЯЗАТЕЛЬНО должен содержать точную фразу с кодом:
        {js_code_snippet}
           
        1.11. Информационный абзац 4: 2-3 информационные предложения        
        1.4. Обобщающее предложение: Подводка к первому списку.
 с коммерческим призывом к действию.

        [II] ОГРАНИЧЕНИЯ И L        
        1.5. Список №1 (6 пунктов): Раскрывает ключевые характеристики, разновидSI:
        1. Объем текста должен составлять до 4000 символов.
        2ности или преимущества.
           - Все 6 пунктов сделай примерно одинаковой средней длины предложениями.
           . LSI ИНСТРУКЦИЯ: Равномерно распредели слова: {{{lsi_string}}}.- Оформление: <ul><li>...</li></ul>.
           - Назывные пункты закрываются точкой с запятой (кроме последнего).
           - Полноценные предложения — точкой. Последний пункт Выдели внедренные слова жирным (<b>слово</b>).
        3. СТОП-СЛ всегда с точкой.
        
        1.6. Информационный абзац 2: 2ОВА: Частота использования следующих слов должна быть менее 1%:
        ({stop_words_list})

        -3 информационные предложения (технология производства, сферы применения или соответствие стандартам ГОСТ).
        
        1[III] ОФОРМЛЕНИЕ И СТИЛЬ:
        - Списки: Назывные пункты закрываются точкой с запятой (кроме последнего), полноценные предложения — точкой. Последний пункт всегда.7. Подзаголовок: Используй <h3>Технические особенности</h3> (или релевантный).
        
         с точкой.
        - Единицы измерения: Используй стандартные сокращения системы СИ (мм,1.8. Информационный абзац 3: 2-3 информационные предложения.
        
        1.9. Обобщающее предложение: Подводка ко второму списку.
        
 м, Н, В, °C).
        - Стиль: Деловой, экспертный, но "челове        1.10. Список №2 (6 пунктов): Детализирует физико-механические свойствачный".
        - Факты: Все суждения подкрепляй измеримыми фактами, цифрами, ссы.
           - Все 6 пунктов сделай примерно одинаковой средней длины.
           - Оформление: лками на ГОСТы.
        - Запрещено: Вопросительные и восклицательные знаки, вводные<ul><li>...</li></ul>.
        
        1.11. Информационный абзац 4: слова, оценочные прилагательные в превосходной степени.
        - Переменные: Оставь переменные 2-3 информационные предложения с коммерческим призывом к действию.

        [III] ОГРАНИЧ #PHONE# и #EMAIL# как есть в коде.

        Этап 2: Комплексный аудит.ЕНИЯ И СТОП-СЛОВА:
        Частота использования следующих слов должна быть НЕ БОЛЕЕ (Выполняется мысленно перед выдачей). Выдай сразу финальный, вычитанный HTML код.
         1% (лучше 0%, заменяй синонимами):
        {stop_words_block}

        [IV] СОДЕРЖАНИЕ И СТИЛЬ:
        - Стиль: Деловой, экспертный, но "человечный". Избегай канцеляризмов.
        - Факты: Подкрепляй суждения цифрами, ГОСТами, марками стали.
        ...
        try:
            ...
            content = re.sub(r: Говори от лица производителя. Вместо "проверенный поставщик" используй формулировки про собственное'^```', '', content.strip())
            content = re.sub(r'```$', '', content.strip производство.
        - Формула Главреда B2B: Ответь на вопросы: что это?())
            return content
        except Exception as e:
            return f"Error: {str(e)}" какую проблему решает? кому подойдет?
        - Запрещено: Вопросительные/восклицательные

    # 3. КНОПКА ЗАПУСКА
    if st.button("🚀 Сгене знаки, вводные слова ("важно", "примечание"), оценочные прилагательные ("высочайшее"),рировать по ТЗ", type="primary", key="btn_run_lsi_gen"):
        if not target_ исторические справки.
        - Единицы измерения: СИ (мм, м, Н, В, °topic.strip() or not target_h2_exact.strip():
            st.error("❌ ЗаполC).

        ЭТАП 2: КОМПЛЕКСНЫЙ АУДИТ.
        Перед выдачейните Тему и Точный H2!")
        elif not lsi_api_key:
            st.error("❌ Введите API ключ!")
        else:
            # Подготовка
            lsi_list = ответа проверь себя:
        1. Вставлен ли JS-код в абзац 1.3?
        2. Есть ли 2 списка по 6 пунктов?
        3. Нет ли стоп-слов [x.strip() for x in re.split(r'[,\n]+', raw_lsi_common) if x.strip()]
            
            with st.spinner(f"⏳ Генерация (Может занять ?
        Если все ок — выдавай HTML код.
        """
        
        try:
            response = client20-30 сек)..."):
                article_html = generate_lsi_article(lsi_api_key.chat.completions.create(
                model="google/gemini-2.5-pro",, target_topic, target_h2_exact, lsi_list)
            
            if article_html.
                messages=[
                    {"role": "system", "content": system_instruction},
                    {"role":startswith("Error"):
                st.error(f"Сбой API: {article_html}")
            else "user", "content": user_prompt}
                ],
                temperature=0.3
            )
:
                st.success("✅ Готово!")
                st.session_state.lsi_gen_            content = response.choices[0].message.content
            content = re.sub(r'^```html', '', contentresult_html = article_html
                st.session_state.lsi_gen_result_topic =.strip())
            content = re.sub(r'^```', '', content.strip())
            content = target_h2_exact # Сохраняем H2 как тему результата
                st.session_state. re.sub(r'```$', '', content.strip())
            return content
        except Exception as e:lsi_gen_result_list = lsi_list
    ...
    if st.session_state.lsi_gen_result_html:
        ...
        stСКА
    if st.button("🚀 Сгенерировать текст (Pro)", type="primary", key="btn_.markdown("---")
        st.subheader(f"Результат для: {st.session_state.run_lsi_gen"):
        if not target_category.strip():
            st.error("❌ Введите H2 заголовок!")
        elif not lsi_api_key:
            st.error("lsi_gen_result_topic}")
        
        res_html = st.session_state.lsi_gen_result_html
        
        st.text_area("HTML Код", value=res❌ Введите API ключ!")
        else:
            # Подготовка
            lsi_list = [x_html, height=400)
        
        with st.expander("👀 Визуальный предпросмотр", expanded=True):
            preview_html = res_html.replace("#PHONE#", "+.strip() for x in re.split(r'[,\n]+', raw_lsi_common) if x.strip()]
            
            with st.spinner(f"⏳ Генерация по структуре 1.1–7 (XXX) ...").replace("#EMAIL#", "mail@...")
            st.markdown(preview_html, unsafe_1.11 для: {target_category}..."):
                # Генерация
                article_html = generate_allow_html=True)
        
        # Excel
        result_data = [{
            "H2 (Заголовок)": st.session_state.lsi_gen_result_topic,
            "HTML Текстlsi_article(lsi_api_key, target_category, lsi_list)
            
": res_html,
            "LSI": ", ".join(st.session_state.lsi_            # Проверка ошибок
            if article_html.startswith("Error"):
                st.error(f"gen_result_list),
            "Длина": len(res_html)
        }]
        dfСбой API: {article_html}")
            else:
                st.success("✅ Готово!")
_one = pd.DataFrame(result_data)
        buffer_one = io.BytesIO()
                        # === СОХРАНЯЕМ В SESSION_STATE ===
                st.session_state.lsi_gen_result_html = article_html
                st.session_state.lsi_gen_result_with pd.ExcelWriter(buffer_one, engine='xlsxwriter') as writer:
            df_one.topic = target_category
                st.session_state.lsi_gen_result_list = lsito_excel(writer, index=False)
    ...
    📥 Скачать в Excel",
            ...
            file_name=f"if st.session_state.lsi_gen_result_html:
        
        st.markdown("Full_Text_{int(time.time())}.xlsx",
            mime="application/vnd.ms-excel",
            type="primary"
        )
