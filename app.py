...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # Математика для среднего балла >= 4.75
    def get_balanced_ratings(n):
    ...
    # Даты строго с 2020 года по сегодняшний день
    ...
    for i in range(target_count):
        ...
        authors_listing.append(f"Автор: {chosen_authors[i]['name']} | Пол: {g_label} | Оценка: {ratings[i]} | Тип: {chosen_authors[i]['type']}")
    ...
    try:
        ...
        prompt = f"""Ты — Senior Копирайтер и эксперт по поведенческой психологии. Твоя задача — сгенерировать {target_count} максимально реалистичных, «живых» отзывов (B2B и B2C) для страницы/раздела: "{h2_header}". 

ДАННЫЕ ДЛЯ ГЕНЕРАЦИИ:
Авторы (Имя | Пол | Оценка | Тип клиента):
{nl.join(authors_listing)}

Список LSI-слов для использования:
{", ".join(lsi_words[:20])}

Сгенерированные даты (распредели их по отзывам по порядку):
{", ".join(raw_dates)}

ПРАВИЛА И ОГРАНИЧЕНИЯ (КРИТИЧЕСКИ ВАЖНО СОБЛЮДАТЬ КАЖДЫЙ ПУНКТ):

1. ФИЛЬТРАЦИЯ И ВСТРАИВАНИЕ LSI:
- В первую очередь мысленно отфильтруй список LSI: полностью исключи из него любые названия городов.
- Запрещено использовать английский язык и любые бренды (исключение: общепринятые маркировки стали/материалов и стандарты типа ГОСТ).
- Оставшиеся LSI-слова органично вписывай в тексты по смыслу, правильно склоняя. 
- Каждое использованное LSI-слово СТРОГО оборачивай в HTML-теги <b>...</b> (пример: "заказали <b>стальные листы</b> на объект"). Следи за закрытием тега!

2. ЛОГИКА ОЦЕНОК И СЮЖЕТОВ:
- Для оценок 5.0: пиши разнообразные отзывы. Не своди всё к доставке! Описывай качество, помощь в подборе, решение нестандартных задач, сервис. Сроки должны быть реалистичными для габаритных грузов (дни, недели, а не "привезли через час").
- Для оценок 3.5, 4.0, 4.5 (СНИЖЕННАЯ ОЦЕНКА): КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО ругать нашу компанию, наш товар, нашу логистику или склад. 
- Причина снижения оценки должна быть ИСКЛЮЧИТЕЛЬНО во внешнем форс-мажоре: задержал оплату банк клиента, подвел подрядчик на объекте, сломался кран при разгрузке, вмешалась плохая погода, задержки из-за транспортной компании, которую нанял сам клиент.
- В таких "проблемных" отзывах ОБЯЗАТЕЛЬНО пиши, что НАШИ менеджеры были всё время на связи, проявили инициативу и помогли спасти ситуацию, несмотря на вину третьих лиц.

3. УНИКАЛЬНОСТЬ СТИЛЯ И РАЗНООБРАЗИЕ ОШИБОК (КРИТИЧНО):
- Тексты должны выглядеть так, будто их писали абсолютно РАЗНЫЕ люди разного возраста, темперамента и уровня образования. Никакой шаблонности нейросети!
- Ошибки (у 30-40% авторов) НЕ ДОЛЖНЫ повторяться по одному паттерну из отзыва в отзыв. 
- Чередуй форматы: один пишет идеально грамотно и развернуто; второй — коротко, сухо и по делу; третий пишет всё только строчными буквами; четвертый пропускает пробелы после запятых; пятый делает 1-2 случайные опечатки по клавиатуре ЙЦУКЕН ("ш" вместо "щ"). 
- Если ты применил один тип опечатки или стиля к одному автору — НЕ используй этот же паттерн для следующего. Текст с опечатками должен оставаться читабельным.
- ЗАПРЕЩЕНЫ любые приветствия ("Всем привет", "Добрый день").
- Смайлы: разрешены только 👍 и 👌 (используй редко).
- Тире: используй только короткое "-" (дефис) без пробелов до/после при опечатках, либо стандартно.

ФОРМАТ ВЫВОДА:
Верни строго валидный массив JSON, состоящий из {target_count} объектов. Не используй Markdown-оформление (```json). Ключи должны быть строго на русском языке:

[
  {{
    "Имя": "Иван",
    "Текст": "здесь текст отзыва со встроенными <b>LSI</b>...",
    "Оценка": 4.5,
    "Дата": "05.02.2026"
  }}
]"""
