...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    try:
        ...
        prompt = f"""Задача: Написать {target_count} КАРДИНАЛЬНО РАЗНЫХ B2B/B2C отзывов о компании-поставщике (товар: "{h2_header}").

СПИСОК АВТОРОВ (Имя, Пол, Оценка):
{nl.join(authors_listing)}

ПРАВИЛО 1: ЕСТЕСТВЕННОЕ ВНЕДРЕНИЕ LSI (КРИТИЧНО!)
- Список LSI: {", ".join(lsi_words[:20])}
- КАЖДОЕ LSI-СЛОВО ИСПОЛЬЗУЙ СТРОГО 1 РАЗ НА ВЕСЬ МАССИВ ОТЗЫВОВ!
- АНТИ-РОБОТ: Запрещены топорные SEO-вставки. НЕ пиши "материал - сталь", пиши "искали болты из прочной стали". Исключи фразы типа "с кругом болтов", используй слова по их прямому техническому смыслу (например, стальной круг).
- ГРАММАТИКА: Обязательно склоняй LSI-слова (падеж, число) и МЕНЯЙ ВИД ГЛАГОЛОВ (совершенный/несовершенный), чтобы текст звучал идеально по-русски. Не "звонил уточнять", а "позвонил уточнить". 
- Оборачивай использованные LSI в теги <b>...</b>. ** не используй.
- Если LSI-слов меньше, чем отзывов — пиши часть отзывов вообще без LSI.

ПРАВИЛО 2: БОРЬБА С ШАБЛОННОСТЬЮ И ИСТОРИЯМИ
- ЗАБУДЬ про структуру: "Обратились -> Купили -> Доставили -> Рекомендую".
- ИСКЛЮЧИ постоянные повторения слов: "менеджер", "качество", "рекомендую", "объект", "партия", "доставка".

ПРАВИЛО 3: РАЗНОЕ ВИЗУАЛЬНОЕ ОФОРМЛЕНИЕ И СТИЛЬ (КРИТИЧНО!)
Визуально все отзывы должны выглядеть по-разному. ЧЕРЕДУЙ СЛЕДУЮЩИЕ 4 ФОРМАТА хаотично между авторами:
1. "Суровый прораб": 2-5 слов максимум. Рубленые фразы. (Пример: "Норм металл. Взяли в цех, работаем.")
2. "С телефона на бегу": НИ ОДНОЙ заглавной буквы (все строчными). Пропуск пробелов после запятых(один,два,три). Возможна пара опечаток на соседние буквы (ш/щ, а/в).
3. "Возрастной клиент": Используй многоточия... Много скобочек ))) Обилие восклицательных знаков!!!
4. "Корпоративный заказчик": Сухо, по делу, идеальная пунктуация, может быть чуть длиннее обычного, без эмоций.

ПРАВИЛО 4: ОЦЕНКИ 3.5 И 4.0
- ЗАПРЕЩЕНО ругать саму компанию, наш склад, качество товара.
- Причина снижения: ТОЛЬКО внешний форс-мажор (пробки, погода, задержал банк, тупил водитель сторонней ТК).
- Обязательно напиши, что НАШИ менеджеры спасли ситуацию.

ПРАВИЛО 5: ОГРАНИЧЕНИЯ И ЗАПРЕТЫ
- ЗАПРЕТ НА ГОРОДА: Если в LSI есть город (Москва, СПБ и т.д.) — ПРОИГНОРИРУЙ ЕГО.
- Сроки доставки должны быть реалистичными (2-3 дня, неделя). Фразы "привезли за 2 часа" — под запретом.
- Никаких приветствий ("Всем привет").
- Смайлы: ТОЛЬКО 👍 и 👌 (очень редко).
- Тире: Только короткий дефис "-".

ВЕРНИ СТРОГО JSON МАССИВ (Ключи на русском):
...
"""
        ...
        if raw_content is None:
            if hasattr(resp.choices[0].message, 'reasoning_content') and resp.choices[0].message.reasoning_content:
                ...
                raise ValueError("API вернуло пустой ответ (None). Сервер перегружен или сбросил соединение.")
        ...
        return [{"Имя": "Ошибка генерации", "Текст": f"Сбой API или парсинга: {str(e)}", "Оценка": 5.0, "Дата": date.today().strftime("%d.%m.%Y")}]
