...
with tab_seo_main:
    ...
    with col_main:
        ...
        with top_col2:
# --- НАЧАЛО ЗАМЕНЫ (Строки 1444-1450) ---
        with st.container(border=True):
            st.markdown("#### 🎯 Ввод данных")
            
            # Первый ряд: Выбор источника и Поисковый запрос
            ...
            # Второй ряд: Динамическое поле (Ссылка или Текст)
            if my_input_type == "Релевантная страница на вашем сайте":
                st.text_input("🔗 URL страницы", placeholder="https://site.ru/catalog/tovar", key="my_url_input")
            elif my_input_type == "Исходный код страницы или текст":
                st.text_area("💻 Исходный код или текст", height=150, placeholder="Вставьте HTML", key="my_content_input")
        # --- КОНЕЦ ЗАМЕНЫ ---
