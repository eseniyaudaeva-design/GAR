...
def generate_ai_content_blocks(client, base_text, tag_name, num_blocks=5, seo_words=None):
    ...
    # 1. Формируем ГЛОБАЛЬНУЮ инструкцию по SEO
    ...
    for attempt in range(3):
        try:
            ...
            # Дополняем пустыми строками до 5, чтобы не ломать Excel структуру
            while len(blocks) < 5: 
                blocks.append("")
                
            return blocks[:5] # Возвращаем всегда 5 элементов (лишние будут пустыми)
        except Exception as e:
