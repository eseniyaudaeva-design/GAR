...
ГЛАВНОЕ: Отзывы должны выглядеть так, будто их писали уставшие люди с телефона, в спешке, на ходу. Хватит писать "правильно". Мне нужны ГРУБЫЕ, ЗАМЕТНЫЕ СРАЗУ ошибки в 70% отзывов. Забудь про литературный русский язык!

ПРИМЕРЫ (КАК НАДО И КАК НЕЛЬЗЯ):
❌ ПЛОХО (ИИ-стиль, так НЕ писать!): "Заказывал стальные трубы. Привезли вовремя, качество отличное. Спасибо менеджерам за работу, рекомендую!"
...
✅ ХОРОШО (Душнила, но с ошибкой): "Долго искали поставщика. В целом норм, но водила перепутал поворот пришлось выходить встречать..."

ПРАВИЛО LSI-СЛОВ (КРИТИЧНО!):
Список слов: {", ".join(lsi_words[:20])}. 
СТРОГОЕ УСЛОВИЕ: Каждое слово из этого списка можно использовать МАКСИМУМ 1 РАЗ на всю выборку из {target_count} отзывов! 
Если ты вставил слово в один отзыв — вычеркивай его из своей памяти, в другие отзывы его писать ЗАПРЕЩЕНО. Распредели их по разным отзывам. 
ОБЯЗАТЕЛЬНО выделяй использованные LSI-слова тегами <b>...</b> (например, <b>стальная труба</b>). ** не используй!
