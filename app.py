...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # --- 3. РАНДОМНЫЕ ДАТЫ (С 1 ЯНВАРЯ 2020) ---
    ...
    # --- 5. БАЗОВЫЕ СЮЖЕТЫ (ТОЛЬКО БЫТ И СТРОЙКА) ---
    plot_types = [
        "Стройка/Дача: Упомяни конкретную бытовую постройку (забор, крыша, сарай, слив, теплица), для которой понадобился этот материал. Не описывай сам товар, опиши, КУДА ты его применил.",
        "Экономия: Искал, где дешевле. Напиши, что везде цены космос, а тут вышло терпимо по деньгам, плюс доставка не подвела.",
        "Помощь: Не особо разбирался в размерах/количестве. Ребята (или менеджер) по телефону помогли посчитать, привезли, всё подошло."
    ]
    ...
    for i in range(target_count):
        ...
        gender_instruction = "Строго ЖЕНСКИЙ пол (глаголы: заказала, привезла, брала)" if assigned_gender == 'Женский' else "Строго МУЖСКОЙ пол (глаголы: заказал, привез, брал)"

        # Сборка сюжета
        base_plot = random.choice(plot_types)
        plot_instruction = f"СЮЖЕТ: {base_plot} Контекст товара '{h2_header}'. Подумай, как именно его используют в частном доме. Пиши от лица простого человека."
        
        if ratings[i] == 5.0:
             plot_instruction += " Оценка 5.0. Всё прошло гладко, без проблем."
        elif ratings[i] == 3.5:
             plot_instruction += " Оценка 3.5. Обязательно добавь бытовой минус сервиса (например, хрен найдешь въезд на базу, кладовщик долго искал заказ, привезли на пару часов позже)."
        else:
        ...
        if i == perfect_index:
            style_inst += "\n- ОШИБКА: Текст без орфографических ошибок. Начни с заглавной буквы и поставь точку в конце."
        else:
            if random.random() <= 0.05:
                style_inst += "\n- ОШИБКА: Допусти 1-2 опечатки (например, 'вообщем', 'как-то' без дефиса, 'ш' вместо 'щ')."
            else:
                style_inst += "\n- ОШИБКА: Без орфографических ошибок."
            
            punctuation_style = random.choice([
                "Идеальная пунктуация внутри предложений.",
                "Запятые есть, но их мало (некоторые пропущены).",
                "Вместо некоторых запятых ставит многоточие '...'"
            ])
            ...
            if random.random() <= 0.80:
                style_inst += "\n- ФОРМАТ КОНЦА: СТРОГО НЕ ставь точку или любой знак в самом конце текста."
            else:
                style_inst += "\n- ФОРМАТ КОНЦА: Поставь точку в конце отзыва."
                
            if random.random() <= 0.20:
                style_inst += "\n- СЛЕНГ: Используй разговорные словечки: норм, збс, четко, спс, пойдет, нормас."
        ...
        if i == ultra_short_index:
            ...
            length_inst = random.choice([
                "Структура: 1-2 коротких предложения.",
    ...
    try:
        ...
        prompt = f"""Ты — обычный покупатель из России, который пишет отзыв со смартфона. Текущий товар: "{h2_header}".
Твоя задача — написать максимально бытовые, разговорные отзывы для частного использования (дача, ремонт, свой дом).

КАК ПИСАТЬ (ЭТО КРИТИЧЕСКИ ВАЖНО):
1. ПИШИ ПРО РЕЗУЛЬТАТ, А НЕ ПРО ТОВАР: Люди не пишут "у трубы хорошая толщина стенки". Они пишут "варится нормально, не фольга", "взял на столбы для забора", "встало как родное", "перекрыл крышу". 
2. ЖЕСТКИЙ СТОП-ЛИСТ СЛОВ (они выдают ИИ): качественный, соответствует заявленному, рекомендую к покупке, отличный выбор, недостатки отсутствуют, приобрел, данный товар, идеальный. ЗАБУДЬ ЭТИ СЛОВА.
3. РАЗРЕШЕННЫЙ СЛОВАРЬ (используй это): брал, взял, норм, пойдет, ценник, мужики на складе, ребята привезли, нормально.
4. СИНТАКСИС: Никаких деепричастных оборотов ("учитывая вышесказанное", "заказывая здесь"). Пиши просто и отрывисто.
