...
def generate_ai_content_blocks(client, base_text, tag_name, forced_header, num_blocks=5, seo_words=None):
    ...
    # 3. Пользовательский промт
    # Мы используем твои переменные tag_name, base_text и vocab_strs (индексы 0-4)
    user_prompt = f"""
    ИСХОДНЫЕ ДАННЫЕ:
    ...
    Тебе нужно внедрить конкретные слова в конкретные блоки. Используй их в любой подходящей под контекст лемме.
    
    РАСПРЕДЕЛЕНИЕ СЛОВ ПО БЛОКАМ (СТРОГО):
    Блок 1: {vocab_strs[0]}
    Блок 2: {vocab_strs[1]}
    Блок 3: {vocab_strs[2]}
    Блок 4: {vocab_strs[3]}
    ...
    1. КОЛИЧЕСТВО: Употреби каждое слово из списка для конкретного блока ровно 1 раз.
    ...
    4. ЕСТЕСТВЕННОСТЬ: Меняй словоформы (падежи, числа, части речи) под контекст. Текст должен быть естественным и логичным, не пиши чушь.
    -------------------------------------------
    ...
    - Внедрить слова соответственно для каждого блока (из списка выше).
