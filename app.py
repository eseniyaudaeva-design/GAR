...
# --- БЛОК СТОП-СЛОВ (КОМПАКТНЫЙ И УЗКИЙ) ---
                st.markdown("---")
                st.markdown("<h5 style='margin-bottom:5px;'>🛑 Стоп-лист (исключения)</h5>", unsafe_allow_html=True)
                
                # Создаем 3 колонки: 
                # 1. Поле ввода (ограниченная ширина)
                # 2. Кнопка
                # 3. Читаемая подсказка
                col_inp, col_btn, col_hint = st.columns([2, 0.8, 1.5])
                ...
                with col_hint:
                    # Крупная и читаемая подсказка сбоку
                    st.markdown(
                        """
                        <div style='font-size: 14px; color: #334155; line-height: 1.4; border-left: 3px solid #277EFF; padding-left: 15px; margin-top: 5px;'>
                            <b>Инструкция:</b> Удалите слово из списка слева и нажмите кнопку, 
                            чтобы оно снова появилось в группах семантики выше.
                        </div>
                        """, 
                        unsafe_allow_html=True
                    )
