...
# --- ФУНКЦИЯ КЛАССИФИКАЦИИ С API ЯНДЕКСА (ИСПРАВЛЕННАЯ v2.4) ---
def classify_semantics_with_api(words_list, yandex_key):
    ...
    # Слова, относящиеся к интерфейсу, соглашениям и навигации
    SITE_UI_GARBAGE = {
        ...
        'соглашаться', 'согласие', 'принимать', 'отправить', # <-- Добавлено
    ...
    # Коммерческие слова и Маркетинговые прилагательные
    COMMERCIAL_WORDS = {
        ...
        # Прилагательные-паразиты (чтобы не попадали в товары)
        'выгодный', 'низкий', 'высокий', 'лучший', 'качественный', 'надежный',
        'большой', 'малый', 'удобный', 'быстрый', 'бесплатный'
    }
    ...
    for word in words_list:
        ...
        # Лемматизация Pymorphy (предварительная)
        if morph:
        ...
        # Проверка услуг (включая окончания на -обработка)
    ...
    # 4. РАСПРЕДЕЛЕНИЕ ПО РЕЗУЛЬТАТАМ API
    for word in api_candidates:
        info = yandex_results.get(word, {'lemma': word, 'pos': 'unknown'})
        lemma = info['lemma']
        pos = info['pos']
        
        # Повторная проверка списков (на случай если лемма от Яндекса отличается)
        if lemma in SITE_UI_GARBAGE or lemma in COMMERCIAL_WORDS:
            categories['commercial'].add(lemma)
            continue
        ...
        # Логика УСЛУГ
        is_service = False
        if lemma.endswith('ние') or lemma.endswith('ение'): # Сверление, Хранение
            is_service = True
        elif lemma.endswith('обработка'): # Металлообработка
            is_service = True
        ...
        # Логика ТОВАРОВ
        # В товары берем только Существительные (noun).
        # Прилагательные (adj) отправляем в коммерцию/общее, так как это обычно свойства (профильный, сварной, выгодный).
        
        if pos == 'noun':
            if len(lemma) > 2:
                categories['products'].add(lemma)
        elif pos == 'unknown':
            # Если не знаем часть речи, считаем товаром только если не похоже на прилагательное
            # (очень грубая эвристика, но лучше чем ничего)
            if not (lemma.endswith('ый') or lemma.endswith('ий') or lemma.endswith('ая') or lemma.endswith('ое')):
                if len(lemma) > 2:
                    categories['products'].add(lemma)
            else:
                categories['commercial'].add(lemma)
        else:
            # Все остальные части речи (глаголы, прилагательные, наречия) -> в общее/коммерцию
            categories['commercial'].add(lemma)
