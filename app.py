...
# ==========================================
# TAB 5: BULK LSI GENERATOR (ELITE PROMPT + GRAMMAR FIX)
# ==========================================
import requests
...
with tab_lsi_gen:
    st.header("🏭 Массовая генерация B2B (Elite Prompt + Grammar)")
    st.markdown("Тот самый креативный промт + **Исправленные H3 (строчные)** + **LSI в начале**.")
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        system_instruction = (
            "Ты — элитный B2B копирайтер. "
            "Ты ненавидишь скучные вступления. Ты пишешь дерзко, по факту и с цифрами. "
            "Ты выдаешь ТОЛЬКО HTML-код."
        )
        ...
        [I] ВАЖНЫЕ ПРАВИЛА ОФОРМЛЕНИЯ:
        1. ЗАГОЛОВКИ H3: Если ты используешь название товара "{exact_h2}" внутри заголовка H3 (в середине фразы), 
           ТЫ ОБЯЗАН НАПИСАТЬ ЕГО СО СТРОЧНОЙ (МАЛЕНЬКОЙ) БУКВЫ.
           Плохо: <h3>Технические характеристики Трубы стальной</h3>
           Хорошо: <h3>Технические характеристики трубы стальной</h3>
           
        2. LSI КЛЮЧИ: Постарайся 70-80% слов из списка LSI употребить в первой половине текста (до раздела Применение).
        
        [II] СТРУКТУРА ТЕКСТА:
        
        1.1. Заголовок: Выведи строго: <h2>{exact_h2}</h2>.
        
        1.2. КОММЕРЧЕСКИЙ ХУК (БЭНГЕР):
        Это 2-3 коротких, ударных предложения в самом начале.
        СТРОЖАЙШИЙ ЗАПРЕТ: Не начинай со слов "Предлагаем", "Реализуем", "Продаем".
        Пример стиля: "Фундамент, который переживет здание. Арматура с запасом прочности 30%."
        
        1.3. Информационный абзац 1 + Контакты: 
        Кратко суть. И сразу вставь этот код:
        {contact_html_block}
        
        1.4. Подводка к списку 1 (заканчивается двоеточием :).
        
        1.5. Список №1 (6 пунктов): ТЕХНИЧЕСКИЕ ХАРАКТЕРИСТИКИ.
        - Пиши КОНКРЕТНЫЕ ЦИФРЫ (диапазоны, марки, ГОСТы).
        - Формат: "Параметр: Значение".
           
        1.6. Информационный абзац 2 (Производство/Стандарты).

        1.7. Подзаголовок H3:
        Придумай заголовок про Особенности или ГОСТ. 
        Включи название товара "{exact_h2}". Помни про правило строчной буквы!
        
        1.8. Информационный абзац 3.
        
        1.9. Подводка к списку 2 (заканчивается двоеточием :).
        
        1.10. Список №2 (6 пунктов): ПРИМЕНЕНИЕ.
        - Не просто "Строительство", а конкретно "Армирование несущих стен".
           
        1.11. Информационный абзац 4.
                              
        1.12. Подзаголовок H3:
        Придумай заголовок (например про Сортамент или Виды). Помни про строчную букву для товара.
        
        1.13. Информационный абзац 5.
        
        1.14. Подводка к списку 3 (заканчивается двоеточием :).
        
        1.15. Список №3 (6 пунктов): УСЛОВИЯ ПОСТАВКИ. 
        (Резка, упаковка, сертификаты).
           
        1.16. Финальный призыв (коротко).

        [III] ОГРАНИЧЕНИЯ:
        1. LSI: Внедри слова: {{{lsi_string}}}. Выдели <b>жирным</b>.
    ...
    with st.expander("⚙️ Настройки и LSI", expanded=True):
        ...
        with c1:
            lsi_api_key = st.text_input("Gemini API Key", value=cached_key, type="password", key="bulk_api_key_v9")
        with c2:
