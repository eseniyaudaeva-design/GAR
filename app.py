...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # --- 4. РАСПРЕДЕЛЕНИЕ СТИЛЕЙ (для программной постобработки Питоном) ---
    ...
    male_personas = [
        "Мужчина, делает забор, навес или мелкий ремонт на даче своими руками",
        "Прораб небольшой бригады, строит частный дом или баню",
        "Частный мастер (сварщик/строитель), делает заказы для частников",
        "Владелец участка, закупает материалы для заливки фундамента/стройки",
    ...
    focus_points = [
        ...
        "Удобно, что помимо основного металла сразу купил всю мелочевку (краску, крепеж, расходники) в одном месте",
        "Водитель на Газели/манипуляторе красавчик, смог аккуратно заехать в узкие ворота СНТ/участка",
        "На складе погрузили быстро, металл чистый (без сильной слоистой ржавчины и грязи)",
        "Заказывал резку в размер, порезали ровно, погрешность минимальная, геометрия норм",
        "Оформление документов и выставление счета заняло минимум времени, цены актуальные"
    ]

    styles_and_lengths = [
        "Короткий (1-2 предложения). Максимально сухо и по делу.",
        ...
        "Разговорный (2-3 предложения). Грамотный, но без сложных оборотов и причастий.",
        "Небрежный (1-2 предложения). Пишет с ошибками (пропуски запятых, опечатки)."
    ]
    ...
    for i in range(target_count):
        ...
        # Подбираем Персонажа и Пол
        if assigned_gender == 'Женский':
            persona = "Женщина, заказывает стройматериалы на участок по списку от мужа или строителей"
            gender_instruction = "СТРОГО ЖЕНЩИНА (глаголы: заказала, привезла)"
        else:
            persona = random.choice(male_personas)
            gender_instruction = "СТРОГО МУЖЧИНА (глаголы: заказал, привез)"

        # Подбираем Эмоцию (Sentiment) на основе нашей математики оценок
        if ratings[i] == 5.0:
            sentiment = random.choice([
                "5 звезд: все прошло гладко, обычная рабочая покупка, без лишних восторгов.",
                "5 звезд: очень помог человеческий фактор (менеджер вошел в положение или водитель помог выгрузить)."
            ])
        elif ratings[i] in [4.0, 4.5]:
            sentiment = random.choice([
                f"{ratings[i]} звезды: товар и цена отличные, но пришлось подождать на погрузке/оформлении минут 20-30.",
                f"{ratings[i]} звезды: привезли все, но забыли мелкую позицию (крепеж/заглушки), вопрос быстро решили довозом."
            ])
        else:
            sentiment = "3.5 звезды: товар нормальный, но была проблема (менеджер долго тупил, въезд на базу ужасный, долго искали заказ на складе)."

        focus = random.choice(focus_points)
        style = random.choice(styles_and_lengths)

        # Инструкция для регистра (для ИИ)
        case_rule = "Пиши весь текст СТРОЧНЫМИ (маленькими) буквами." if case_pool[i] == 'lower' else "Начинай с заглавной буквы."

        authors_listing.append(
            f"--- ОТЗЫВ #{i+1} ---\n"
            f"Имя: {name} | Пол: {gender_instruction}\n"
...
1. ЗАПРЕТ НА ШАБЛОНЫ И СТРУКТУРУ.
Категорически запрещена классическая структура отзыва (Вступление -> Плюсы -> Минусы -> Вывод). Не используй приветствия и прощания. Начинай текст сразу с сути. Не делай обобщающих выводов в конце.
2. СТОП-СЛОВА (МАРКЕРЫ ИИ).
Никогда не используй слова: безупречный, высококачественный, настоятельно рекомендую, порадовало, в целом, подводя итог, оптимальный, профессионализм, клиентоориентированность, превзошел ожидания, на высшем уровне, данного, поставщик, компания.
3. ЕСТЕСТВЕННОСТЬ И СТИЛЬ.
Пиши разговорным, бытовым языком, без сложных литературных конструкций (убери причастные и деепричастные обороты). Текст должен быть читаемым, но без академической выверенности. 
4. СТРОГИЙ ФОКУС.
Описывай только ту ситуацию, товары и эмоцию, которые переданы во вводных данных отзыва. Не придумывай лишние этапы сделки (если фокус на доставке, не пиши про резку). 
5. РАБОТА С LSI И КЛЮЧАМИ.
Глобальный список LSI: {", ".join(final_lsi)}. 
ОБЯЗАТЕЛЬНО распредели эти слова между отзывами так, чтобы каждое слово было использовано ровно 1 раз на всю пачку. Оборачивай их в <b>...</b>. Разрешается склонять ключи и менять падежи.

ВВОДНЫЕ ДАННЫЕ ДЛЯ ГЕНЕРАЦИИ:
...
"""
        ...
        for i in range(len(reviews)):
            ...
            if text:
                # 50/50 Регистр первой буквы (жестко через Python)
                if case_pool[i] == 'lower':
                    text = text[0].lower() + text[1:]
                elif case_pool[i] == 'upper':
                    text = text[0].upper() + text[1:]
                
                # Удаляем точки на конце в 60% случаев для эффекта "не дописал"
