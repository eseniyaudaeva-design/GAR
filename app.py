...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # ФОКУС: Водитель возвращен, но с системой весов (градацией)
    focus_points = [
    ...
    # Веса для фокусов (в сумме 100%). Водитель выпадает только в 10% случаев.
    ...
    for i in range(target_count):
        ...
        # Выбираем фокус с учетом весов (градации)
...
2. РЕАЛИСТИЧНАЯ ДОСТАВКА: Жесткий запрет писать, что доставили "в тот же день", "сегодня", "за пару часов". Металлопрокат так быстро не возят. Доставка занимает минимум день-два. Пиши "привезли в назначенный срок", "ждал пару дней".
...
5. Естественность и стиль: Пиши разговорным, бытовым языком. Соблюдай переданные в переменных правила регистра (если сказано писать с маленькой буквы — пиши весь текст с маленькой).
6. Работа с LSI и SEO-ключами.
Переданные ключевые фразы должны быть вписаны грамматически верно, без орфографических ошибок. Внедряй их органично.

СПИСОК LSI: {", ".join(final_lsi)} 
(РАСПРЕДЕЛИ ИХ ТАК, ЧТОБЫ КАЖДОЕ СЛОВО ВСТРЕТИЛОСЬ РОВНО 1 РАЗ НА ВСЮ ПАЧКУ ОТЗЫВОВ. Оборачивай в <b>...</b>)
