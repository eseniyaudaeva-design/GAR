...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 1. Математика для среднего балла >= 4.71 и СТРОГО одной оценки 3.5
    def get_balanced_ratings(n):
        if n == 1:
            return [5.0]
            
        # Гарантируем ровно одну 3.5 на весь массив (для отработки форс-мажоров)
        ratings = [3.5]
        for _ in range(n - 1): 
            ratings.append(5.0)
            
        # Слегка разбавляем пятерки четверками, чтобы было реалистично, но средний балл держался
        indices = list(range(1, n))
        random.shuffle(indices)
        for idx in indices:
            if (sum(ratings) / n) <= 4.75: 
                break
            new_val = random.choice([4.0, 4.5])
    ...
    # 2. Даты строго с 2020 года по сегодняшний день
    ...
    # 3. Принудительная процентовка полов: 80% мужчин / 20% женщин
    ...
    for i in range(target_count):
        # Берем имя и тип из переданных, но ПОЛ жестко контролируем нашей пропорцией 80/20
        if i < len(chosen_authors):
            name = chosen_authors[i].get('name', f'Автор_{i}')
            a_type = chosen_authors[i].get('type', random.choice(['B2B', 'B2C']))
        else:
            name = f'Автор_{i}'
            a_type = random.choice(['B2B', 'B2C'])
            
        assigned_gender = gender_pool[i]
        authors_listing.append(f"Автор: {name} | Пол: {assigned_gender} | Оценка: {ratings[i]} | Тип: {a_type}")
    ...
    try:
        ...
        prompt = f"""Задача: Написать {target_count} КАРДИНАЛЬНО РАЗНЫХ B2B/B2C отзывов о компании-поставщике металлопроката. 
Категория/Товар: "{h2_header}".

СПИСОК АВТОРОВ (Имя, Пол, Оценка, Тип):
{nl.join(authors_listing)}

ПРАВИЛО 1: ЖЕСТКАЯ УНИКАЛЬНОСТЬ LSI (КРИТИЧНО!)
- Список LSI: {", ".join(lsi_words[:20])}
- КАЖДОЕ LSI-СЛОВО ИСПОЛЬЗУЙ СТРОГО 1 РАЗ НА ВЕСЬ МАССИВ ОТЗЫВОВ! 
- Если ты вставил слово в один отзыв, больше НИКОГДА его не используй в других.
- Оборачивай использованные LSI в теги <b>...</b>. Символы ** не используй.
- Если LSI-слов меньше, чем отзывов — пиши часть отзывов вообще без LSI.

ПРАВИЛО 2: БОРЬБА С ШАБЛОННОСТЬЮ И СТИЛЬ МЕТАЛЛОБАЗЫ (80% МУЖЧИН)
- Целевая аудитория суровая: снабженцы (B2B), прорабы, сварщики, мужики-дачники (B2C). Женщины (20%) — это обычно жены, которым поручили заказать металл, или бухгалтеры (B2B).
- ИСПОЛЬЗУЙ СЛЕНГ ГРАМОТНО: хлысты (6м, 12м), кругляк, профтруба, деловой остаток, резка газом/болгаркой, весовая, УПД, ЭДО, сертификаты ГОСТ.
- ИСКЛЮЧИ стоп-слова нейросети: "менеджер", "качество", "рекомендую", "объект", "в целом", "данный товар", "эстетичный", "идеально подошел". 
- РАЗНЫЕ СЮЖЕТЫ: 
  * Самовывоз (загружали в прицеп, как прошла весовая).
  * Доставка манипулятором (заказывал на участок).
  * Бухгалтерия (как быстро дали закрывашки и ЭДО).
- РАЗНАЯ ДЛИНА: От 2-3 слов ("Норм база, отгрузили быстро") до 3-4 предложений (поток сознания дачника).

ПРАВИЛО 3: РЕАЛИСТИЧНЫЕ ОШИБКИ (ХАОТИЧНО ДЛЯ 30-40% АВТОРОВ)
- Не делай ошибок у B2B-снабженцев и бухгалтеров! Ошибки делают только B2C-частники.
- "Батя-стайл": много многоточий... нет заглавных букв... рубленые фразы.
- Ошибка "без пробела": Игнорирование пробелов после запятых (металл не ржавый,документы дали,погрузка быстрая).
- Опечатки "толстых пальцев": случайная соседняя буква по клавиатуре ЙЦУКЕН (швеллер ровныц, спсибо, всп четко).

ПРАВИЛО 4: ОЦЕНКА 3.5 И 4.0 (КАК ПОВЫШАТЬ ДОВЕРИЕ - КРИТИЧЕСКИ ВАЖНО)
- У тебя в списке есть отзыв с оценкой 3.5 и, возможно, 4.0. 
- ЗАПРЕЩЕНО ругать саму компанию, наш склад, качество товара, недовес или наших сотрудников!
- ПРИЧИНА СНИЖЕНИЯ ОЦЕНКИ ТОЛЬКО ВО ВНЕШНЕМ ФОРС-МАЖОРЕ:
  1. Водитель сторонней транспортной компании (ПЭК/Деловые линии) или наемной ГАЗели опоздал, застрял, заблудился.
  2. На объекте самого клиента сломался кран/размыло дорогу, пришлось ждать разгрузку.
  3. Банк клиента завис и платеж шел 2 дня, из-за чего затянулась стройка.
  4. Сам клиент неправильно посчитал размеры и пришлось делать дозаказ.
- ОБЯЗАТЕЛЬНАЯ КОНЦОВКА такого отзыва, как пример: "Менеджеры компании или ребята на базе спасли ситуацию, пошли навстречу, вошли в положение". Оценка снижена чисто из-за испорченных нервов внешними факторами.

ПРАВИЛО 5: ОГРАНИЧЕНИЯ И ЗАПРЕТЫ
- ЗАПРЕТ НА ГОРОДА: Если в LSI есть город (Москва, СПБ и т.д.) — ПРОИГНОРИРУЙ ЕГО.
- Сроки доставки должны быть реалистичными (пару дней, неделя, согласованные сроки). Никаких "привезли за 2 часа".
- Никаких приветствий ("Всем привет", "Доброго времени суток").
- Смайлы: ТОЛЬКО 👍 и 👌 (используй очень редко, не больше 1 раза на 10 отзывов).
- Тире: любые тире запрещены.

ВЕРНИ СТРОГО JSON МАССИВ (Ключи на русском):[{{ "Имя": "...", "Дата": "...", "Оценка": ..., "Текст": "..." }}]
(Даты используй СТРОГО ПО ПОРЯДКУ из списка: {", ".join(raw_dates)})
"""
