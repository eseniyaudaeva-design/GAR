...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    # 2. Рандомные даты (Строго заданный диапазон)
    ...
    # 4. ОСЬ СТИЛИСТИКИ (Только структура мысли)
    styles_pool = [
        "БЛИЦ: Короткие рубленые фразы. 3-6 слов. Просто констатация фактов (Пример: Норм база. Отгрузили быстро).",
        "ПОТОК СОЗНАНИЯ: Одно-два длинных предложения без четких пауз, всё пишется на одном дыхании.",
        "ИСТОРИЯ: Начни с того, ЗАЧЕМ понадобился металл (на забор, стройку, в цех), а потом про саму покупку.",
        "ФОКУС НА ДЕТАЛИ: Опиши покупку вскользь, но сделай акцент на одной мелочи (работа крановщика, быстрая выписка пропуска).",
        "С КОНЦА: Начни с финального вердикта ('Рекомендую', 'Нормальная металлобаза'), а потом коротко объясни почему."
    ]

    # 5. ОСЬ МИКРО-ОШИБОК (Только визуальные дефекты)
    micro_errors_pool = [
        "РЕГИСТР 1: Весь текст строчными, но ВДРУГ одно случайное слово в середине напиши с Заглавной.",
        "РЕГИСТР 2: Ровно один раз начни новое предложение (после точки) с маленькой буквы.",
        "РЕГИСТР 3: Напиши абсолютно ВЕСЬ текст только маленькими буквами (без заглавных вообще).",
        "ПРОБЕЛЫ 1: Ровно ОДИН РАЗ во всем тексте не поставь пробел после запятой (слово,слово).",
        "ПРОБЕЛЫ 2: Ровно ОДИН РАЗ во всем тексте поставь пробел перед запятой (слово , слово).",
        "ПУНКТУАЦИЯ 1: Вообще не ставь точку в самом конце отзыва.",
        "ПУНКТУАЦИЯ 2: Ровно один раз используй многоточие '...' вместо обычной запятой или точки.",
        "ПУНКТУАЦИЯ 3: Закончи одно случайное предложение двумя восклицательными знаками '!!'.",
        "ОПЕЧАТКА 1: Сделай ровно одну опечатку по соседней кнопке клавиатуры ЙЦУКЕН (например: спсибо, очннь).",
        "ОПЕЧАТКА 2: Используй одно слово с частой ошибкой (например: вообщем, зделали, извеняюсь).",
        "ЭМОДЗИ: В самом конце текста поставь ровно один простой смайлик 👍 или 👌."
    ]
    ...
    for i in range(target_count):
        ...
        if assigned_gender == 'Женский':
            gender_instruction = "Женский (КРИТИЧНО: ВСЕ глаголы писать от женского лица: заказалА, приехалА, забралА!)"
        else:
            gender_instruction = "Мужской (глаголы мужского рода: заказал, забрал)"

        # РЕАЛИСТИЧНЫЕ СЮЖЕТЫ
        if ratings[i] == 3.5:
            plot = "СЮЖЕТ 3.5 ЗВЕЗД: Тебя взбесила ИНФРАСТРУКТУРА базы (ямы на въезде, сломался терминал, долгая весовая). Металл отличный, менеджеры молодцы, но за организацию — минус."
        elif ratings[i] in [4.0, 4.5]:
            plot = "СЮЖЕТ 4 ЗВЕЗДЫ: Всё ок, но маленькая бытовая придирка (на складе грязно, долго искали остаток). В целом всё хорошо."
        else:
            plot = "СЮЖЕТ 5 ЗВЕЗД: Обычный успешный заказ. Расскажи про быструю погрузку, ровную резку или наличие нужного металла."

        # ДИНАМИЧЕСКИЙ ГЕНЕРАТОР ХАОСА
        assigned_style = random.choice(styles_pool)
        
        # Бросаем кубик: 20% - 0 ошибок, 40% - 1 ошибка, 30% - 2 ошибки, 10% - 3 ошибки
        num_errors = random.choices([0, 1, 2, 3], weights=[20, 40, 30, 10])[0]
        
        if num_errors == 0:
            error_instruction = "ОШИБКИ: 0. Напиши идеально грамотный текст, соблюдая все правила русского языка."
        else:
            chosen_errors = random.sample(micro_errors_pool, num_errors)
            error_instruction = f"ТВОИ МИКРО-ПРИВЫЧКИ (КРИТИЧНО: примени каждую ровно ОДИН РАЗ на весь текст!):\n- " + "\n- ".join(chosen_errors)

        authors_listing.append(
            f"Отзыв #{i+1} | Имя: {name} | Пол: {gender_instruction} | Оценка: {ratings[i]}\n"
            f"ЗАДАЧА: {plot}\n"
            f"СТИЛЬ ИЗЛОЖЕНИЯ: {assigned_style}\n"
            f"{error_instruction}\n"
        )
    ...
    try:
        ...
        prompt = f"""Задача: Написать {target_count} АБСОЛЮТНО РАЗНЫХ отзывов о металлобазе (товар: "{h2_header}").

Список отзывов и персональные задачи:
{nl.join(authors_listing)}

ГЛОБАЛЬНЫЕ ПРАВИЛА (ДЛЯ ВСЕХ ОТЗЫВОВ):
1. ЖЕСТКАЯ УНИКАЛЬНОСТЬ LSI: {", ".join(lsi_words[:20])}
- Каждое LSI-слово используй СТРОГО 1 РАЗ на весь массив! Оберни в <b>...</b>.
- ОБЯЗАТЕЛЬНО СКЛОНЯЙ LSI по падежам (не "из <b>нержавейка</b>", а "из <b>нержавейки</b>").

2. АБСОЛЮТНЫЙ ЗАПРЕТ СИМВОЛОВ И КАНЦЕЛЯРИТА (КРИТИЧНО):
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать: двоеточия (:), длинные тире (—), значки градусов (°), кавычки.
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать номера ГОСТов (например, "ГОСТ 8509-93"). Пиши просто "по госту".
- ЗАПРЕЩЕНЫ фразы-шаблоны: "качество на высоте", "пошли навстречу", "соответствует заявленному", "Я как бухгалтер", "Закупаем постоянно".
- Можно использовать ТОЛЬКО: обычные запятые, точки и короткий дефис (-). 

3. ПРАВИЛО "БЕЗ КАРИКАТУР" (САМОЕ ВАЖНОЕ):
- 95% текста любого отзыва должно быть написано обычным, простым русским языком. 
- Указанные микро-привычки (ошибки) применяй ТОЧЕЧНО! Не превращай текст в нечитаемый мусор.

4. ОГРАНИЧЕНИЯ:
- Если в LSI есть город — проигнорируй его.

ВЕРНИ СТРОГО JSON МАССИВ (Ключи на русском):[{{ "Имя": "...", "Оценка": ..., "Текст": "..." }}]
"""
