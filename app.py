...
# ЧЕРНЫЙ СПИСОК МУСОРНЫХ СЛОВ НА ЛАТИНИЦЕ
# Сюда добавляем всё, что НЕ является марками стали или полезными терминами
...
def generate_five_blocks(client, base_text, tag_name, seo_words=None):
    ...
    if seo_words and len(seo_words) > 0:
        ...
        В тексте ДОЛЖНЫ быть использованы слова из этого списка (интегрируй минимум 70% из них): 
        {keywords_str}.
        
        ПРАВИЛА ДЛЯ КЛЮЧЕВЫХ СЛОВ:
        1. Вписывай их максимально органично и естественно.
        2. Можно менять окончания (склонять), но корень должен сохраняться.
        3. !!! ВАЖНО !!! Каждое использованное слово из списка ОБЯЗАТЕЛЬНО выдели тегом <b> (пример: <b>слово</b>).
        """
    ...
    ПОМНИ ПРО ВЫДЕЛЕНИЕ ЖИРНЫМ <b>КЛЮЧЕВЫХ СЛОВ</b>.
...
with tab_ai:
    ...
    if start_gen:
        ...
        with st.status("Скачивание данных со страницы...", expanded=True) as status:
            ...
            # --- СБОР КЛЮЧЕВЫХ СЛОВ ИЗ ВКЛАДКИ SEO (С ФИЛЬТРАЦИЕЙ И ЛИМИТОМ) ---
            seo_keywords_list = []
            if st.session_state.analysis_results:
                ...
                if high_list:
                    ...
                    # 2. Берем только ТОП-30, чтобы не перегружать нейросеть
                    seo_keywords_list = clean_candidates[:30]
                    
                    st.info(f"Выбрано {len(seo_keywords_list)} лучших слов (ТОП-30) для внедрения: {', '.join(seo_keywords_list)}")
                else:
