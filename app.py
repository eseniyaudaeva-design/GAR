...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    try:
        ...
        prompt = f"""Ты — генератор сырого, неидеального пользовательского контента.
Тема: "{h2_header}". 
Задача: {target_count} отзывов.

ГЛАВНОЕ: Отзывы должны выглядеть так, будто их писали уставшие люди с телефона, в спешке, на ходу. Мне нужны ГРУБЫЕ, ЗАМЕТНЫЕ СРАЗУ ошибки в 70% отзывов (пунктуация, опечатки, отсутствие заглавных букв). Забудь про вылизанный литературный язык!

ПРИМЕРЫ (КАК НАДО И КАК НЕЛЬЗЯ):
❌ ПЛОХО (ИИ-стиль): "Заказывал стальные трубы. Привезли вовремя, качество отличное. Спасибо менеджерам за работу, рекомендую!"
✅ ХОРОШО (Живой человек, криво): "брали на забор. привезли с опозданием но манагеры норм отработали все обьяснили. металл вроде ниче так"
✅ ХОРОШО (Торопыга): "все четко.спс"
✅ ХОРОШО (Грубые опечатки): "доставили бытсро, качетсво устроило, будем брать езе."
✅ ХОРОШО (Душнила): "Долго искали поставщика. В целом норм, но водила перепутал поворот пришлось выходить встречать..."

ПРАВИЛО LSI-СЛОВ (КРИТИЧНО!):
Список слов: {", ".join(lsi_words[:20])}. 
1. ИСПОЛЬЗУЙ 1 РАЗ: Каждое слово из этого списка можно использовать МАКСИМУМ 1 РАЗ на всю выборку из {target_count} отзывов! Вставил в один отзыв — в другие писать ЗАПРЕЩЕНО.
2. ЗАПРЕТ ОШИБОК В LSI: Сами LSI-слова должны быть написаны ИДЕАЛЬНО ГРАМОТНО, без единой опечатки! Весь остальной текст отзыва может быть кривым и с ошибками, но LSI-ключ (внутри тегов) должен быть 100% правильным для SEO. (Склонять по падежам можно, но без искажений корня и букв).
3. ТЕГИ: ОБЯЗАТЕЛЬНО выделяй использованные LSI-слова тегами <b>...</b> (например, <b>стальная труба</b>). ** не используй!
...
2. НИКАКОГО АНГЛИЙСКОГО: Исключительно кириллица.
3. НИКАКИХ ГОРОДОВ: Если в LSI есть города (Москва, Спб и т.д.) — игнорируй их полностью. Не пиши города в отзывах.
4. СМАЙЛЫ: ТОЛЬКО 👍 и 👌. Ставь редко и хаотично. Строго запрещены ✅.
