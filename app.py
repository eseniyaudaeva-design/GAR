...
# ==========================================
# TAB 5: BULK LSI GENERATOR (PRO B2B - FIXED GRAMMAR & STRUCTURE)
# ==========================================
import requests
...
with tab_lsi_gen:
    st.header("🏭 Массовая генерация B2B (Smart Grammar + LSI Priority)")
    st.markdown("Умные H3 (падежи), LSI в начале текста, Техническая структура.")
    ...
    def get_h2_from_url(url):
        # (Функция парсинга осталась прежней, она рабочая)
        try:
            from curl_cffi import requests as cffi_requests
            r = cffi_requests.get(
        ...
        try:
            ...
            # Приоритет поиска: H1 (обычно точнее для товара), затем H2
            ...
            if soup.find('title'): return soup.find('title').get_text(strip=True).split('|')[0].strip()
            return f"ERROR: H1/H2 not found"
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        system_instruction = (
            "Ты — профессиональный технический редактор и инженер-металловед. "
            "Ты мастерски владеешь русским языком: идеально склоняешь названия товаров по падежам. "
            "Ты пишешь сухой, фактологический B2B контент. Никакой воды."
        )
        
        user_prompt = f"""
        ЗАДАЧА: Напиши техническую статью для товара: "{exact_h2}".
        
        [I] АНАЛИЗ И ЧИСТКА НАЗВАНИЯ:
        1. Из входной фразы "{exact_h2}" вычлени ЧИСТОЕ название товара.
           Пример: если вход "Купить Трубу ВГП недорого", чистое название = "труба ВГП".
        2. Далее в тексте используй это чистое название, склоняя его по падежам (Родительный: кого/чего? и т.д.).
        
        [II] ГЛАВНЫЕ ПРАВИЛА:
        1. LSI КЛЮЧИ: 80% слов из списка LSI используй в ПЕРВОЙ ПОЛОВИНЕ текста (до раздела "Сферы применения").
        2. ЗАПРЕЩЕНО писать отдельные абзацы про "Сервис", "Доставку", "Оплату" в основном теле статьи.
        3. Типографика: Используй только среднее тире (–).
        4. СТРОЧНЫЕ БУКВЫ: Если название товара стоит в середине заголовка H3, пиши его с маленькой буквы!
           ПЛОХО: <h3>Технические характеристики Трубы стальной</h3>
           ХОРОШО: <h3>Технические характеристики трубы стальной</h3>
        
        [III] СТРУКТУРА СТАТЬИ (СТРОГО):
        
        1. Заголовок H2: 
           Выведи строго как есть: <h2>{exact_h2}</h2>
        
        2. Лид-абзац (Бэнгер):
           2-3 предложения. Сразу суть: что это, главная функция, надежность. Без воды "Мы рады предложить".
           
        3. Блок контактов:
           {contact_html_block}
        
        4. Заголовок H3 (Технические характеристики):
           Сгенерируй заголовок по шаблону: "Технические характеристики [товара в Род. падеже]".
           Пример: "Технические характеристики полосы стальной" (со строчной буквы!).
        
        5. Текст + Список №1:
           - Вводное предложение (заканчивается двоеточием).
           - Список (ul/li) из 5-6 пунктов: конкретные свойства (марки стали, прочность, ГОСТы, допуски).
           - Важно: Насыщай этот блок LSI словами.
        
        6. Заголовок H3 (Производство/Особенности):
           Сгенерируй заголовок по шаблону: "Особенности производства [товара в Род. падеже]" ИЛИ "Требования ГОСТ к [товару в Дат. падеже]".
           Следи за грамматикой и регистром!
           
        7. Текст (Технический):
           Описание технологии изготовления, стандартов качества. Максимальная концентрация технических терминов.
        
        8. Заголовок H3 (Применение):
           Сгенерируй заголовок по шаблону: "Сферы применения [товара в Род. падеже]".
        
        9. Текст + Список №2:
           - Список отраслей (Строительство, Машиностроение, ЖКХ и т.д.) с кратким пояснением, где именно используется.
        
        10. Заголовок H3 (Поставка):
            Шаблон: "Параметры поставки" (без названия товара).
        
        11. Финальный список №3:
            Сухой перечень условий отгрузки (в бухтах/прутках, упаковка, сертификаты).
            
        [IV] ВХОДНЫЕ ДАННЫЕ:
        LSI слова для внедрения (выдели тегом <b>): {{{lsi_string}}}
        
        ВЫВОД: Только чистый HTML код (без ```html).
        """
        ...
        try:
            ...
            content = response.choices[0].message.content
            # Чистка маркдауна на всякий случай
    ...
    with st.expander("⚙️ Настройки и LSI", expanded=True):
        ...
        with c1:
            lsi_api_key = st.text_input("Gemini API Key", value=cached_key, type="password", key="bulk_api_key_v8")
        with c2:
            raw_lsi_common = st.text_area("LSI (общий)", height=68, placeholder="купить, цена, гост, сталь, вес метра...")
    ...
    if total_q > 0:
        ...
        if run_btn:
            if not lsi_api_key:
                ...
                table_placeholder.info("⏳ Запуск процессов...")
