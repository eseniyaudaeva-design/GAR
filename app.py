...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    try:
        ...
        prompt = f"""Задача: Написать {target_count} реалистичных B2B/B2C отзывов о компании-поставщике (товар: "{h2_header}").
Вернуть СТРОГО валидный JSON массив. Никаких вступлений, только JSON.

СПИСОК АВТОРОВ (Имя, Пол, Оценка):
{nl.join(authors_listing)}

ПРАВИЛО 1: СМЫСЛ И ВНЕДРЕНИЕ LSI
- Список LSI: {", ".join(lsi_words[:20])}
- ВАЖНО: Вписывай LSI-слова строго ПО СМЫСЛУ. Отзыв должен быть логичным рассказом (зачем покупали, куда применяли, как прошло взаимодействие).
- ИСПОЛЬЗУЙ 1 РАЗ: Каждое LSI-слово используй максимум 1 раз на всю выборку из {target_count} отзывов.
- ГРАМОТНОСТЬ В LSI: Сами LSI-слова пиши идеально без опечаток (склонять по падежам можно).
- ТЕГИ: Выделяй использованные LSI тегами <b>...</b> (например, <b>труба</b>). ** не используй!

ПРАВИЛО 2: ЗАПРЕТ НА ПРИВЕТСТВИЯ И "ФОРУМНЫЙ" СТИЛЬ
- КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать приветствия: "Всем привет", "Здравствуйте", "Доброго времени суток". 
- Это отзывы на картах, начинай сразу по делу: "Искали...", "Брали на объект...", "Менеджер помог...".

ПРАВИЛО 3: РЕАЛИСТИЧНЫЕ ОШИБКИ И ОПЕЧАТКИ
- Не делай из авторов идиотов, текст должен оставаться читабельным и осмысленным.
- Опечатки должны быть РЕАЛИСТИЧНЫМИ: промах по соседней клавише на ЙЦУКЕН-клавиатуре ("ш" вместо "щ", "в" вместо "а"), пропуск пробела после запятой, игнорирование заглавных букв в начале предложения. Никакого бессмысленного набора букв!

ПРАВИЛО 4: РЕАЛИСТИЧНЫЕ СРОКИ ДОСТАВКИ (КРИТИЧНО)
- Мы возим габаритный металлопрокат и стройматериалы. МЫ НЕ ДОСТАВЛЯЕМ ЗА 1 ДЕНЬ ИЛИ ПАРУ ЧАСОВ.
- Если упоминаешь доставку, сроки должны быть адекватными: 2-3 дня, 5 дней, неделя, "в оговоренный срок". Фразы "привезли в тот же день" — СТРОГО ЗАПРЕЩЕНЫ.

ПРАВИЛО 5: ЛОГИКА ОЦЕНОК 3.5 И 4.0
- ЗАПРЕЩЕНО ругать саму компанию, наш склад, качество товара.
- Причина снижения оценки: ТОЛЬКО внешний форс-мажор.
- Обязательно напиши, что НАШИ менеджеры спасли ситуацию и были на связи, а товар идеальный.

ПРАВИЛО 6: ЖЕСТКИЕ ОГРАНИЧЕНИЯ
- ЗАПРЕТ НА ГОРОДА: Никаких упоминаний городов! Если в переданном LSI-списке есть город (Москва, СПБ и т.д.) — ПРОИГНОРИРУЙ ЕГО, не вставляй в текст.
- ЗАПРЕТ НА АНГЛИЙСКИЙ: Исключительно кириллица.
- СМАЙЛЫ: ТОЛЬКО 👍 и 👌. Ставь редко. Смайл ✅ СТРОГО ЗАПРЕЩЕН.
- ТИРЕ: Только короткий дефис "-". Длинное "—" под запретом.
...
"""
        resp = client.chat.completions.create(
            ...
            temperature=1.0 # Немного снизил температуру (с 1.15 до 1.0), чтобы GPT-4o не уходил в бред, но сохранял разнообразие
        )
