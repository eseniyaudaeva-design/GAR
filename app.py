...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    plot_types = [
        "Для дачи: забор, крыша, сарай, теплица.",
        "Экономия: искал дешевле, сравнивал цены.",
        "Сервис: менеджеры, доставка, помощь в выборе."
    ]
    ...
    for i in range(target_count):
        ...
        # ФОРМИРОВАНИЕ СЮЖЕТА ПОД ОЦЕНКУ
        base_plot = random.choice(plot_types)
        plot_instruction = f"Ситуация: {base_plot}"
        
        if ratings[i] == 5.0:
             plot_instruction += " Отзыв позитивный. Всё прошло без проблем."
        elif ratings[i] == 3.5:
             plot_instruction += " Есть серьезный минус (очередь, путаница на складе, сложный въезд)."
        else:
             plot_instruction += " В целом отлично, но есть мелкая придирка (тяжело нести, грязная упаковка)."

        # УНИКАЛЬНЫЙ СТИЛЬ И ОШИБКИ ДЛЯ КОНКРЕТНОГО ОТЗЫВА
        style_inst = ""
        ...
        # СОБИРАЕМ ИНДИВИДУАЛЬНУЮ КАРТОЧКУ
    ...
    try:
        ...
        # ГЛОБАЛЬНЫЙ ПРОМПТ (Теперь без "ты мужик", просто координатор)
...
2. ЖИВОЙ ЯЗЫК: Не пиши как копирайтер. Запрещены слова: "качественный", "соответствует заявленному", "рекомендую к покупке", "недостатки отсутствуют", "данный товар".
3. РАЗНООБРАЗИЕ: Каждый отзыв должен звучать по-разному. Не дублируй одни и те же опечатки или слова-паразиты между отзывами.
4. LSI СЛОВА: {", ".join(final_lsi)}. Каждое слово используй 1 РАЗ на всю партию отзывов! Оборачивай в <b>...</b>.
...
"""
        ...
        for i in range(len(reviews)):
            ...
            if text:
                # 50/50 Регистр первой буквы
                if case_pool[i] == 'lower':
                    text = text[0].lower() + text[1:]
                elif case_pool[i] == 'upper':
                    text = text[0].upper() + text[1:]
                
                # Удаление точки в конце для "неграмотных" отзывов (85% шанс)
