...
# --- 2. ТАБЛИЦА (УМНАЯ, БЕЗ ДУБЛЕЙ ТЕКСТА) ---
                if use_tables and client:
                    for t_topic in table_prompts:
                        ...
                        if not t_topic or t_topic == "Характеристики":
                            topic_instruction = "Тема: Технические параметры (размеры, веса, нагрузки)."

                        prompt_tbl = (
                            f"Ты — технический редактор. Твоя задача — создать HTML-таблицу для товара: '{header_for_ai}'.\n"
                            f"Категория: '{page['name']}'.\n"
                            f"{topic_instruction}.\n\n"
                            
                            f"SEO-СЛОВА ДЛЯ ВНЕДРЕНИЯ: [{tech_data}]\n\n"
                            
                            f"🚨 ГЛАВНЫЕ ПРАВИЛА (СТРОГОЕ СОБЛЮДЕНИЕ):\n"
                            f"1. АНТИ-ДУБЛЬ КОНТЕНТА: Не дублируй то, что обычно пишут в тексте!\n"
                            f"   - Если в тексте обычно пишут хим. состав или применение списком — НЕ ДЕЛАЙ это таблицей.\n"
                            f"   - Таблица нужна для ЦИФР, РАЗМЕРОВ, ВЕСОВ, МОДЕЛЕЙ. Того, что в тексте читать неудобно.\n"
                            f"   - Лучший выбор: Таблица типоразмеров, Таблица нагрузок, Геометрические параметры.\n\n"
                            
                            f"2. ПРАВИЛО ВНЕДРЕНИЯ СЛОВ (МАКСИМУМ 1 РАЗ):\n"
                            f"   - Используй ключевые слова из списка ВЫШЕ только ОДИН РАЗ на всю таблицу.\n"
                            f"   - Вставляй их ТОЛЬКО в заголовки колонок (th) или название таблицы (caption).\n"
                            f"   - ЗАПРЕЩЕНО писать ключевые слова внутри ячеек данных (td)!\n\n"
                            
                            f"3. АНТИ-СПАМ В ЯЧЕЙКАХ:\n"
                            f"   - Не пиши название товара в каждой строке.\n"
                            f"   - Первая колонка — это всегда ПЕРЕМЕННАЯ (Диаметр, Артикул, Модель), а не название.\n\n"
                        ...
                        try:
                            # Температура 0.5 — баланс между строгостью структуры и разнообразием данных
