...
# --- ФУНКЦИЯ КЛАССИФИКАЦИИ С API ЯНДЕКСА (ИСПРАВЛЕННАЯ) ---
def classify_semantics_with_api(words_list, yandex_key):
    ...
    # Коммерция + Маркетинговые прилагательные
    COMMERCIAL_WORDS = {
        ...
        'гарантия', 'возврат', 'обмен', 'снабжение',
        ...
        'обратный', 'связь', 'звонить', 'партнер', 'преимущество'
    }

    # Глаголы действия (Инфинитивы), которые часто попадают в товары ошибочно
    VERB_ENDINGS = ('ать', 'ять', 'ить', 'еть', 'уть', 'оть', 'ыть', 'тся', 'ться')

    # Гео-корни (Расширенный список)
    GEO_ROOTS = [
        ...
        'новосиб', 'санкт', 'владивосто', 'оренбург', 'кемеров', 'рязань', 'томск',
        'тула', 'пенза', 'липецк', 'курск', 'тверь', 'сочи', 'сургут'
    ]
    ...
    SERVICE_KEYWORDS = {
        ...
        'изготовление', 'размотка', 'протяжка'
    }

    # --- САМОЕ ВАЖНОЕ: БЕЛЫЙ СПИСОК ТОВАРОВ (Металлопрокат) ---
    # Если слово содержит корень из этого списка — это 100% ТОВАР, игнорируя всё остальное.
    PRODUCT_ROOTS = [
        'труб', 'балк', 'швел', 'лист', 'угол', 'армат', 'круг', 'квадрат',
        'шестигран', 'проволок', 'лент', 'сетк', 'катан', 'прокат', 'метиз',
        'сталь', 'медн', 'медь', 'алюмин', 'титан', 'бронз', 'латун',
        'цинк', 'никель', 'олов', 'свинец', 'вольфрам', 'нихром', 'дюраль',
        'анод', 'катод', 'сляб', 'блюм', 'рельс', 'двутавр', 'канат', 'профиль',
        'настил', 'штрипс', 'фольга', 'метиз', 'крепеж', 'болт', 'гайк',
        'шайб', 'гвоздь', 'электрод', 'чугун', 'сплав', 'втулк', 'поковк'
    ]
    ...
    for word in words_list:
        ...
        # --- HARD FORCE: ПРОВЕРКА НА ТОВАР ПО КОРНЮ ---
        # Если это явно металл (балка, швеллер, алюминий), сразу в товары, не думая.
        is_hard_product = False
        for root in PRODUCT_ROOTS:
            if root in lemma:
                categories['products'].add(lemma)
                is_hard_product = True
                break
        if is_hard_product:
            continue
        # ----------------------------------------------

        # ГЕО ФИЛЬТР (Расширенный)
        ...
        # ГЛАГОЛЫ (Использоваться, являться и т.д.) -> Коммерция/Мусор
        if lemma.endswith(VERB_ENDINGS):
            categories['commercial'].add(lemma)
            continue
        ...
        # Если не отсеялось — спрашиваем API
        api_candidates.append(word_lower)

    # 3. ЗАПРОС К API YANDEX (для оставшихся спорных слов)
    yandex_results = {} 
    ...
    for word in api_candidates:
        ...
        # Повторная проверка на мусор (на случай лемматизации API)
        if lemma in SITE_UI_GARBAGE or lemma in COMMERCIAL_WORDS:
            categories['commercial'].add(lemma)
            continue
        ...
        if lemma.endswith('ние') or lemma.endswith('ение'):
            # Исключение для оборудования, если оно вдруг не попало в whitelist
            if 'оборудован' not in lemma: 
                is_service = True
        ...
        if pos == 'noun':
            ...
            # Прилагательные добавляем в товары только если они не коммерческие
            if len(lemma) > 2:
                categories['products'].add(lemma)
        elif pos == 'unknown':
            # Если Яндекс не знает слово, а оно прошло все фильтры - скорее всего это спец. термин (Товар)
            if len(lemma) > 2:
                categories['products'].add(lemma)
        else:
            categories['commercial'].add(lemma)
