        ...
        if st.session_state.get('analysis_done') and st.session_state.get('analysis_results'):
            ...
            if 'relevance_top' in results and not results['relevance_top'].empty:
                ...
                with st.expander("📊 График релевантности (Нажмите, чтобы раскрыть)", expanded=False):
                    ...
                    st.info(
                        """
                        **Почему на графике больше сайтов, чем я выбрал?**  
                        Для построения честного **Тренда** (зеленая линия) и поиска аномалий система всегда сканирует **Топ-30**. 
                        Большая выборка нужна, чтобы понять общую картину рынка и отсеять "случайные" сайты.
                        
                        *Примечание: Все таблицы рекомендаций ниже (слова, теги, структура) строятся строго по вашему лимиту (Топ-10 или Топ-20).*
                        """, 
                        icon="ℹ️"
                    )
                    # -----------------------------
