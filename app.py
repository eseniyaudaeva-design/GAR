...
# --- ФУНКЦИЯ КЛАССИФИКАЦИИ v8.0 (Strict Lemmatization) ---
def classify_semantics_with_api(words_list, yandex_key):
    ...
    # 1. ЧЕРНЫЙ СПИСОК (ЛЕММЫ)
    # Важно: Сюда пишем слова только в начальной форме (ед.ч, им.падеж, инфинитив)
    BLACKLIST = {
        # Интерфейс и Юридическая часть
        'меню', 'поиск', 'главная', 'карта', 'сайт', 'личный', 'кабинет', 'вход', 'регистрация', 'корзина', 
        'избранное', 'телефон', 'адрес', 'контакт', 'email', 'звонок', 'callback', 'отзыв', 'вопрос', 'ответ', 
        'менеджер', 'политика', 'конфиденциальность', 'соглашение', 'cookie', 'соглашаться', 'подписаться', 
        'ошибка', 'успешно', 'кнопка', 'форма', 'обзор', 'новость', 'статья', 'калькулятор', 'фильтр', 
        'сортировка', 'показать', 'сбросить', 'интернет', 'магазин', 'каталог', 'год', 'день', 'час', 'время',
        'еще', 'весь', 'наш', 'ваш', 'мы', 'вы', 'он', 'она', 'оно',
        
        # Абстрактные понятия (НЕ ТОВАРЫ)
        'согласие', 'соответствие', 'требование', 'условие', 'правило', 'положение', 'внимание', 'информация',
        'данные', 'обработка', 'использование', 'применение', 'назначение', 'особенность', 'преимущество',
        'качество', 'характеристика', 'описание', 'параметр', 'свойство', 'артикул', 'код', 'стандарт', 
        'гост', 'ту', 'сертификат', 'документ', 'паспорт', 'инструкция', 'гарантия',
        
        # Торговля
        'купить', 'заказать', 'цена', 'прайс', 'стоимость', 'продажа', 'недорого', 'дешево', 'дорого', 
        'скидка', 'акция', 'распродажа', 'опт', 'розница', 'руб', 'рубль', 'уе', 'заказ', 'оплата', 'платеж', 
        'рассрочка', 'кредит', 'лизинг', 'доставка', 'самовывоз', 'отгрузка', 'поставка', 'транспорт', 'логистика', 
        'возврат', 'обмен', 'наличие', 'склад', 'остаток',
        
        # Единицы измерения и общие сущ.
        'вес', 'длина', 'ширина', 'высота', 'толщина', 'диаметр', 'размер', 'объем', 'масса', 'тонна', 'метр', 
        'шт', 'кг', 'упаковка', 'марка', 'бренд', 'производитель', 'производство', 'изготовление',
        ...
        'прочий', 'другой', 'готовый', 'сложный', 'простой', 'любой', 'нужный', 'важный'
    }
    
    # Корни услуг (для отделения от товаров)
    SERVICE_ROOTS = [
        'рез', 'руб', 'гиб', 'гну', 'вар', 'свар', 'цин', 'цинк', 'монт', 'клад', 'проект', 
        'изол', 'сверл', 'грунт', 'крас', 'шлиф', 'вальц', 'перф', 'точ', 'фрез', 'работ',
        'строй', 'ремонт'
    ]
    ...
    # Сбор всех слов для запроса лемм (чтобы не терять ничего на старте)
    words_to_process = []
    
    # 2. ПЕРВИЧНАЯ ОЧИСТКА (Размеры и цифры)
    for word in words_list:
        w_lower = word.lower()
        if dim_pattern.search(w_lower) or grade_pattern.match(w_lower) or gost_pattern.search(w_lower) or w_lower.isdigit():
            categories['dimensions'].add(w_lower)
        else:
            words_to_process.append(w_lower)

    # 3. ПОЛУЧЕНИЕ ЛЕММ (API / Pymorphy)
    # Это самый важный этап. Мы сначала узнаем начальную форму, и только потом фильтруем.
    lemmatized_data = {}
    
    if words_to_process and yandex_key:
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_word = {executor.submit(get_yandex_dict_info, w, yandex_key): w for w in words_to_process}
            for future in concurrent.futures.as_completed(future_to_word):
                orig = future_to_word[future]
                try: lemmatized_data[orig] = future.result()
                except: lemmatized_data[orig] = {'lemma': orig, 'pos': 'unknown'}
    else:
        for w in words_to_process:
            ...
            if morph:
                ...
                elif 'GRND' in p.tag: pos = 'verb'
            lemmatized_data[w] = {'lemma': lemma, 'pos': pos}

    # 4. ФИНАЛЬНОЕ РАСПРЕДЕЛЕНИЕ (СТРОГО ПО ЛЕММАМ)
    for word in words_to_process:
        info = lemmatized_data.get(word, {'lemma': word, 'pos': 'unknown'})
        lemma = info['lemma']
        pos = info['pos']

        # 1. Проверка ЛЕММЫ по Черному списку (самый приоритет)
        if lemma in BLACKLIST:
            categories['commercial'].add(lemma)
            continue
            
        # 2. Проверка ЛЕММЫ по Гео
        if any(root in lemma for root in GEO_ROOTS):
            categories['commercial'].add(lemma)
            continue

        # 3. Глаголы и Наречия -> Коммерция (если это не услуга)
        is_service_root = any(root in lemma for root in SERVICE_ROOTS)
        
        if pos == 'verb' or pos == 'adverb' or pos == 'infinitive':
            if is_service_root and not lemma.endswith('ться') and not lemma.endswith('тся'):
                # Резать, Варить, Оцинковать - Услуги
                categories['services'].add(lemma)
            else:
                # Звонить, Отправить, Находиться - Коммерция
                categories['commercial'].add(lemma)
            continue

        # 4. Существительные
        if pos == 'noun':
            # Если это процесс на -ка, -ние (Резка, Сверление) с корнем услуги
            if is_service_root and (lemma.endswith('ка') or lemma.endswith('ние') or lemma.endswith('ция')):
                categories['services'].add(lemma)
            elif len(lemma) > 2:
                # Иначе - Товар (Труба, Лист, Алюминий)
                categories['products'].add(lemma)
            continue

        # 5. Прилагательные и Причастия -> Товары
        if (pos == 'adjective' or pos == 'participle') and len(lemma) > 2:
            categories['products'].add(lemma)
            continue
            
        # 6. Unknown -> Товары (если длинное)
        if pos == 'unknown' and len(lemma) > 2:
             categories['products'].add(lemma)
