...
with tab_lsi_gen:
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
        ...
        system_instruction = (
            ...
            "Ты фанатично соблюдаешь форматирование списков."
        )
        ...
        1. ФОРМАТ СПИСКОВ (КРИТИЧЕСКИ ВАЖНО):
           У тебя есть два типа строк в списках. Различай их:
           
           ТИП А) ЧИСЛОВЫЕ ПАРАМЕТРЫ (Размер, Диаметр, Вес):
           - Пиши БЕЗ ТИРЕ между названием параметра и значением.
           - ПРАВИЛЬНО: "Диаметр 0,1-10 мм;" | "Толщина 5-20 мм;"
           - НЕПРАВИЛЬНО: "Диаметр – 0,1..." | "Диаметр: 0,1..."
           
           ТИП Б) ТЕКСТОВЫЕ ПАРАМЕТРЫ (Марки, ГОСТ, Покрытие, Вид):
           - Пиши ЧЕРЕЗ ТИРЕ (–) или ДЕФИС.
           - ПРАВИЛЬНО: "Марки сплавов – X20H80, Ст3;" | "Покрытие – цинковое;"
           
        2. ДИАПАЗОНЫ ЧИСЕЛ:
           - Простые диапазоны: пиши строго через дефис (4-10 мм). ЗАПРЕЩЕНО писать "от 4 до 10".
           - Температуры и сложные: "от -40 до +450 °C".
           
        3. ЧИСЛА:
           - Только значащие цифры (3; 5; 0,5). 
           - ЗАПРЕЩЕНО писать "3,0".

        [II] СТРУКТУРА ТЕКСТА:
        1.1. Заголовок: <h2>{exact_h2}</h2>.
        
        1.2. ВСТУПЛЕНИЕ (БЭНГЕР): 3-4 предложения.
        - Первое предложение ОБЯЗАТЕЛЬНО начинается с "{exact_h2}" (или содержит как подлежащее).
        - ЗАПРЕЩЕНО начинать с отглагольных существительных ("Снижение...", "Повышение...").
        - ПРАВИЛЬНО: "{exact_h2} гарантируют защиту..."
        ...
        try:
            ...
            # Специфическая очистка тире, чтобы не сломать "Диаметр 1-10"
            # Мы заменяем ТОЛЬКО изолированные тире, окруженные пробелами
