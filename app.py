...
with tab_lsi_gen:
    ...
    def generate_full_article(api_key, exact_h2, lsi_list):
            ...
            system_instruction = (
                ...
                "Ты избегаешь союзов 'и' в перечислениях, заменяя их запятыми. "
                "Ты фанатично соблюдаешь требование по вхождению ключевой фразы в заголовки."
            )
            ...
            1. ЗАГОЛОВКИ H3 (КРИТИЧНО ВАЖНО):
               - В КАЖДОМ H3 ОБЯЗАНА БЫТЬ фраза "{exact_h2}" (склоняй по падежам).
               - СТРОГО ЗАПРЕЩЕНО заменять название товара на слова "прокат", "изделие", "материал", "товар".
               - Если товар "Труба стальная", то заголовок "Монтаж трубы стальной", А НЕ "Монтаж проката".
            ...
            3. КЛЮЧЕВОЕ СЛОВО ("{exact_h2}"):
               - Плотность 1.5% (используй именно слова из запроса).
            ...
            1. СПИСКИ:
               ...
               - В конце пункта точка с запятой (;), в конце списка точка.
    
            2. ФОРМАТ ЧИСЕЛ:
               - ТОЛЬКО ЗНАЧАЩИЕ ЦИФРЫ (нет "3,0").
               - Максимальные значения одним числом ("Макс. температура +400 °C").
    
            [III] СТРУКТУРА ТЕКСТА:
            
            1.1. Заголовок: <h2>{exact_h2}</h2>.
            
            1.2. БЭНГЕР (ВСТУПЛЕНИЕ):
               - Напиши 3-4 НОРМАЛЬНЫХ, ПОЛНЫХ предложения.
               - ЗАПРЕЩЕН "телеграфный" стиль (не пиши "Материал — сталь. Назначение — стройка").
               - ПИШИ СВЯЗНО: "Данный металлопрокат производится из конструкционной стали. Он применяется в строительстве..." (без воды и креатива, просто нормальный язык).
            ...
            1.6. Абзац 2. Описание производства (метод изготовления).
            
            1.7. ТАБЛИЦА ХАРАКТЕРИСТИК (СПРАВОЧНАЯ):
            4-5 строк. Без дублей списка №1.
            (ГОСТ/ТУ, Марка сплава, Плотность, Твердость, Упаковка).
            КОД ТАБЛИЦЫ:
            ...
            "Классификация {exact_h2} (в родит. падеже, с маленькой буквы)"
            (Никаких "Классификация проката"! Только название товара!).
            ...
            "Монтаж {exact_h2} (в родит. падеже, с маленькой буквы)" ИЛИ "Обработка {exact_h2} (род. падеж)".
            ...
            (Без союзов "и", через запятую).
               
            1.17. Абзац 6. Итоговое резюме (связный текст). Условия отгрузки.
