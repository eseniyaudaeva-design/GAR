...
st.set_page_config(layout="wide", page_title="GAR PRO v3.5 (FIXED)", page_icon="📊")
...
st.markdown(f"""
    <style>
        ...
        /* Класс для второй вкладки, чтобы не ломать первую */
        .tool-card {{ 
            padding: 20px; 
...
# ==========================================
# 1. ФУНКЦИИ ИЗ ИСХОДНИКА (ДЛЯ ТАБА 1)
# ==========================================
...
def load_lemmatized_dictionaries():
    base_path = "data"
    product_lemmas, commercial_lemmas, specs_lemmas, geo_lemmas, services_lemmas = set(), set(), set(), set(), set()
    # (Код загрузки JSON как в оригинале, опущен для краткости, но возвращаемые типы важны)
    # В реальном коде здесь должны быть блоки try-except для загрузки файлов
...
def classify_semantics_with_api(words_list, yandex_key):
    ...
    DEFAULT_COMMERCIAL = {'цена', 'купить', 'прайс', 'корзина', 'заказ', 'руб', 'наличие', 'склад', 'магазин', 'акция', 'скидка', 'опт', 'розница', 'каталог', 'телефон', 'менеджер', 'сайт', 'главная', 'вход', 'регистрация', 'отзыв', 'гарантия'}

    categories = {'products': set(), 'services': set(), 'commercial': set(), 'dimensions': set(), 'geo': set(), 'general': set()}
    
    for word in words_list:
        word_lower = word.lower()
        if word_lower in SPECS_SET: categories['dimensions'].add(word_lower); continue
        ...
        if len(lemma) > 5: 
            for city in GEO_SET:
                if len(city) > 4 and lemma.startswith(city[:-1]): 
                    categories['geo'].add(lemma); is_geo_derivative = True; break
        if is_geo_derivative: continue

        if lemma in SERVICES_SET or lemma.endswith('обработка') or lemma.endswith('изготовление'): categories['services'].add(lemma); continue
...
# ИСПРАВЛЕННАЯ ИНИЦИАЛИЗАЦИЯ (добавлены все переменные из Таба 1)
...
with tab_gen:
    ...
    if st.button("🚀 ЗАПУСТИТЬ ГЕНЕРАЦИЮ (ВСЕ В ОДНУ ТАБЛИЦУ)", type="primary", use_container_width=True):
        ...
        if use_sidebar:
                ...
                for key in keys:
                    ...
                    if level == 1:
                        if has_child: h += f'<li class="level-1-header"><span class="dropdown-toggle">{name}</span><ul class="collapse-menu list-unstyled">{r_tree(child, 2)}</ul></li>'
                         ...
                         if has_child:
                            h += f'<li class="level-2-header"><span class="dropdown-toggle">{name}</span><ul class="collapse-menu list-unstyled">{r_tree(child, 3)}</ul></li>'
                         else:
                            h += f'<li class="level-2-link-special"><a href="{url}">{name}</a></li>'
                    elif level >= 3:
                         h += f'<li class="level-3-link"><a href="{url}">{name}</a></li>'
                return h
            ...
            sidebar_html = f"""<div class="page-content-with-sidebar"><button id="mobile-menu-toggle" class="menu-toggle-button">☰</button><div class="sidebar-wrapper"><nav id="sidebar-menu"><ul class="list-unstyled components">{r_tree(tree)}</ul></nav></div></div>{SIDEBAR_ASSETS}"""
        ...
        if use_promo:
            ...
            promo_block = f"""{css_styles}<div class="outer-full-width-section"><div class="gallery-content-wrapper"><h3 class="gallery-title">{promo_title}</h3><div class="five-col-gallery">{items_html}</div></div></div>"""
