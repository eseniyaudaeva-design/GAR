...
# --- 2. ТАБЛИЦА (ФИНАЛЬНАЯ: БЕЗ CAPTION, УМНАЯ, УНИВЕРСАЛЬНАЯ) ---
                if use_tables and client:
                    for t_topic in table_prompts:
                        ...
                        if not t_topic or t_topic == "Характеристики":
                            topic_instruction = "Тема: Технические параметры / Модификации."

                        prompt_tbl = (
                            f"Ты — технический специалист. Сгенерируй HTML-таблицу для товара: '{header_for_ai}'.\n"
                            f"Контекст категории: '{page['name']}'.\n"
                            f"{topic_instruction}.\n\n"
                            
                            f"ВХОДНЫЕ ДАННЫЕ (КЛЮЧЕВЫЕ СЛОВА): [{tech_data}]\n\n"
                            
                            f"ТВОЯ ЗАДАЧА (СТРОГИЕ ПРАВИЛА):\n"
                            f"1. ПРИНЦИП ПЕРЕМЕННОЙ:\n"
                            f"   - Определи, какой параметр отличает одну единицу этого товара от другой (Размер? Модель? Артикул? Цвет?).\n"
                            f"   - Сделай этот параметр ПЕРВОЙ КОЛОНКОЙ.\n"
                            f"   - Остальные колонки — это характеристики, которые зависят от первой.\n\n"
                            
                            f"2. АНТИ-СПАМ (ЗАПРЕТЫ):\n"
                            f"   - ЗАПРЕЩЕНО писать название товара ('{header_for_ai}') внутри ячеек данных (td).\n"
                            f"   - ЗАПРЕЩЕНО создавать колонки, где значение повторяется во всех строках (например, 'Бренд', 'ГОСТ'). Такие данные выноси в названия колонок.\n"
                            f"   - НЕ дублируй информацию, которая уже описана в тексте словами.\n\n"
                            
                            f"3. РАБОТА С КЛЮЧЕВЫМИ СЛОВАМИ:\n"
                            f"   - Используй слова из списка ВХОДНЫЕ ДАННЫЕ.\n"
                            f"   - Вставь их МАКСИМУМ 1 РАЗ на всю таблицу.\n"
                            f"   - Место вставки: ТОЛЬКО заголовки колонок (th).\n\n"
                            
                            f"4. ЧИСТОТА ВЫВОДА (ВАЖНО!):\n"
                            f"   - ЗАПРЕЩЕНО использовать тег <caption> (заголовок над таблицей). Таблица должна быть без внешнего названия.\n"
                            f"   - Не генерируй никаких <h2>, <h3> или <p> перед таблицей.\n"
                            f"   - Выдай строго только код от <table> до </table>.\n\n"
                        ...
                        try:
                            ...
                            if start_idx != -1 and end_idx != -1:
                                ...
                                # Дополнительная очистка от caption, если он все-таки пролез
                                clean_table_inner = re.sub(r'<caption.*?>.*?</caption>', '', clean_table_inner, flags=re.DOTALL)
