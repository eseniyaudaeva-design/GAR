...
def generate_ai_content_blocks(client, base_text, tag_name, num_blocks=5, seo_words=None):
    ...
    5. <p> развернутый завершающий.

    SEO (строго):
    Используй ключи только из соответствующего набора:
    блок1: {kw_str_1}
    блок2: {kw_str_2}
    блок3: {kw_str_3}
    блок4: {kw_str_4}
    ...
    Для блоков, которые НЕ генерируются (выше {num_blocks}), их ключи не использовать вообще.
