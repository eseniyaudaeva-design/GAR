...
def generate_reviews_deepseek(api_key, h2_header, lsi_words, target_count, chosen_authors):
    ...
    for i in range(target_count):
        ...
        if lsi_words_for_review:
            lsi_rule = f"КАНДИДАТЫ ДЛЯ ВСТАВКИ: {', '.join(lsi_words_for_review)}. Не пытайся впихнуть невпихуемое: если слово не лезет в текст - ПРОСТО ВЫБРОСЬ ЕГО. То, что решишь использовать, ОБЯЗАТЕЛЬНО выдели жирным шрифтом (оберни в **...** с двух сторон)."
        else:
...
6. РАБОТА С LSI (УМНЫЙ ФИЛЬТР):
   ...
   - Использованные слова ОБЯЗАТЕЛЬНО склоняй и оборачивай в <b>...</b>.
7. ИМЕНА И ОПЕЧАТКИ:
